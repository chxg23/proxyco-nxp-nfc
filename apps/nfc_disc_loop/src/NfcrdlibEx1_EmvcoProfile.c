...
static phStatus_t EmvcoDataExchange(uint8_t *com_buffer, uint8_t cmdsize, uint8_t **resp_buffer,
    uint32_t *wRxLength);
...
void
EmvcoProfileProcess(phacDiscLoop_Sw_DataParams_t *pDataParams, phStatus_t eDiscStatus)
{
...
*/
static void
EmvcoRfReset(phacDiscLoop_Sw_DataParams_t *pDataParams)
{
...
*/
static phStatus_t
EmvcoDataLoopBack(phacDiscLoop_Sw_DataParams_t *pDataParams)
{
  ...
  if (respsize > 0) {
    ...
    console_printf("\nFCI not recieved\n");
#ifdef PPSE_NO_LE
    console_printf("Transaction Done Remove card\n");
#else
    console_printf("Transaction Failed Replace the card\n");
#endif
  }
...
*/
static phStatus_t
EmvcoDataExchange(uint8_t *com_buffer, uint8_t cmdsize, uint8_t **resp_buffer,
    uint32_t *wRxLength)
{
  phStatus_t status;
  uint8_t *ppRxBuffer;
  ...
  status = phpalI14443p4_Exchange(phNfcLib_GetDataParams(PH_COMP_PAL_ISO14443P4),
          PH_EXCHANGE_DEFAULT,
