...
typedef struct { /* phnpSnep_Sw_DataParams */
  ...
  void
  *plnLlcpDataParams;                   /**< LLCP Data Parameters structure pointer. */
  void
  *psSocket;                            /**< LLCP transport Socket structure pointer. */
  uint8_t
  bSnepVersion;                        /**< SNEP protocol version supported by Implementation. */
...
#include "../../src/comps/phnpSnep/src/Sw/phnpSnep_Sw.h"
...
phStatus_t phnpSnep_ClientInit(
    ...
    phnpSnep_Server_type_t
    eServerType,         /**< [In] Type of the SNEP Server to which Client has to connect. */
...
phStatus_t phnpSnep_ServerListen(
    void *pDataParams,                        /**< [In] Pointer to this layer's parameter structure. */
    uint32_t
    dwGetReqBuffSize,              /**< [In] Size of Buffer to store received NDEF Message in Get Request. */
