...
typedef struct phlnLlcp_Transport_Socket {
  phOsal_SemObj_t
  xSema;                                                /**< Handle to the semaphore. */
  phOsal_SemObj_t
  xRxSema;                                              /**< Handle to the Receive semaphore. */
  phlnLlcp_Socket_Seq_t
  sSeq;                                                 /**< Sequence of the I PDU. */
  struct phlnLlcp_Transport_Socket
    *pNext;                                                /**< Pointer to the next Registered Socket. */
  uint8_t
  *pUri;                                                 /**< Pointer to URI (Uniform Resource Identifier). */
  uint32_t
  dwBufLen;                                             /**< The size of the Socket Rx Buffer. */
  uint8_t
  *pbRxBuffer;                                           /**< Pointer to the Socket Rx buffer. Size of the Socket Buffer should be less than or
  ...
  uint32_t
  dwLength;                                             /**< Length of the data received. */
  uint16_t
  wRMiu;                                                /**< Remote Link's MIU. */
  /**
  ...
  uint8_t                            fReady;
  phlnLlcp_Transport_Socket_Type_t
  eSocketType;                                          /**< Based on this, I PDU (Connection-Oriented) or UI PDU (Connection-Less) will be sent. */
  phStatus_t
  wStatus;                                              /**< Determines the status of the Socket. */
  uint8_t
  bLsap;                                                /**< Local SAP (Service Access Point) address. */
  uint8_t
  bRsap;                                                /**< Remote SAP address. */
  uint8_t
  bState;                                               /**< Socket state can be as per <b>LLCP Socket States</b>. */
...
typedef struct { /* phlnLlcp_Sw_DataParams */
  uint16_t
  wId;                                                           /**< Layer ID for this component, NEVER MODIFY!. */
  void
  *pPalI18092DataParams;                                          /**< Pointer to PAL 18092mPI/18092mT parameter structure. */
  phlnLlcp_LMDataParams_t
  sLocalLMParams;                                                /**< Local General Bytes. */
  phlnLlcp_LMDataParams_t
  sRemoteLMParams;                                               /**< Remote General Bytes. */
  uint8_t
  bAgreedVersion;                                                /**< Agreed Version number after Version Agreement. */
  uint8_t
  bCurrentClientCnt;                                             /**< Number of Registered Client count. */
  uint8_t
  bSdpClientSAP;                                                 /**< SAP Address of the Client requested for SDP. */
  uint8_t
  bMacType;                                                      /**< MAC type initialized either PHLN_LLCP_INITIATOR OR PHLN_LLCP_TARGET. */
...
#include "../../src/comps/phlnLlcp/src/Sw/phlnLlcp_Sw.h"
...
phStatus_t phlnLlcp_Transport_Socket_Init(
    void *pDataParams,                                           /**< [In] Pointer to this layer's parameter structure. */
    phlnLlcp_Transport_Socket_t
    *psSocket,                       /**< [In] Pointer to Socket allocated by the Application. */
    phlnLlcp_Transport_Socket_Type_t
    eSocketType,                /**< [In] Type of Socket to be Initialized, indicating the End Point. */
...
phStatus_t phlnLlcp_Transport_Socket_Register(
    void *pDataParams,                                       /**< [In] Pointer to this layer's parameter structure. */
    phlnLlcp_Transport_Socket_t
    *pSocket,                    /**< [In] Pointer of Socket to be Registered. */
...
phStatus_t phlnLlcp_Transport_Socket_Send(
    void *pDataParams,                                           /**< [In] Pointer to this layer's parameter structure. */
    phlnLlcp_Transport_Socket_t
    *pClientSocket,                  /**< [In] Pointer of Client Socket. */
...
phStatus_t phlnLlcp_Transport_Socket_Unregister(
    void *pDataParams,                                     /**< [In] Pointer to this layer's parameter structure. */
    phlnLlcp_Transport_Socket_t
    *pSocket                   /**< [In] Pointer of Socket to be Unregistered. */
);
