    ...
    uint8_t bLayer4Comm)
{
  ...
  if (wNumBytesReceived == 1U) {
    if (0U != (bLayer4Comm)) {
      ...
      return PH_ADD_COMPCODE(status,
              PH_COMP_PAL_MIFARE); /* For compatibility reasons with SW stack we use here PAL MIFARE */
    }
    ...
    uint16_t wBlockNr, uint8_t bNumBlocks, uint8_t *pValue)
{
  ...
  if (0U != (bLayer4Comm)) {
    ...
    if (0U != (bWrappedMode)) {
      ...
      PH_CHECK_SUCCESS_FCT(wStatus, phalMfpEVx_Int_Send7816Apdu(
              ...
              (uint16_t)((bCmdLen - 1U /* Excluding the command code */) + (bNumBlocks *
  ...
  PH_CHECK_SUCCESS_FCT(wStatus, phalMfpEVx_Int_ComputeErrorResponse(wRespLen, pResponse[0],
          bLayer4Comm));
  ...
  if ((wOptions == PH_EXCHANGE_BUFFER_LAST) || (wOptions == PH_EXCHANGE_DEFAULT)) {
    ...
    if (pResponse[wRespLen - 1U] != PHAL_MFPEVX_RESP_ADDITIONAL_FRAME) {
      ...
      if (0U == bHasCustomBits) {
        ...
        PH_CHECK_SUCCESS_FCT(wStatus, phalMfpEVx_Int_ComputeErrorResponse(1, pResponse[wRespLen - 1],
                PH_ON));
      }
    }
