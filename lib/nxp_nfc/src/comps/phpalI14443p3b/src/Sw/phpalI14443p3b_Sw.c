...
)
{
  ...
  if ((bFsdi < PHPAL_I14443P3B_EMVCO_FRAMESIZE_MIN) &&
  ...
  while ((bTemp > 0U) && (PH_ERR_SUCCESS != (statusTmp & PH_ERR_MASK))) {
    ...
    if (pDataParams->bOpeMode == RD_LIB_MODE_EMVCO) {
      if ((((statusTmp & PH_ERR_MASK) == PH_ERR_IO_TIMEOUT) ||
              ((statusTmp & PH_ERR_MASK) == PH_ERR_NOISE_ERROR))) {
  ...
  if ((pDataParams->bOpeMode != RD_LIB_MODE_ISO) && (((pAtqb[11] & 0x01U) == 0U) &&
          ((pResp[0] & 0x0FU) != 0x0U))) {
    return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_PAL_ISO14443P3B);
  }
  ...
  *pMbli = (uint8_t)(pResp[0] >> 4U);
  ...
  if (bAtqbLen == 13U) {
    ...
    dwSfgt = (PHPAL_I14443P3B_SW_FWT_MIN_US * ((uint32_t)1U << bTemp)) +
        PHPAL_I14443P3B_SW_EXT_TIME_US;

    /* Perform SFGT Wait */
    if (dwSfgt > 0xFFFFU) {
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Wait(pDataParams->pHalDataParams,
              PHHAL_HW_TIME_MILLISECONDS, (uint16_t)(dwSfgt / 1000U)));
    } else {
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Wait(pDataParams->pHalDataParams,
              PHHAL_HW_TIME_MICROSECONDS, (uint16_t)dwSfgt));
    }
  }
...
)
{
  ...
  *pMoreCardsAvailable = 0x00;
  ...
  if ((bNumSlots > 4U) || ((bPupiLength > 0U) && (bPupiLength != 4U) &&
          (pDataParams->bPollCmd != PHPAL_I14443P3B_USE_WUPB))) {
    return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_PAL_ISO14443P3B);
  }
  ...
  if (pDataParams->bOpeMode == RD_LIB_MODE_EMVCO) {
    status = phpalI14443p3b_WakeUpB(pDataParams, bNumSlots, bAfi, bExtAtqb, pLocalAtqb,
            &bLocalAtqbLen);
    ...
    while (((status & PH_ERR_MASK) == PH_ERR_IO_TIMEOUT) &&
        (bCount < PH_NXPNFCRDLIB_CONFIG_EMVCO_RETRYCOUNT)) {
      bCount++;
      /* Wait for at least Tmin retransmission delay. */
    ...
    if (bPupiLength != 0U) {
      ...
      status = phpalI14443p3b_RequestB(pDataParams, bNumSlots, bAfi, bExtAtqb, pLocalAtqb,
              &bLocalAtqbLen);
    }
  }
...
)
{
  ...
  */
  ...
  if (pDataParams->bOpeMode == RD_LIB_MODE_ISO) {
    ...
    bTR2 = (uint8_t)((pAtqb[10] & PHPAL_I14443P3B_SW_MINIMUM_TR2_MASK) >>
    ...
    if ((uint8_t)((pAtqb[10] & PHPAL_I14443P3B_SW_MINIMUM_TR2_MASK) >>
            PHPAL_I14443P3B_SW_MINIMUM_TR2_POS)
...
phpalI14443p3b_Sw_SetReaderBaudRateEx(phpalI14443p3b_Sw_DataParams_t *pDataParams)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
          PHHAL_HW_CONFIG_TXDATARATE_FRAMING, wTxDataRate));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
          PHHAL_HW_CONFIG_RXDATARATE_FRAMING, wRxDataRate));
