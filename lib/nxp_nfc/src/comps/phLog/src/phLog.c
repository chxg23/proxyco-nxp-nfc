...
#include <nxp_nfc/phLog.h>

#ifdef NXPBUILD__PH_LOG

#include <nxp_nfc/ph_Status.h>
#include <nxp_nfc/ph_RefDefs.h>
#include "nxp_nfc/phLog_Int.h"

static const uint8_t       *PH_MEMLOC_CONST_ROM gpkphLog_String_BufferOverflow =
    (uint8_t *)"OVERFLOW";
...
)
{
  ...
  /* Add Register Entry */
  gphLog_LogDataParams.pRegisterEntries[gphLog_LogDataParams.wNumRegisterEntries].pDataParams =
      pDataParams;
  gphLog_LogDataParams.pRegisterEntries[gphLog_LogDataParams.wNumRegisterEntries].pLogEntries =
      pLogEntries;
  gphLog_LogDataParams.pRegisterEntries[gphLog_LogDataParams.wNumRegisterEntries].wNumLogEntries =
      0;
  gphLog_LogDataParams.pRegisterEntries[gphLog_LogDataParams.wNumRegisterEntries].wMaxLogEntries =
      wMaxLogEntries;
...
)
{
  ...
  (*(gphLog_LogDataParams.pLogCallback))(pDataParams, bOption, pRegisterEntry->pLogEntries,
...
)
{
  ...
  /* Else return buffer overflow error */
    ...
    pRegisterEntry->pLogEntries[pRegisterEntry->wNumLogEntries].pString =
        (uint8_t const *)gpkphLog_String_BufferOverflow;
