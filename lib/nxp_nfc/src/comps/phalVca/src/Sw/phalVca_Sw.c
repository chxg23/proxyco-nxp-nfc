    ...
    phalVca_Sw_CardTableEntry_t *pCardTableStorage, uint16_t wNumCardTableStorageEntries)
{
  ...
  if (0U != (wNumIidTableStorageEntries)) {
    PH_ASSERT_NULL_PARAM(pIidTableStorage, PH_COMP_AL_VCA);
  }
  if (0U != (wNumCardTableStorageEntries)) {
    ...
    uint16_t wKeyNumber, uint16_t wKeyVersion)
{
  ...
  if ((status  & PH_ERR_MASK) == PH_ERR_INVALID_PARAMETER) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phCryptoRng_Rnd(pDataParams->pCryptoRngDataParams,
            PHAL_VCA_IID_SIZE - 1, &aTxBuffer[2]));
  } else {
    PH_CHECK_SUCCESS(status);
    (void)memcpy(&aTxBuffer[2], &pDataParams->pCardTable[wValidIidIndex].pCardData[1],
        PHAL_VCA_IID_SIZE - 1);
  }
    ...
    uint16_t wKeyMacNumber, uint16_t wKeyMacVersion)
{
  ...
  pDataParams->pIidTable[pDataParams->wCurrentIidTablePos].wIidIndex =
    ...
    uint16_t wKeyEncNumber, uint16_t wKeyEncVersion, uint16_t wKeyMacNumber, uint16_t wKeyMacVersion)
{
  ...
  /* Generate RNDQ */
  PH_CHECK_SUCCESS_FCT(statusTmp, phCryptoRng_Rnd(pDataParams->pCryptoRngDataParams, 12,
    ...
    uint8_t *pDFname, uint8_t *pFCI, uint16_t *pwFCILen)
{
   ...
   */
  PH_CHECK_SUCCESS_FCT(statusTmp, phalVca_Sw_SetConfig(pDataParams, PHAL_VCA_ADDITIONAL_INFO,
          PH_ERR_SUCCESS));
  ...
  if (pDataParams->bExtendedLenApdu) {
    ...
    bCmdBuff[wCmdLen++] =
        0x00;    /* first byte will be 0x00 if Ext apdu present. Next 2 byte contains actual data */
    bCmdBuff[wCmdLen++] =
        0x00;    /* As of now this field will be set to 0x00 since maximum data that can be sent is 16 bytes. In case if data to be sent exceeds 255 bytes, this byte shall be used. */
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phalVca_Int_ComputeErrorResponse_Extended(pDataParams,
          statusTmp));
  if (wRxlen != 0U) {
    ...
    if (wRxlen == 0x02) {
      ...
      *pwFCILen = wRxlen - 2 ;    /* FCI Length should be 36 bytes excluding 2 bytes of status data */
      ...
      if ((pRecv[0] == 0x6F) && (pRecv[1] == 0x22) && (pRecv[2] == 0x85) && (pRecv[3] == 0x20)) {
        if (memcpy(pFCI, &pRecv[0], 36) ==
            NULL) { /* Copy the entire 36 bytes of data (RndChal + Payload) */
    ...
    uint8_t *pInData, uint16_t wKeyNumber, uint16_t wKeyVersion)
{
  ...
  if (0U != (pDataParams->bExtendedLenApdu)) {
    ...
    bCmdBuff[wCmdLen++] = 0x00;
    bCmdBuff[wCmdLen++] =
        0x08;  /* As per ref arch ver 0.15, Response to Challenge  will be 8 bytes always.*/
    ...
    uint8_t bNumSteps, uint16_t wKeyNumber, uint16_t wKeyVersion, uint8_t *pUsedRndRC)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phalVca_Sw_Cmd_ProximityCheck(pDataParams, bGenerateRndC, pRndC,
          bNumSteps, pUsedRndRC));

  return (phalVca_Sw_Cmd_VerifyProximityCheck(pDataParams, pUsedRndRC, bPps1, wKeyNumber,
              wKeyVersion));
}
    ...
    uint8_t *pCumRndRC)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phalVca_Sw_Cmd_PrepareProximityCheckNew(pDataParams, pOption,
          pPubRespTime, pResponse, pRespLen));

  PH_CHECK_SUCCESS_FCT(statusTmp, phalVca_Sw_Cmd_ProximityCheckNew(pDataParams, bGenerateRndC,
          pPrndC, bNumSteps, pPubRespTime, pCumRndRC));

  return (phalVca_Sw_Cmd_VerifyProximityCheckNew(pDataParams, *pOption, pPubRespTime, pResponse,
    ...
    uint16_t *pRespLen)
{
  ...
  /* send "Prepare Proximity Check" command */
  statusTmp = phalVca_Int_PrepareProximityCheckNew(pDataParams, pOption, pPubRespTime, pResponse,
          pRespLen);
    ...
    uint16_t wRespLen, uint16_t wKeyNumber, uint16_t wKeyVersion, uint8_t *pRndCmdResp)
{
  ...
  uint8_t     PH_MEMLOC_REM aTmpBuf[15 /* Cmd / Rsp + Option + PubRspTime + PPS + ActBitRate */ +
         PHAL_VCA_PC_RND_LEN_NEW * 2];
  ...
  (void)memcpy(Key_bak, ((phCryptoSym_Sw_DataParams_t *)pDataParams->pCryptoDataParams)->pKey,
      PH_CRYPTOSYM_SW_MAX_BLOCK_SIZE);
  ...
  /* restore backup key */
  (void)memcpy(((phCryptoSym_Sw_DataParams_t *)pDataParams->pCryptoDataParams)->pKey, Key_bak,
      PH_CRYPTOSYM_SW_MAX_BLOCK_SIZE);

  /* Compare MAC */
  if (memcmp(bMac, (((pDataParams->bWrappedMode) != 0U) ? &pResponseTmp[0] : &pResponseTmp[1]),
          PHAL_VCA_TRUNCATED_MAC_SIZE) != 0) {
    return PH_ADD_COMPCODE_FIXED(PHAL_VCA_ERR_AUTH, PH_COMP_AL_VCA);
  }
    ...
    uint8_t *pSessionKey, uint8_t bAuthMode)
{
  ...
  switch (bAuthMode) {
    ...
    case PHAL_MFDFEVX_AUTHENTICATEAES:
      ...
      (void)memcpy(pDataParams->bSessionAuthMACKey, pSessionKey,
          sizeof(pDataParams->bSessionAuthMACKey));
      break;
