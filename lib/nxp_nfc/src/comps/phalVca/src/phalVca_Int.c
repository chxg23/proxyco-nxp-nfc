    ...
    uint8_t *pRndC, uint8_t *pRndRC)
{
  ...
  /* parameter checking */
  if ((bNumSteps == 0U) || (bNumSteps > PHAL_VCA_PC_RND_LEN) || (pRndC == NULL) ||
      (pRndRC == NULL)) {
    ...
    uint8_t *pPubRespTime, uint8_t *pRndC, uint8_t *pRndRC)
{
  ...
  /* parameter checking */
  if ((bNumSteps == 0U) || (bNumSteps > PHAL_VCA_PC_RND_LEN_NEW) || (pRndC == NULL) ||
      (pRndRC == NULL)) {
  ...
  while (0U != (bNumSteps--)) {
    ...
    if (0U != (wValue & 0x01U)) {
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pPalDataParamsTmp->pHalDataParams,
              PHHAL_HW_CONFIG_TIMING_MODE, PHHAL_HW_TIMING_MODE_FDT));
    }
    /* Check for ISO Wrapped Mode */
    ...
    if (0U != (wValue & 0x01U)) {
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_GetConfig(pPalDataParamsTmp->pHalDataParams,
              PHHAL_HW_CONFIG_TIMING_US, &wValue));
  ...
  switch (pApdu[1]) {
    case PHAL_VCA_CMD_PPC:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phpalMifare_ExchangeL4(
              ...
              PH_EXCHANGE_DEFAULT,
              pApdu,
              (uint16_t)((pDataParams->bExtendedLenApdu != 0U) ? 7U :
                  5U),    /* 2 bytes Le should be passed in case of Extended Length Apdu since Lc field is not present */
              pResponse,
              pRxlen
          ));
      break;
