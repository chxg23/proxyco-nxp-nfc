...
#include "nxp_nfc/phhalHw_PN7462AU.h"
#include "nxp_nfc/phhalRf.h"
#include "nxp_nfc/phUser.h"
#include "nxp_nfc/phhalRf_LP_Int.h"
#include "nxp_nfc/phhalHw_PN7462AU_Int.h"
    ...
    uint16_t wConfig, uint16_t wValue)
{
  ...
  switch (wConfig) {
          ...
          case PHHAL_HW_CARDTYPE_ISO15693: {
            ...
            if ((wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) == PHHAL_HW_RF_TX_DATARATE_1_OUT_OF_4) {
              if (0U != (pDataParams->bAsk100)) {
                status = phhalRf_LoadProtocol_Initiator(E_PHHAL_RF_LP_I_TX_15693_100,
                        E_PHHAL_RF_LP_I_RX_KEEP_ASIS);
              } else {
                status = phhalRf_LoadProtocol_Initiator(E_PHHAL_RF_LP_I_TX_15693_10,
                        E_PHHAL_RF_LP_I_RX_KEEP_ASIS);
              }
            }
      ...
      if (((wValue & PHHAL_HW_RF_FRAMING_OPTION_MASK) == PHHAL_HW_RF_TYPE_ACTIVE_FRAMING)
        ...
        if (0U != (pDataParams->wTargetMode)) {
          status = phhalRf_LoadProtocol_Target(E_PHHAL_RF_LP_T_TX_KEEP_ASIS,
                  (phhalRf_LP_T_Rx_t)(PHHAL_HW_CARDTYPE_ISO14443A + (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) +
                      1U));
        } else {
          status = phhalRf_LoadProtocol_Initiator(E_PHHAL_RF_LP_I_TX_KEEP_ASIS,
                  (phhalRf_LP_I_Rx_t)(PHHAL_HW_CARDTYPE_ISO14443A + (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) +
                      1U));
        }
      } else if (((wValue & PHHAL_HW_RF_FRAMING_OPTION_MASK) == PHHAL_HW_RF_TYPE_B_FRAMING)
          && ((wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) < 4U)) {
        if (0U != (pDataParams->wTargetMode)) {
          status = phhalRf_LoadProtocol_Target(E_PHHAL_RF_LP_T_TX_KEEP_ASIS,
                  (phhalRf_LP_T_Rx_t)(PHHAL_HW_CARDTYPE_ISO14443B + (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) +
                      8U));
        } else {
          status = phhalRf_LoadProtocol_Initiator((phhalRf_LP_I_Tx_t)E_PHHAL_RF_LP_I_TX_KEEP_ASIS,
                  (phhalRf_LP_I_Rx_t)(PHHAL_HW_CARDTYPE_ISO14443B + (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) +
                      8U));
        }
      } else if (((wValue & PHHAL_HW_RF_FRAMING_OPTION_MASK) == PHHAL_HW_RF_TYPE_F_FRAMING)
          && ((wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) < 4U)) {
          ...
          case PHHAL_HW_CARDTYPE_ISO15693: {
            switch (wValue) {
              ...
              case PHHAL_HW_RF_RX_DATARATE_FAST_HIGH: {
                status = phhalRf_SetConfig(PHHAL_RF_CONFIG_15693_RX_BAUDRATE,
                        (uint32_t)(PHHAL_HW_15693_FASTHIGH));
              }
              break;
              case PHHAL_HW_RF_DATARATE_106: {
    ...
    case PHHAL_HW_CONFIG_CRCTYPE: {
      ...
      status = phhalRf_SetConfig(PHHAL_RF_CONFIG_CRCINVERTED,
              (uint32_t)((wValue & PH_COMP_MASK) >> 8U));
...
)
{
  ...
  if (0U != (pDataParams->bRxMultiple)) {
    ...
    phUser_MemSet(&pDataParams->pRxBuffer[pDataParams->wRxBufStartPos], 0x00,
    ...
    uint16_t wTxLength)
{
  phStatus_t status = PH_ERR_INTERNAL_ERROR;
  /* Check options */
  if (wOption & (~(PH_TRANSMIT_BUFFERED_BIT | PH_TRANSMIT_LEAVE_BUFFER_BIT |
    ...
    uint16_t *pRxLength, uint16_t *pProtParams)
{
  ...
  do {
    ...
    status = phhalRf_AutoColl(&pDataParams->pAutocoll, pDataParams->bPiccAState,
...

{
  ...
  phUser_MemSet(&pDataParams->pAutocoll, 0x00, sizeof(pDataParams->pAutocoll));
  if (0U != (pSensRes)) {
    phUser_MemCpy(&pDataParams->pAutocoll.pSenseRes, pSensRes,
        sizeof(pDataParams->pAutocoll.pSenseRes));
  }
  if (0U != (pNfcid1)) {
  ...
  if (0U != (pSystemCode)) {
    phUser_MemCpy(&pDataParams->pAutocoll.pSystemCode, pSystemCode,
        sizeof(pDataParams->pAutocoll.pSystemCode));
  }
...
phhalHw_PN7462AU_Lpcd(phhalHw_PN7462AU_DataParams_t *pDataParams)
{
  ...
  status = phhalRf_LPCD(pDataParams->dwLpcdThreshold, pDataParams->dwLpcdReference,
          &pDataParams->dwLpcd_NewValue);
