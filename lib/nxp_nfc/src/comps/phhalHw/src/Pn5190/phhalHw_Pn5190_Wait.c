...
)
{
  ...
  if ((E_PH_OSAL_EVT_ABORT & *pReceivedEvents) != 0U) {
    phOsal_EventClear(&pDataParams->HwEventObj.EventHandle, E_OS_EVENT_OPT_NONE, E_PH_OSAL_EVT_ABORT,
        NULL);
...
#ifndef _WIN32
    PH_CHECK_SUCCESS_FCT(statustmp, phhalHw_Pn5190_Int_UserAbort(pDataParams));

    statustmp = phhalHw_Pn5190_EventWait_AbortHandling(pDataParams, dwEventsReceived,
    ...
    uint32_t *dwEventsReceived, uint32_t dwEvtsTimeOut)
{
  ...
  if ((dwEventReg & (PH_PN5190_EVT_IDLE | PH_PN5190_EVT_BOOT | PH_PN5190_EVT_GENERAL_ERROR |
              PH_PN5190_EVT_LPCD | PH_PN5190_EVT_AUTOCOLL)) != 0U) {
    ...
    if ((dwEventReg & (PH_PN5190_EVT_LPCD | PH_PN5190_EVT_AUTOCOLL | PH_PN5190_EVT_BOOT)) != 0U) {
      ...
      if (((dwEventReg == PH_PN5190_EVT_AUTOCOLL) &&
              ((dwEvtStatusReg & 0x000000FF) != PH_ERR_PN5190_USER_CALNCELLED))
          || ((dwEventReg == PH_PN5190_EVT_LPCD) &&
              (dwEvtStatusReg != PHHAL_HW_LPCD_EVT_STATUS_HIF_ACTIVITY))
