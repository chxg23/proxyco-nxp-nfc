...
)
{
  ...
  if ((dwStartCond & dwStartNowMask) == dwStartNowMask) {
    phOsal_EventClear(&pDataParams->HwEventObj.EventHandle, E_OS_EVENT_OPT_NONE, E_PH_OSAL_EVT_SIG,
        NULL);
  }
  ...
  PH_CHECK_SUCCESS_FCT(status, phhalHw_Pn5190_Instr_WriteRegisterMultiple(pDataParams,
  ...
  switch (wConfig) {
    case PHHAL_HW_CONFIG_PARITY:
      ...
      if (wValue == PH_OFF) {
        ...
        /* Turn ON Rx-Parity */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegisterOrMask(pDataParams,
                CLIF_RX_PROTOCOL_CONFIG, CLIF_RX_PROTOCOL_CONFIG_RX_PARITY_ENABLE_MASK));
      }
      ...
      if (PHHAL_HW_CARDTYPE_ISO15693 == pDataParams->bCardType) {
        ...
        if (wValue == PH_OFF) {
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_LoadRfConfiguration(pDataParams,
                  PHHAL_HW_PN5190_RF_TX_ISO15693_26_1OF4_ASK10, 0xFF));
        }
        /* switch on 100% ASK */
        else {
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_LoadRfConfiguration(pDataParams,
                  PHHAL_HW_PN5190_RF_TX_ISO15693_26_1OF4_ASK100, 0xFF));
        }
      }
      break;
    ...
    case PHHAL_HW_CONFIG_RXWAIT_US:
      ...
      if (0U != dwTemp) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegister(pDataParams, CLIF_RX_WAIT,
                dwValue));
      } else {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegister(pDataParams, CLIF_RX_WAIT,
                dwValue));
...
)
{
  ...
  if (pDataParams->bOpeMode != RD_LIB_MODE_FELICA) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegister(pDataParams,
            FELICA_EMD_CONTROL, PHHAL_HW_PN5190_DEFAULT_FELICA_EMD_REGISTER));
...
)
{
  ...
  if (wValue != PH_OFF) {
    ...
    if (pDataParams->dwFelicaEmdReg & dwMaskValue) {
      /* Update FELICA_EMD_CONTROL Register */
      PH_CHECK_SUCCESS_FCT(statusTmp,
          phhalHw_Pn5190_Instr_WriteRegisterAndMask(pDataParams, FELICA_EMD_CONTROL,
              (uint32_t)~dwMaskValue));
      /* Update shadow register */
    ...
    uint16_t wSubcarrier)
{
  ...
  if ((PHHAL_HW_PN5190_CURRENT_RF_CONFIGURATION_INDEX != bTxConfig) ||
      (PHHAL_HW_PN5190_CURRENT_RF_CONFIGURATION_INDEX != bRxConfig)) {
    PH_CHECK_SUCCESS_FCT(statusTmp,
    ...
    uint16_t wConfig, uint16_t wValue)
{
  ...
  if ((pDataParams->wCfgShadow[wConfig] & PHHAL_HW_RF_DATARATE_OPTION_MASK) !=
    ...
    if (pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO15693) {
      if (wValue == PHHAL_HW_RF_TX_DATARATE_1_OUT_OF_4) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5190_Instr_WriteRegisterAndMask(pDataParams,
                SYSTEM_CONFIG, (uint32_t)(~SYSTEM_CONFIG_15693_CHANGE_DATARATE_MASK)));
        /* set the value */
        PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5190_Instr_WriteRegisterOrMask(pDataParams,
                SYSTEM_CONFIG, (uint32_t)(PHHAL_HW_15693_26KBPS)));
        ...
        /* set the value */
        PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5190_Instr_WriteRegisterOrMask(pDataParams,
                SYSTEM_CONFIG, (uint32_t)(PHHAL_HW_15693_53KBPS)));
        ...
        /* set the value */
        PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5190_Instr_WriteRegisterOrMask(pDataParams,
                SYSTEM_CONFIG, (uint32_t)(PHHAL_HW_15693_106KBPS)));
        ...
        /* set the value */
        PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5190_Instr_WriteRegisterOrMask(pDataParams,
                SYSTEM_CONFIG, (uint32_t)(PHHAL_HW_15693_212KBPS)));
      } else {
        /* To avoid the warning */
        return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER,
                PH_COMP_HAL);    /* return the invalid parameter */
      }
    }
...
)
{
  ...
  if (bUnit == PHHAL_HW_TIME_MICROSECONDS) {
    ...
    if (wTimeout <= PHHAL_HW_PN5190_MAX_TIME_DELAY_MS) {
      dwLoadValue = (uint32_t)(wTimeout * (PHHAL_HW_PN5190_MIN_FREQ  /
...
)
{
  ...
  if (wNumByteReceived != 0) {
    PH_CHECK_SUCCESS_FCT(status, phhalHw_Pn5190_Instr_RetrieveRFData(pDataParams, ppRxBuffer,
            pRxLength));
