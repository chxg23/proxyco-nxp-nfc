    ...
    phhalHw_InstMngr_CmdParams_t *pCmdParams, uint8_t *pTxBuffer, uint8_t bExchAction)
{
  ...
  if (pDataParams->wTxBufStartPos == 0U) {
    pTxBuffer[pDataParams->wTxBufStartPos++] =
        bCmd; /* Reserved 0th Location of buffer for Command code */
  } else {
    ...
    phhalHw_InstMngr_CmdParams_t *pCmdParams, uint16_t wTlv_Length)
{
  ...
  if (status == PH_ERR_SUCCESS) {
    ...
    if (((status & PH_ERR_MASK) == PH_ERR_ABORTED) ||
        ((status & PH_ERR_MASK) == PH_OSAL_IO_TIMEOUT)) {
    ...
    phhalHw_InstMngr_CmdParams_t *pCmdParams)
{
  ...
  if (wRespLength > STATUS_FIELD_LEN) {
    do {
      ...
      wCopyRespLength = (((wDataLenToRead > 0) ? (pDataParams->sIrqResp.bIsrBytesRead - bHeaderLen) :
                  wRespLength) - STATUS_FIELD_LEN);
...
//#ifdef T_UTF
//    uint8_t     PH_MEMLOC_REM bWaitLoop = 10U;
//#endif /* T_UTF */
//#endif /* NXPBUILD__PHHAL_HW_DEV_PN5190 */
  ...
  if (bCheckIRQ == PH_ON) {
    ...
    do {
      if (phDriver_PinRead(PHDRIVER_PIN_IRQ, PH_DRIVER_PINFUNC_INPUT) == 0) {
        break;
      } else {
