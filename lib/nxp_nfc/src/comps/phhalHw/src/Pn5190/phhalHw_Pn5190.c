...
)
{
  ...
  (void)phOsal_EventClear(&pDataParams->HwEventObj.EventHandle, E_OS_EVENT_OPT_NONE,
      (E_PH_OSAL_EVT_RF | E_PH_OSAL_EVT_SIG), NULL);
...
static phStatus_t phhalHw_Pn5190_EnBootNormalMode(phhalHw_Pn5190_DataParams_t *pDataParams)
{
  ...
  pDataParams->dwEventStatus |= ((uint32_t) pDataParams->sIrqResp.aISRReadBuf[EVENT_LSB_INDEX + 5])
      << 8U ;
  pDataParams->dwEventStatus |= ((uint32_t) pDataParams->sIrqResp.aISRReadBuf[EVENT_LSB_INDEX + 6])
      << 16U;
  pDataParams->dwEventStatus |= ((uint32_t) pDataParams->sIrqResp.aISRReadBuf[EVENT_LSB_INDEX + 7])
      << 24U;
  ...
  if (!((bEvent == 0x80) && (dwEventStatusReg == PH_PN5190_EVT_BOOT))) {
    if (dwEventStatusReg & PH_PN5190_EVT_GENERAL_ERROR) {
      ...
      pDataParams->dwGenError |= ((uint32_t) pDataParams->sIrqResp.aISRReadBuf[EVENT_LSB_INDEX + 9]) <<
          8U ;
      pDataParams->dwGenError |= ((uint32_t) pDataParams->sIrqResp.aISRReadBuf[EVENT_LSB_INDEX + 10]) <<
          16U;
      pDataParams->dwGenError |= ((uint32_t) pDataParams->sIrqResp.aISRReadBuf[EVENT_LSB_INDEX + 11]) <<
          24U;
    }
    return PH_ADD_COMPCODE_FIXED(PH_ERR_INTERFACE_ERROR, PH_COMP_HAL);
  }
...
)
{
  ...
  /* Disable the EMD. */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_SetConfig(pDataParams, PHHAL_HW_CONFIG_SET_EMD,
          PH_OFF));
...
)
{
  ...
  if (pDataParams->bActiveMode == PH_ON) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_ApplyProtocolSettings(pDataParams,
...
)
{
  ...
  switch (pDataParams->bCardType) {
    ...
    case PHHAL_HW_CARDTYPE_ISO14443A:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_LoadRfConfiguration(pDataParams,
              PHHAL_HW_PN5190_RF_TX_ISO14443A_106_MILLER,
              PHHAL_HW_PN5190_RF_RX_ISO14443A_106_MANCH_SUBC));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_ISO14443B:
      /* Use 14443b shadow */
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_LoadRfConfiguration(pDataParams,
              PHHAL_HW_PN5190_RF_TX_ISO14443B_106_NRZ,
              PHHAL_HW_PN5190_RF_RX_ISO14443B_106_BPSK));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_FELICA_212:
      /* Use Felica shadow */
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_LoadRfConfiguration(pDataParams,
              PHHAL_HW_PN5190_RF_TX_FELICA_212,
              PHHAL_HW_PN5190_RF_RX_FELICA_212));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_FELICA_424:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_LoadRfConfiguration(pDataParams,
              PHHAL_HW_PN5190_RF_TX_FELICA_424,
              PHHAL_HW_PN5190_RF_RX_FELICA_424));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_I18092M_ACTIVE_106:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_LoadRfConfiguration(pDataParams,
              PHHAL_HW_PN5190_RF_TX_NFC_AI_106_106,
              PHHAL_HW_PN5190_RF_RX_NFC_AI_106));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_I18092M_ACTIVE_212:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_LoadRfConfiguration(pDataParams,
              PHHAL_HW_PN5190_RF_TX_NFC_AI_212_212,
              PHHAL_HW_PN5190_RF_RX_NFC_AI_212));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_I18092M_ACTIVE_424:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_LoadRfConfiguration(pDataParams,
              PHHAL_HW_PN5190_RF_TX_NFC_AI_424_424,
              PHHAL_HW_PN5190_RF_RX_NFC_AI_424));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_ISO15693:
      /* Use 15693 shadow */
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_LoadRfConfiguration(pDataParams,
              PHHAL_HW_PN5190_RF_TX_ISO15693_26_1OF4_ASK100,
              PHHAL_HW_PN5190_RF_RX_ISO15693_26_SC));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_I18000P3M3:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_LoadRfConfiguration(pDataParams,
              PHHAL_HW_PN5190_RF_TX_ISO180003M3_TARI_9_44_ASK,
              PHHAL_HW_PN5190_RF_RX_ISO180003M3_MANCH848_4_PERIOD));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    for (wIndex = 0U; wIndex < wShadowCount; ++wIndex) {
      ...
      if ((wConfig == PHHAL_HW_CONFIG_TXCRC) ||
        ...
        if (pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO14443B) {
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_SetConfig(pDataParams, wConfig,
...
)
{
  ...
  switch (wConfig) {
    ...
    case PHHAL_HW_CONFIG_RXBUFFER_STARTPOS:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Int_GetRxBuffer(pDataParams, &pBuffer, &wBufferLen,
              &wBufferSize));
    ...
    case PHHAL_HW_CONFIG_TXBUFFER_LENGTH:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Int_GetTxBuffer(pDataParams, &pBuffer, &wBufferLen,
              &wBufferSize));
    ...
    case PHHAL_HW_CONFIG_TXDATARATE_FRAMING:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Int_SetTxDataRateFraming(pDataParams, wConfig,
              wValue));
      break;

    case PHHAL_HW_CONFIG_RXDATARATE_FRAMING:

      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Int_SetRxDataRateFraming(pDataParams, wConfig,
              wValue));

      if (((pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXDATARATE_FRAMING] &
                  PHHAL_HW_RF_DATARATE_OPTION_MASK) != PHHAL_HW_RF_DATARATE_106)
          && ((pDataParams->wCfgShadow[wConfig] & PHHAL_HW_RF_DATARATE_OPTION_MASK) ==
              PHHAL_HW_RF_DATARATE_106)) {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_SetConfig_Int(pDataParams,
                PHHAL_HW_CONFIG_RXWAIT_US, 70U));
      }
      break;
    ...
    case PHHAL_HW_CONFIG_TXWAIT_US:
      ...
      if (pDataParams->wCfgShadow[wConfig] != wValue) {
        ...
        if (0U != dwTemp) {
          ...
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegister(pDataParams, CLIF_TX_WAIT,
                  dwValue));
        } else {
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegister(pDataParams, CLIF_TX_WAIT,
                  dwValue));
    ...
    case PHHAL_HW_CONFIG_JEWEL_MODE:
      ...
      if (wValue == PH_ON) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegisterOrMask(pDataParams,
                CLIF_TRANSCEIVE_CONTROL, CLIF_TRANSCEIVE_CONTROL_TX_FRAMESTEP_ENABLE_MASK));
      } else if (wValue == PH_OFF) {
        pDataParams->bJewelActivated = (uint8_t)wValue;
        /* Disable FrameStep in Transceive Control Register. */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegisterAndMask(pDataParams,
                CLIF_TRANSCEIVE_CONTROL, (uint32_t)~(CLIF_TRANSCEIVE_CONTROL_TX_FRAMESTEP_ENABLE_MASK)));
      } else {
        return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
      }
      break;
    ...
    case PHHAL_HW_CONFIG_SET_EMD:
      ...
      if (wValue == 0x1U) {
        ...
        if (pDataParams->bCardType != PHHAL_HW_CARDTYPE_ISO14443A) {
          ...
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_ReadRegister(pDataParams, EMD_CONTROL,
                  &dwRegister));
    ...
    case PHHAL_HW_CONFIG_RXMULTIPLE:
      ...
      if (wValue == PH_ON) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegisterOrMask(pDataParams,
                CLIF_TRANSCEIVE_CONTROL, CLIF_TRANSCEIVE_CONTROL_RX_MULTIPLE_ENABLE_MASK));
        /* Reset Timer1 (FDT Timer) stop condition. */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegisterAndMask(pDataParams,
                CLIF_TIMER1_CONFIG, (uint32_t)~(CLIF_TIMER1_CONFIG_T1_STOP_ON_RX_STARTED_MASK)));
      } else {
        pDataParams->bRxMultiple = PH_OFF;
        /* Disable Rx Multiple in Transceive Control Register. */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegisterAndMask(pDataParams,
                CLIF_TRANSCEIVE_CONTROL, (uint32_t)~(CLIF_TRANSCEIVE_CONTROL_RX_MULTIPLE_ENABLE_MASK)));
        /* Set Timer1 (FDT Timer) stop condition to Stop on RX Start condition. */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegisterOrMask(pDataParams,
                CLIF_TIMER1_CONFIG, CLIF_TIMER1_CONFIG_T1_STOP_ON_RX_STARTED_MASK));
      }
      break;
    ...
    case PHHAL_HW_CONFIG_MFHALTED:
      ...
      if (wValue == PH_ON) {
        PH_CHECK_SUCCESS_FCT(statusTmp,
            phhalHw_Pn5190_Instr_WriteRegisterOrMask(pDataParams, SYSTEM_CONFIG,
                SYSTEM_CONFIG_AUTOCOLL_STATE_A_MASK));
        pDataParams->bCardMode  = PH_ON;
      } else {
        PH_CHECK_SUCCESS_FCT(statusTmp,
            phhalHw_Pn5190_Instr_WriteRegisterAndMask(pDataParams, SYSTEM_CONFIG,
    ...
    case PHHAL_HW_CONFIG_SET_SYMBOL_SEND:
      ...
      if (wValue == PH_ON) {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegisterAndMask(pDataParams,
                CLIF_TX_FRAME_CONFIG,  ~(uint32_t)CLIF_TX_FRAME_CONFIG_TX_DATA_ENABLE_MASK));
      }
      /* PH_OFF implies Setting Bit-3 (DataEn Bit in TxDataNum). */
      else {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegisterOrMask(pDataParams,
                CLIF_TX_FRAME_CONFIG, CLIF_TX_FRAME_CONFIG_TX_DATA_ENABLE_MASK));
      }
      break;
    ...
    case PHHAL_HW_CONFIG_SYMBOL_START:
      ...
      if (wValue == PH_OFF) {
        dwTemp = (uint32_t) ~(CLIF_TX_FRAME_CONFIG_TX_START_SYMBOL_MASK |
                CLIF_TX_FRAME_CONFIG_TX_DATA_ENABLE_MASK);
      } else {
        dwTemp = (uint32_t) ~(CLIF_TX_FRAME_CONFIG_TX_START_SYMBOL_MASK);
      }
      ...
      /*Set the new value  */
      dwTemp = (uint32_t)((dwValue << CLIF_TX_FRAME_CONFIG_TX_START_SYMBOL_POS) &
              CLIF_TX_FRAME_CONFIG_TX_START_SYMBOL_MASK);
      dwTemp |= (uint32_t)((dwValue << CLIF_TX_FRAME_CONFIG_TX_DATA_ENABLE_POS) &
              CLIF_TX_FRAME_CONFIG_TX_DATA_ENABLE_MASK);
      wRegTypeValueSets[wSizeOfRegTypeValueSets++] = CLIF_TX_FRAME_CONFIG;
    ...
    case PHHAL_HW_PN5190_CONFIG_CTS_EVENT_ENABLE:
      if (wValue == PH_ON) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegister(pDataParams, EVENT_ENABLE,
                EVENT_ENABLE_CTS_EVENT_ENABLE_MASK));
      } else if (wValue == PH_OFF) {
        /* Disable CTS Event Interrupt*/
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegisterAndMask(pDataParams,
                EVENT_ENABLE,  ~(uint32_t)EVENT_ENABLE_CTS_EVENT_ENABLE_MASK));
    ...
    case PHHAL_HW_PN5190_CONFIG_LPCD_EVENT_ENABLE:
      if (wValue == PH_ON) {
        /* Enable LPCD Event */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegisterOrMask(pDataParams,
                EVENT_ENABLE, EVENT_ENABLE_LPCD_EVENT_ENABLE_MASK));
      } else if (wValue == PH_OFF) {
        /* Disable LPCD Event */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_WriteRegisterAndMask(pDataParams,
                EVENT_ENABLE, (uint32_t)~(EVENT_ENABLE_LPCD_EVENT_ENABLE_MASK)));
      } else {
        return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
      }
      break;
    ...
    case PHHAL_HW_CONFIG_SET_FELICA_EMD:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp,
          phhalHw_Pn5190_Int_SetConfig_FelicaEmdRegBit(pDataParams, wValue,
...
)
{
  ...
  switch (wConfig) {
    ...
    case PHHAL_HW_CONFIG_INT_RF_ON:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_Instr_ReadRegister(pDataParams, CLIF_STATUS,
              &dwRegister));
...
)
{
  ...
  return phhalHw_Pn5190_Instr_WriteE2Prom(pDataParams, PHHAL_HW_PN5190_SET_LISTEN_EEPROM_ADDR, aCmd,
...
)
{
  ...
  bRespConfig = PH_RF_EXCH_CONF_ENABLE_RF_STATUS | PH_RF_EXCH_CONF_ENABLE_RX_STATUS |
      PH_RF_EXCH_CONF_ENABLE_EVT_STATUS;
  ...
  if (pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXLASTBITS] != 0U) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5190_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXLASTBITS,
            0x00U));
  }
...
)
{
  ...
  *  since the data to transmit would get overwritten by received data. */
  ...
  status =  phhalHw_Pn5190_Instr_TransmitRFData(pDataParams, wOption, bNumberOfValidBits, pTxBuffer,
          wTxLength);
  PH_CHECK_ABORT(status);
...
)
{
  ...
  /* Disable the EMD. */
  PH_CHECK_SUCCESS_FCT(status, phhalHw_Pn5190_SetConfig(pDataParams, PHHAL_HW_CONFIG_SET_EMD,
          PH_OFF));
  ...
  *pRxLength = 0U;
  ...
  PH_CHECK_ABORT_FCT(status, phhalHw_Pn5190_Instr_SwitchModeAutocoll(pDataParams, bRfTechnology,
          PHHAL_HW_PN5190_AUTOCOLL_MODE_TERMINATE_ON_RF_OFF));
    ...
    if (status == PH_ERR_SUCCESS) {
      if (dwEventReceived == PH_PN5190_EVT_AUTOCOLL) {
        if (pEvtPayload[PHHAL_HW_PN5190_AUTOCOLL_STATUS_INDEX] == PHHAL_HW_PN5190_AUTOCOLL_ACTIVATED) {
          PH_CHECK_SUCCESS_FCT(status,  phhalHw_Pn5190_Instr_ReadRegister(pDataParams, CLIF_RX_STATUS_ERROR,
                  &dwRegister));
          ...
          if ((dwRegister & CLIF_RX_STATUS_ERROR_RX_CL_ERROR_MASK) == 0U) {
            ...
            if ((pEvtPayload[PHHAL_HW_PN5190_AUTOCOLL_TECH_ACT_INDEX]
                    & PHHAL_HW_PN5190_AUTOCOLL_ACTIVATION_MODE_MASK) ==
                PHHAL_HW_PN5190_AUTOCOLL_ACTIVATION_PASSIVE_MODE)
            ...
            if (((*pProtParams & PHHAL_HW_RF_DATARATE_OPTION_MASK) == PHHAL_HW_RF_DATARATE_106) &&
                (**ppRxBuffer == PHHAL_HW_PN5190_I18092MT_START_BYTE)) {
        ...
        } else if (pEvtPayload[PHHAL_HW_PN5190_AUTOCOLL_STATUS_INDEX] ==
            PHHAL_HW_PN5190_AUTOCOLL_EXTRF_OFF) {
  ...
  do {
    ...
    if (bUnit == PHHAL_HW_TIME_MILLISECONDS) {
      ...
      while (wTimeout > PHHAL_HW_PN5190_MAX_TIME_DELAY_MS) {
        ...
        dwLoadValue = (uint32_t)(PHHAL_HW_PN5190_MAX_TIME_DELAY_MS * (wFreq  /
                    PHHAL_HW_PN5190_CONVERSION_MS_SEC));
...
)
{
  ...
  if ((pDataParams->bLpcdMode == PHHAL_HW_PN5190_LPCD_MODE_DEFAULT) ||
    ...
    status = phhalHw_Pn5190_Instr_SwitchModeLpcd(pDataParams, pDataParams->bLpcdConfig,
            pDataParams->bLpcdWakeUpCtrl,
