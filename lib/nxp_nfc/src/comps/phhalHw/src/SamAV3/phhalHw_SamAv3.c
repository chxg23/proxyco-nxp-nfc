...
phStatus_t phhalHw_SamAV3_Init(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wSizeOfDataParams, void * pBalDataParams,
		void * pReaderHalDataParams, void * pKeyStoreDataParams, void * pCryptoENCDataParams, void * pCryptoMACDataParams,
		void * pCryptoRngDataParams, void * pPLUpload_CryptoENCDataParams, void * pPLUpload_CryptoMACDataParams, uint8_t bOpMode,
		uint8_t bLogicalChannel, uint8_t* pTxBuffer, uint16_t wTxBufSize, uint8_t* pRxBuffer, uint16_t wRxBufSize,	uint8_t* pPLUploadBuf)
{
	...
	uint8_t ** ppRxBuffer, uint16_t * pRxLength)
{
    ...
    /* Check options */
    if (wOption & (uint16_t)~(uint16_t)(PH_EXCHANGE_BUFFERED_BIT | PH_EXCHANGE_LEAVE_BUFFER_BIT | PHHAL_HW_SAMAV3_EXCHANGE_NO_ENCIPHERING_BIT | PHHAL_HW_SAMAV3_EXCHANGE_NO_DECIPHERING_BIT))
    {
        return PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
    }
    ...
    *pRxLength = 0;
    ...
    else
    {
        ...
        if (!(pDataParams->bMifareCryptoDisabled) && !(wOption & PHHAL_HW_SAMAV3_EXCHANGE_NO_ENCIPHERING_BIT))
        {
        ...
        pDataParams->pTxBuffer[pDataParams->wTxBufStartPos + PHHAL_HW_SAMAV3_ISO7816_P1_POS] = (uint8_t)pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXLASTBITS];

        /* Modify LC byte */
        pDataParams->pTxBuffer[pDataParams->wTxBufStartPos + PHHAL_HW_SAMAV3_ISO7816_LC_POS] = (uint8_t)(pDataParams->wTxBufLen - PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH);
...
//		}
	...
	memcpy(pDataParams->bUid, &aVersion[PHHAL_HW_SAMAV3_CMD_GET_VERSION_RESPONSE_UID_OFFSET], PHHAL_HW_SAMAV2_HC_SAM_UID_SIZE);	/* PRQA S 3200 */
	...
	switch (wConfig)
	{
		...
		case PHHAL_HW_CONFIG_TXBUFFER:
			...
			if (pDataParams->wTxBufLen_Cmd >= (PHHAL_HW_SAMAV3_RESERVED_TX_BUFFER_LEN - 1))
			{
				...
				pDataParams->pTxBuffer[(PHHAL_HW_SAMAV3_RESERVED_TX_BUFFER_LEN - 1) + pDataParams->wTxBufStartPos + pDataParams->wAdditionalInfo] = (uint8_t)wValue;
			}
			/* Normal Exchange */
			else
			{
				...
				/* Modify TxBuffer byte */
				pDataParams->pTxBuffer[PHHAL_HW_SAMAV3_RESERVED_TX_BUFFER_LEN + pDataParams->wTxBufStartPos + pDataParams->wAdditionalInfo] = (uint8_t)wValue;
			}
			break;
...
phStatus_t phhalHw_SamAV3_GetConfig(phhalHw_SamAV3_DataParams_t * pDataParams, uint16_t wConfig, uint16_t * pValue)
{
    ...
    case PHHAL_HW_CONFIG_TXBUFFER_BUFSIZE:
        ...
        /* Return parameter */
        *pValue = pDataParams->wTxBufSize - (PHHAL_HW_SAMAV3_RESERVED_TX_BUFFER_LEN + pDataParams->wTxBufStartPos);
        break;
...
phStatus_t phhalHw_SamAV3_ApplyProtocolSettings(phhalHw_SamAV3_DataParams_t * pDataParams, uint8_t bCardType)
{
    ...
    for (wIndex = 0; wIndex < wShadowCount; ++wIndex)
    {
            ...
            (pDataParams->bTimeoutUnit != PHHAL_HW_TIME_MILLISECONDS))))
        {
            ...
            {
                PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_SamAV3_SetConfig(pDataParams, wConfig, pShadowDefault[(wIndex << 1) + 1]));
            }
            /* Current shadow: */
            else
            {
                PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_SamAV3_SetConfig(pDataParams, wConfig, pDataParams->wCfgShadow[wConfig]));
            }
        }
    }
...
phStatus_t phhalHw_SamAV3_FieldReset(phhalHw_SamAV3_DataParams_t * pDataParams)
{
    ...
    /* Perform field reset */
    PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_SamAV3_Cmd_RC_RFControl(pDataParams, (uint8_t)pDataParams->wFieldOffTime));
...
phStatus_t phhalHw_SamAV3_Utils_UpdateLc(phhalHw_SamAV3_DataParams_t * pDataParams)
{
    ...
    pTmpBuffer[PHHAL_HW_SAMAV3_ISO7816_LC_POS] = (uint8_t)(pDataParams->wTxBufLen_Cmd - PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH);
