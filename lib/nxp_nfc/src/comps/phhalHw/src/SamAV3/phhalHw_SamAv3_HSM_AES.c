...
* Private constants
*/
static const uint8_t PH_MEMLOC_CONST_ROM
phhalHw_SamAV3_Hc_AV2_FirstIv[PH_CRYPTOSYM_AES_BLOCK_SIZE] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

phStatus_t
phhalHw_SamAV3_HSM_AES_GetFirstLastCommand(phhalHw_SamAV3_DataParams_t *pDataParams, uint8_t aCmd,
    uint8_t p1, uint8_t p2, uint8_t *bFirstCmd,
    uint8_t *bLastCmd)
{
  *bFirstCmd = PH_ON;
  *bLastCmd = PH_ON;
...
phStatus_t
phhalHw_SamAV3_HSM_AES_GetFirstLastResponse(phhalHw_SamAV3_DataParams_t *pDataParams, uint8_t sw1,
    uint8_t sw2, uint8_t *bFirstResponse,
...
phStatus_t
phhalHw_SamAV3_HSM_AES_AppendMac(phhalHw_SamAV3_DataParams_t *pDataParams, uint8_t *pBuffer,
    uint16_t wTxLength, uint16_t wBufferSize,
    uint16_t *pTxLength, uint8_t bFirst, uint8_t bLast)
{
  ...
  *pTxLength = wTxLength;
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SamAV3_Utils_GetCheckLcLe(pBuffer, wTxLength, &bLcPresent,
          &bLc, &bLePresent));
  ...
  if (bFirst) {
    ...
    /* Copy pending data */
    memcpy(aTmpBuf, pDataParams->bPendingMacCmdData,
        (uint16_t)pDataParams->bPendingMacCmdDataLength);  /* PRQA S 3200 */
  ...
  memcpy(&aTmpBuf[wValidMacData], &pBuffer[PHHAL_HW_SAMAV3_ISO7816_LC_POS + 1],
      (uint16_t)wHelper);  /* PRQA S 3200 */
  ...
  if (wValidMacData == 16) {
    ...
    if ((bLc > wHelper) || (bLePresent)) {
      ...
      memcpy(aTmpBuf, &pBuffer[wHelper + PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + wValidMacData],
          (uint16_t)bLc);  /* PRQA S 3200 */
      wValidMacData = bLc;
    }
  }
  ...
  if (bLast) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp,
        phhalHw_SamAV3_Utils_TruncateMacBuffer(pDataParams->bPendingCmdMac, &bMacLength));

    /* Append MAC at end of buffer */
    if ((bLcPresent) && (bLePresent)) {
      /* Le is still on the correct position, so copy mac and move Le */
      memcpy(&pBuffer[wTxLength - 1], pDataParams->bPendingCmdMac,
          (uint16_t)bMacLength);  /* PRQA S 3200 */
      ...
      /* Le is still on the correct position, so copy mac and move Le */
      memcpy(&pBuffer[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH], pDataParams->bPendingCmdMac,
          (uint16_t)bMacLength);  /* PRQA S 3200 */
...
phStatus_t
phhalHw_SamAV3_HSM_AES_Decrypt(phhalHw_SamAV3_DataParams_t *pDataParams, uint8_t *pBuffer,
    uint16_t wRxLength, uint16_t *pRxLength, uint8_t bFirst,
    uint8_t bLast)
{
  ...
  *pRxLength = 0;
  ...
  if (!bLast) {
    ...
    memcpy(pDataParams->bPendingRespIv,
        &pBuffer[wRxLength - PHHAL_HW_SAMAV3_ISO7816_SW1SW2_LENGTH - 16],
...
phStatus_t
phhalHw_SamAV3_HSM_AES_VerifyRemoveMac(phhalHw_SamAV3_DataParams_t *pDataParams, uint8_t *pBuffer,
    uint16_t wRxLength, uint16_t *pRxLength, uint8_t bFirst,
    uint8_t bLast)
{
  phStatus_t  PH_MEMLOC_REM statusTmp;
  ...
  if (wRxLength > 2 && ((pDataParams->bResponseChaining == PHHAL_HW_SAMAV3_HSM_AES_CHAINING) ||
          (!bFirst && bLast))) {
    if (bFirst) {
      pDataParams->bPendingRespDataLength = 0;
    } else {
      memcpy(aTmpBuf, pDataParams->bPendingRespData,
          pDataParams->bPendingRespDataLength);  /* PRQA S 3200 */
    ...
    if (!bLast) {
      ...
      memcpy(pDataParams->bPendingRespData,
          &pBuffer[(wRxLength - 2) - pDataParams->bPendingRespDataLength],
          pDataParams->bPendingRespDataLength);  /* PRQA S 3200 */
      memcpy(&pBuffer[(wRxLength - 2) - pDataParams->bPendingRespDataLength], &pBuffer[(wRxLength - 2)],
          PHHAL_HW_SAMAV3_ISO7816_SW1SW2_LENGTH);  /* PRQA S 3200 */
  ...
  *pRxLength = 0;
  ...
  if (bFirst) {
    ...
    if ((pBuffer[wRxLength - PHHAL_HW_SAMAV3_ISO7816_SW1SW2_LENGTH] == 0x90) &&
        (pBuffer[wRxLength - 1] == 0xAF)) {
      aTmpBuf[wValidMacData++] = 0x00;
    } else {
    ...
    /* Get pending data*/
    memcpy(aTmpBuf, pDataParams->bPendingMacRespData,
        (uint16_t)pDataParams->bPendingMacRespDataLength);  /* PRQA S 3200 */
  ...
  if (bLast) {
    ...
    /* we have to truncate the MAC*/
    PH_CHECK_SUCCESS_FCT(statusTmp,
        phhalHw_SamAV3_Utils_TruncateMacBuffer(pDataParams->bPendingRespMac, &bMacLength));
    ...
    if (memcmp(pDataParams->bPendingRespMac,
            &pBuffer[wRxLength - (PHHAL_HW_SAMAV3_ISO7816_SW1SW2_LENGTH + bMacLength)], bMacLength)) {
      return PH_ADD_COMPCODE(PHHAL_HW_SAMAV3_ERR_CRYPTO, PH_COMP_HAL);
    }
    ...
    *pRxLength = wRxLength - bMacLength;
    ...
    /* Reorder SW1 SW2 */
    pBuffer[wRxLength - (PHHAL_HW_SAMAV3_ISO7816_SW1SW2_LENGTH + bMacLength)] = pBuffer[wRxLength -
            PHHAL_HW_SAMAV3_ISO7816_SW1SW2_LENGTH];
...
phStatus_t
phhalHw_SamAV3_HSM_AES_Encrypt(phhalHw_SamAV3_DataParams_t *pDataParams, uint8_t *pBuffer,
    uint16_t wTxLength, uint16_t wBufferSize, uint16_t *pTxLength,
    uint8_t bFirst, uint8_t bLast)
{
  ...
  *pTxLength = wTxLength;
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SamAV3_Utils_GetCheckLcLe(pBuffer, wTxLength, &LcPresent,
          &bLc, &LePresent));

  if (bFirst) {
    /* load the InitializationVector, because we start a new Encryption.
    This has to be done even if no data are send to the SAM. */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SamAV3_HSM_AES_InitAndLoadIV(pDataParams,
            pDataParams->bPendingCmdIv, true));
  } else {
    /* Load encryption IV */
    PH_CHECK_SUCCESS_FCT(statusTmp, phCryptoSym_LoadIv(
            pDataParams->pENCCryptoDataParams,
  ...
  if (bFirst) {
    ...
    memcpy(bTmpBuf, &pBuffer[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + wCurrentEncryptedDataSize],
        wRemainingDataSize);  /* PRQA S 3200 */
  } else {
    ...
    if (wHelper > bLc) {
      memcpy(bTmpBuf, pDataParams->bPendingEncCmdData,
          pDataParams->bPendingEncCmdDataLength);  /* PRQA S 3200 */
      memcpy(&bTmpBuf[pDataParams->bPendingEncCmdDataLength],
          &pBuffer[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH], bLc);  /* PRQA S 3200 */
      wRemainingDataSize = pDataParams->bPendingEncCmdDataLength + bLc;
    } else {
      memcpy(&pDataParams->bPendingEncCmdData[pDataParams->bPendingEncCmdDataLength],
          &pBuffer[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH], wHelper);  /* PRQA S 3200 */
      ...
      if (wCurrentEncryptedDataSize) {
        ...
        memcpy(bTmpBuf, &pBuffer[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + wHelper +
                                                      wCurrentEncryptedDataSize ], wRemainingDataSize);  /* PRQA S 3200 */

        /* Now move encrypted payload to the end */
        memmove(&pBuffer[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + 16],
            &pBuffer[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + wHelper],
            (uint16_t)wCurrentEncryptedDataSize);  /* PRQA S 3200 */
      ...
      memcpy(&pBuffer[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH], pDataParams->bPendingEncCmdData,
          16);  /* PRQA S 3200 */
  ...
  if (bLast) {
    ...
    if (wCurrentEncryptedDataSize >= 16) {
      /* Recopy last block of the encrypted data into the temporary IV space */
      memcpy(pDataParams->bPendingCmdIv,
          &pBuffer[PHHAL_HW_SAMAV3_ISO7816_HEADER_LENGTH + wCurrentEncryptedDataSize - 16],
...
phStatus_t
phhalHw_SamAV3_HSM_AES_InitAndLoadIV(phhalHw_SamAV3_DataParams_t *pDataParams, uint8_t *pIV,
