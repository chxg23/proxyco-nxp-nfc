...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
          PHHAL_HW_RC663_REG_FIFOLENGTH, &bValue));
  ...
  if (pDataParams->bFifoSize == PH_NXPNFCRDLIB_CONFIG_RC663_VALUE_FIFOSIZE_512) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
            PHHAL_HW_RC663_REG_FIFOCONTROL, &bValue));
  ...
  if (wLength > 0U) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteData(pDataParams, PHHAL_HW_RC663_REG_FIFODATA,
            pData, wLength));
  }
...
)
{
  ...
  if (*pLength > 0U) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadData(pDataParams, PHHAL_HW_RC663_REG_FIFODATA,
            *pLength, pData));
  }
...
)
{
  ...
  /* RS232 protocol */
    ...
    for (wIndex = 0; wIndex < wLength; ++wIndex) {
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, bAddress,
              pData[wIndex]));
    }
  }
...
)
{
  ...
  /* RS232 protocol */
    ...
    for (wIndex = 0; wIndex < wLength; ++wIndex) {
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, bAddress,
              &pData[wIndex]));
    }
  }
...
)
{
  ...
  if (pDataParams->bLoadRegMode != PH_OFF) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
            PHHAL_HW_RC663_REG_DRVMODE, &bTxConfig));
    ...
    bRegister = (bTxConfig & PHHAL_HW_RC663_BIT_TXEN) | (bRegister & (uint8_t)~
            (uint8_t)PHHAL_HW_RC663_BIT_TXEN);
    /* Write the DrvMode Register. */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
            PHHAL_HW_RC663_REG_DRVMODE, bRegister));
  ...
  /* Apply RegisterSet using library register set */
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetConfig_Int(pDataParams, PHHAL_HW_CONFIG_ASK100,
            pDataParams->wCfgShadow[PHHAL_HW_CONFIG_ASK100]));
  }

  /* Apply the TxWait as this changes for ISO, EMVCO. */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetTxWait(pDataParams,
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetConfig_Int(pDataParams, PHHAL_HW_CONFIG_PARITY,
          pDataParams->wCfgShadow[PHHAL_HW_CONFIG_PARITY]));

  /* Apply Tx-CRC settings from Shadow */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetConfig_Int(pDataParams, PHHAL_HW_CONFIG_TXCRC,
          pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXCRC]));

  /* Apply Rx-CRC settings from Shadow */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetConfig_Int(pDataParams, PHHAL_HW_CONFIG_RXCRC,
          pDataParams->wCfgShadow[PHHAL_HW_CONFIG_RXCRC]));

  /* Apply TxLastBits settings from Shadow */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetConfig_Int(pDataParams,
          PHHAL_HW_CONFIG_TXLASTBITS, pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXLASTBITS]));
...
)
{
  ...
  switch (wConfig) {
    case PHHAL_HW_CONFIG_PARITY:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
              PHHAL_HW_RC663_REG_FRAMECON, &bRegister));
      /* Parity is OFF */
      if (wValue == PH_OFF) {
        bValue = bRegister & (uint8_t)~(uint8_t)(PHHAL_HW_RC663_BIT_TXPARITYEN |
                PHHAL_HW_RC663_BIT_RXPARITYEN);
      }
      /* Parity is ON */
      else {
      ...
      if (bValue != bRegister) {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
                PHHAL_HW_RC663_REG_FRAMECON, bValue));
      }
      break;
    ...
    case PHHAL_HW_CONFIG_TXCRC:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
              PHHAL_HW_RC663_REG_TXCRCCON, &bRegister));
      ...
      if (bValue != bRegister) {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
                PHHAL_HW_RC663_REG_TXCRCCON, bValue));
      }
      break;
    ...
    case PHHAL_HW_CONFIG_RXCRC:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
              PHHAL_HW_RC663_REG_RXCRCCON, &bRegister));
      ...
      if (bValue != bRegister) {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
                PHHAL_HW_RC663_REG_RXCRCCON, bValue));
      }
      break;
    ...
    case PHHAL_HW_CONFIG_MODINDEX:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_TXAMP,
              &bRegister));
      ...
      /* Only perform the operation, if the new value is different */
      if (bValue != bRegister) {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_TXAMP,
                bValue));
      }
    ...
    case PHHAL_HW_CONFIG_TXLASTBITS:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
              PHHAL_HW_RC663_REG_TXDATANUM, &bRegister));
    ...
    case PHHAL_HW_CONFIG_ASK100:
      ...
      /* Clear ClkMode bits */
      bValue = bRegister & (uint8_t)~(uint8_t)(PHHAL_HW_RC663_BIT_RFON | PHHAL_HW_RC663_BIT_TPUSHON |
              PHHAL_HW_RC663_BIT_TPULLON);
      ...
      if (bValue != bRegister) {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
                PHHAL_HW_RC663_REG_DRVMODE, bValue));
      }
      break;
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXWAIT, bRegister));
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXWAITCTRL, &bRegister));
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXWAITLO, bRegister));
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_T0RELOADHI, (uint8_t)(wT0Reload >> 8U)));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_T0RELOADLO, (uint8_t)(wT0Reload)));

  /* Write T1 reload value */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_T1RELOADHI, (uint8_t)(wT1Reload >> 8U)));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_T1RELOADLO, (uint8_t)(wT1Reload)));
...
)
{
  ...
  /* Get T0 reload value */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
          PHHAL_HW_RC663_REG_T0RELOADHI, &bRegister));
  wT0Reload = (uint16_t)bRegister << 8U;
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
          PHHAL_HW_RC663_REG_T0RELOADLO, &bRegister));
  wT0Reload |= (uint16_t)bRegister;

  /* Get T1 reload value */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
          PHHAL_HW_RC663_REG_T1RELOADHI, &bRegister));
  wT1Reload = (uint16_t)bRegister << 8U;
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
          PHHAL_HW_RC663_REG_T1RELOADLO, &bRegister));
  wT1Reload |= (uint16_t)bRegister;

  /* Get T0 counter value */
  if ((wExchangeStatus & PH_ERR_MASK) != PH_ERR_IO_TIMEOUT) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
            PHHAL_HW_RC663_REG_T0COUNTERVALHI, &bRegister));
    wT0Value = (uint16_t)bRegister << 8U;
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
            PHHAL_HW_RC663_REG_T0COUNTERVALLO, &bRegister));
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
          PHHAL_HW_RC663_REG_T1COUNTERVALHI, &bRegister));
  wT1Value = (uint16_t)bRegister << 8U;
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
          PHHAL_HW_RC663_REG_T1COUNTERVALLO, &bRegister));
...
)
{
  ...
  /* Normal command -> do not overwrite Tx- and Rx-Contents */
    ...
    /* Redundant code */
    return phhalHw_Rc663_GetRxBuffer(pDataParams, bIsExchange, pTxBuffer, pTxBufferLen,
            pTxBufferSize);
  }
...
)
{
  ...
  /* Check auth success */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_STATUS,
          &bRegister));
...
)
{
  ...
  switch (bCmd) {
    case PHHAL_HW_RC663_CMD_ACKREQ:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_Cmd_AckReq(pDataParams, &bBuffer[0], 3, pRxBuffer,
              pRxLength));
    ...
    default:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_GetRxBuffer(pDataParams, PH_OFF, &pTmpBuffer,
              &wFifoBytes, &wTmpBufferSize));
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
              PHHAL_HW_RC663_REG_COMMAND, PHHAL_HW_RC663_CMD_IDLE));
      ...
      if (0U != (wTxLength)) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteFifo(pDataParams, pTmpBuffer, wTxLength,
                &wFifoBytes));
      }
      ...
      if (0U != (bCmdNoWrite)) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_IRQ0EN,
                &bRegister));
        bRegister &= PHHAL_HW_RC663_BIT_IRQINV; /* clear all the interrupts */
        bRegister |= (0x7FU & bIrq0WaitFor); /* Do not modify the IRQINV bit. */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
                PHHAL_HW_RC663_REG_IRQ0EN, bRegister));

        /* Enable IRQ1 interrupt sources */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_IRQ1EN,
                &bRegister));
        bRegister = PHHAL_HW_RC663_BIT_IRQPUSHPULL;
        bRegister |= PHHAL_HW_RC663_BIT_GLOBALIRQ | bIrq1WaitFor;
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
                PHHAL_HW_RC663_REG_IRQ1EN, bRegister));
      }
      ...
      if (0U != (wOption & PHHAL_HW_RC663_RXTX_TIMER_START)) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
                PHHAL_HW_RC663_REG_TCONTROL, &bRegister));
        /* manually start T0 */
      ...
      if ((bCmd & (uint8_t)~(uint8_t)PHHAL_HW_RC663_BIT_STANDBY) == PHHAL_HW_RC663_CMD_LPCD) {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
                PHHAL_HW_RC663_REG_T4CONTROL, &bRegister));
        /* Start T4. */
      ...
      /* start the command */
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
              PHHAL_HW_RC663_REG_COMMAND, bCmd));

      /* wait until the command is finished */
      bWaitIrqEn = (bCmdNoWrite != PH_OFF) ? PH_OFF : PH_ON;
      bWaitIrqPowerDownCheck = ((bCmd == PHHAL_HW_RC663_CMD_SOFTRESET) ||
              (0U != (bCmd & PHHAL_HW_RC663_BIT_STANDBY))) ? PH_ON : PH_OFF;
      ...
      if ((pRxBuffer != NULL) &&
        ...
        do {
          ...
          *pRxLength = *pRxLength + wFifoBytes;
          ...
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_IRQ0,
                  &bIrq0Reg));
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_IRQ1,
                  &bIrq1Reg));
      ...
      /* */
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_ERROR,
              &bError));
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_TXAMP,
          0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_TXI,
          0x00U));

  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_RCV,
          0x12));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_SIGPROTEST, 0x00U));

  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_DRVCON, 0x01));
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_RXANA,
          0x0A));

  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_FRAMECON, 0x01));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXDATAMOD, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_TXBITMOD,
          0x80));

  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYM0L, 0x41));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYM0H, 0x68));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYM10LEN, 0x8E));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYM10BURSTLEN, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYM10BURSTCTRL, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYM10MOD, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYM2, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYM3, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYM32LEN, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYM32MOD, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYM1L, 0xA1));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYM1H, 0x01));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXMODWIDTH, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXWAITCTRL, 0x80));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXWAITLO, 0x12));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXDATACON, 0xC5));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXSYMFREQ, 0x05));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXCRCCON, 0x05));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXBITMOD, 0x08));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXWAIT, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXBITCTRL, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXEOFSYM, 0x0B));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXCOLL, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXCRCCON, 0x05));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXSYNCMOD, 0x04));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXSYNCVAlL, 0x01));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXSYNCVAlH, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXTHRESHOLD, 0x36));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXSVETTE, 0xF0));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_RXMOD,
          0x0C));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_LPO_TRIMM, 0x00U));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_DACVAL, 0x02));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_DACVAL, 0x27));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_ANAXTAL, 0x0A));

  /* Initialize FIFO */

  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_FIFOCONTROL, 0x14));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_WATERLEVEL, 0x00U));

  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TCONTROL, 0x0F));

  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXDATANUM, 0x0C));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXBITCTRL, 0x70));
...
)
{
  ...
  /* collision */
    ...
    /* Else perform collision handling */
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_RXCOLL,
              &bRegister));
...
phStatus_t
phhalHw_Rc663_GetMultiReg(phhalHw_Rc663_DataParams_t *pDataParams, uint8_t *pData)
{
...
phStatus_t
phhalHw_Rc663_CheckForEmdError(phhalHw_Rc663_DataParams_t *pDataParams,
    uint8_t bIrq0WaitFor, uint8_t bIrq1WaitFor, uint32_t *pdwMultiReg, uint32_t *pdwPrevReg)
{
  ...
  uint8_t    PH_MEMLOC_REM bIrq0EnReg_Orig =
      0x00; /* Original register value to ensure 8th bit is not touched. */

#ifndef _WIN32
  /* Clear all events that might have occurred. */
  PH_CHECK_SUCCESS_FCT(statusTmp, phOsal_EventClear(&pDataParams->HwEventObj.EventHandle,
          E_OS_EVENT_OPT_NONE, (E_PH_OSAL_EVT_RF | E_PH_OSAL_EVT_ABORT), NULL));
#endif
  ...
  do {
    ...
    if (((0U != (PHHAL_HW_EMD_NOISE_CHECK(aTempRxBuf[3]))) &&
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
              ...
              (PHHAL_HW_RC663_BIT_T0STARTSTOPNOW | PHHAL_HW_RC663_BIT_T1STARTSTOPNOW |
                  PHHAL_HW_RC663_BIT_T0RUNNING | PHHAL_HW_RC663_BIT_T1RUNNING)));
...
void
phhalHw_Rc663_CreateRespFrame(uint8_t *bInBuff, uint8_t bDataPos, uint8_t *bOutBuff)
{
  ...
  if (0U != (bTemp & PHHAL_HW_RC663_BIT_PROTERR)) {
    bOutBuff[bStartPos + PHHAL_HW_STATUS_FRAME_ERR_BYTE_POS] |= (PHHAL_HW_STATUS_FRAME_PROT_ERR_POS |
            PHHAL_HW_STATUS_FRAME_CL_ERR_POS);
  }

  /* Update Collision Error Status */
  if (0U != (bTemp & PHHAL_HW_RC663_BIT_COLLDET)) {
    bOutBuff[bStartPos + PHHAL_HW_STATUS_FRAME_ERR_BYTE_POS] |= (PHHAL_HW_STATUS_FRAME_COLL_ERR_POS |
            PHHAL_HW_STATUS_FRAME_CL_ERR_POS);
  }

  /* Update CRC and Parity Error Status */
  if (0U != (bTemp & PHHAL_HW_RC663_BIT_INTEGERR)) {
    bOutBuff[bStartPos + PHHAL_HW_STATUS_FRAME_ERR_BYTE_POS] |= (PHHAL_HW_STATUS_FRAME_DATA_ERR_POS |
            PHHAL_HW_STATUS_FRAME_CL_ERR_POS);
  }
}

phStatus_t
phhalHw_Rc663_FrameRxMultiplePkt(phhalHw_Rc663_DataParams_t *pDataParams)
{
  uint8_t bIndex;
  uint8_t bFrameCount;
...
phStatus_t
phhalHw_Rc663_18000p3m3_Sw_Exchange(
    phhalHw_Rc663_DataParams_t *pDataParams,
    uint16_t wOption,
    uint8_t *pTxBuffer,
    uint16_t wTxLength,
...
)
{
  ...
  /*  last Tx-Byte. */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXLASTBITS,
          bTxLastBits));
...
phStatus_t
phhalHw_Rc663_18000p3m3_Sw_Select(
    phhalHw_Rc663_DataParams_t *pDataParams,
    uint8_t *pSelCmd,
    uint8_t bSelCmdLen,
...
)
{
  ...
  /* set Tx-CRC as CRC16  */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams, PHHAL_HW_CONFIG_CRCTYPE,
          PHHAL_HW_CRCTYPE_CRC16 | PHHAL_HW_CRCOPTION_INVERTED));
...
phStatus_t
phhalHw_Rc663_18000p3m3_Sw_BeginRound(
    phhalHw_Rc663_DataParams_t *pDataParams,
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(
          pDataParams,
          PHHAL_HW_CONFIG_TIMEOUT_VALUE_US,
          PHHAL_HW_RC663_I18000P3M3_SW_T1_MAX_US + PHHAL_HW_RC663_I18000P3M3_SW_T3_MIN_US +
          PHHAL_HW_RC663_I18000P3M3_SW_DELTA_MIN_US));
...
phStatus_t
phhalHw_Rc663_18000p3m3_Sw_NextSlot(
    phhalHw_Rc663_DataParams_t *pDataParams
)
{
  phStatus_t  PH_MEMLOC_REM statusTmp;
  ...
  /* Build Command frame */
  aCmd[0] = (PHHAL_HW_RC663_I18000P3M3_CMD_NEXT_SLOT << 4U) | (pDataParams->bSession <<
          2U); /* 2 bits left */
...
phStatus_t
phhalHw_Rc663_18000p3m3_Sw_Ack(
    phhalHw_Rc663_DataParams_t *pDataParams,
    uint8_t **ppRxBuffer,
    uint16_t *pRxLength,
    uint8_t *pRxLastBits
)
{
  ...
  /* Build Command frame */
  aCmd[0] = (PHHAL_HW_RC663_I18000P3M3_CMD_ACK << 6U) | (pDataParams->abStoredCRC[0] >>
          2U);    /* 0 bits left */
  aCmd[1] = (pDataParams->abStoredCRC[0] << 6U) | (pDataParams->abStoredCRC[1] >>
          2U);                    /* 0 bits left */
  aCmd[2] = (pDataParams->abStoredCRC[1] <<
          6U);                                      /* 6 bits left */
...
phStatus_t
phhalHw_Rc663_18000p3m3_Sw_ReqRn(
    phhalHw_Rc663_DataParams_t *pDataParams,
    uint8_t **ppRxBuffer
)
{
...
phStatus_t
phhalHw_Rc663_Int_ReadDerivative(phhalHw_Rc663_DataParams_t *pDataParams,
    uint8_t *pFEDerivate,
    uint8_t *pVersion)
{
  phStatus_t  PH_MEMLOC_REM statusTmp;
  ...
  if (pDataParams->wDerivative == 0U) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
            PHHAL_HW_RC663_REG_VERSION, pVersion));

    /* Update the derivative in pDataParams. */
    pDataParams->wDerivative = ((uint16_t)(*pFEDerivate) << PHHAL_HW_DERIVATIVE_POS) &
        PHHAL_HW_DERIVATIVE_MASK;
    pDataParams->wDerivative |= ((uint16_t)(*pVersion) << PHHAL_HW_VERSION_POS) &
        PHHAL_HW_VERSION_MASK;
  } else {
...
phStatus_t
phhalHw_Rc663_Int_ProtocolSupport(phhalHw_Rc663_DataParams_t *pDataParams,
    uint8_t bMode)
{
  phStatus_t  PH_MEMLOC_REM status;
  phStatus_t  PH_MEMLOC_REM statusTmp;
...
void
phhalHw_Rc663_WriteSSEL(void *pBalDataParams, uint8_t bValue)
{
#ifndef _WIN32
