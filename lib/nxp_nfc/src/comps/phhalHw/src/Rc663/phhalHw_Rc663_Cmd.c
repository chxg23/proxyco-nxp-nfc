...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, bTimerAdd,
          (uint8_t)(dwReload >> 8U)));
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_LPCD_RESULT_Q, 0));
  ...
  if ((0U == ((pDataParams->bMode & PHHAL_HW_RC663_CMD_LPCD_MODE_OPTION_IGNORE_IQ)))) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
            PHHAL_HW_RC663_REG_LPCD_QMIN, bRegister));

    /* Set Qmax register */
    bRegister = bQMax | (uint8_t)((bIMax & 0x0CU) << 4U);
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
            PHHAL_HW_RC663_REG_LPCD_QMAX, bRegister));

    /* Set Imin register */
    bRegister = bIMin | (uint8_t)((bIMax & 0x03U) << 6U);
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
            PHHAL_HW_RC663_REG_LPCD_IMIN, bRegister));
  }

  /* Set Mix2Adc bit */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_RCV,
          &bRegister));
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_RXANA,
          &bRxAnaBackup));
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_RXANA,
          PHHAL_HW_RC663_LPCD_RECEIVER_GAIN));
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_LPCD_RESULT_Q, PHHAL_HW_RC663_BIT_LPCDIRQ_CLR));
...
)
{
  ...
  /* Set Qmin register */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_LPCD_QMIN, PHHAL_HW_RC663_LPCD_CALIBRATE_QMIN_REG));

  /* Set Qmax register */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_LPCD_QMAX, PHHAL_HW_RC663_LPCD_CALIBRATE_QMAX_REG));

  /* Set Imin register */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_LPCD_IMIN, PHHAL_HW_RC663_LPCD_CALIBRATE_IMIN_REG));
  ...
  if (bVersion == PHHAL_HW_CLRC663PLUS_VERSION) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
            PHHAL_HW_RC663_REG_LPCD_OPTIONS, &bRegister));
    pDataParams->bLpcdOption |= bRegister & (PHHAL_HW_RC663_BIT_LPCD_Q_UNSTABLE |
            PHHAL_HW_RC663_BIT_LPCD_I_UNSTABLE);
  }
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
          PHHAL_HW_RC663_REG_LPCD_RESULT_I, pI));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
          PHHAL_HW_RC663_REG_LPCD_RESULT_Q, pQ));
...
)
{
  ...
  if ((wAddress >= PHHAL_HW_RC663_EEPROM_NUM_PAGES) || (bDataLen == 0U) ||
      (bDataLen > PHHAL_HW_RC663_EEPROM_PAGE_SIZE)) {
    return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
  }
...
)
{
  ...
  if ((wEEAddress < PHHAL_HW_RC663_EEPROM_SECTION2_BEGIN) ||
      ((wEEAddress + bNumBytes) > (PHHAL_HW_RC663_EEPROM_SECTION2_END + 1U))) {
    return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
  }
...
)
{
  ...
  /* Parameter check */
  if ((bTxProtocol > PHHAL_HW_RC663_MAX_TX_SETTINGS) ||
      (bRxProtocol > PHHAL_HW_RC663_MAX_RX_SETTINGS)) {
    return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
  }
...
)
{
  ...
  return phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_COMMAND,
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXCORR, 0x88));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXCTRL, 0x07));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXWAIT, 0x3D));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_TXWAITLO, 0x80));
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetConfig(pDataParams, PHHAL_HW_CONFIG_PARITY,
          PH_OFF));
  ...
  do {
    ...
    wNumPrecachedBytes = (wTxLength > pDataParams->wMaxPrecachedBytes) ?
        pDataParams->wMaxPrecachedBytes : wTxLength;
