...
#include <nxp_nfc/phhalHw_Rc663_Cmd.h>
...
static void phhalHw_Rc663_EventCallback(void *pDataParams)
{
  ...
  /* Post Abort Event. */
  (void)phOsal_EventPost(&pRc663DataParams->HwEventObj.EventHandle, E_OS_EVENT_OPT_POST_ISR,
  ...
  if (pDataParams->bPollGuardTimeFlag == PH_OFF) {
    ...
    if (pDataParams->bCardType == PHHAL_HW_CARDTYPE_UNKNOWN) {
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
              PHHAL_HW_RC663_REG_T0CONTROL, &bRegister));
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
              PHHAL_HW_RC663_REG_T0CONTROL, bRegister | PHHAL_HW_RC663_BIT_TAUTORESTARTED |
              PHHAL_HW_RC663_VALUE_TCLK_1356_MHZ));

      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
              PHHAL_HW_RC663_REG_T1CONTROL, &bRegister));
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
              PHHAL_HW_RC663_REG_T1CONTROL, bRegister | PHHAL_HW_RC663_VALUE_TCLK_T0));
    }

    /* Calculate values for Microsecond values */
    if (wConfig == PHHAL_HW_CONFIG_TIMEOUT_VALUE_US) {
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetFdt(pDataParams, PHHAL_HW_TIME_MICROSECONDS,
              wValue));
      pDataParams->bTimeoutUnit = PHHAL_HW_TIME_MICROSECONDS;
    }
    /* Calculate values for Millisecond values */
    else {
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetFdt(pDataParams, PHHAL_HW_TIME_MILLISECONDS,
              wValue));
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(status, phDriver_PinConfig(PHDRIVER_PIN_RESET, PH_DRIVER_PINFUNC_OUTPUT,
          &pinCfg));
  ...
  if ((pBalParams->bBalType == PHBAL_REG_TYPE_SPI) ||
    ...
    if (pBalParams->bBalType == PHBAL_REG_TYPE_SPI) {
      ...
      PH_CHECK_SUCCESS_FCT(status, phDriver_PinConfig(PHDRIVER_PIN_SSEL, PH_DRIVER_PINFUNC_OUTPUT,
              &pinCfg));
    }
...
)
{
  ...
  do {
    ...
    if ((pDataParams->bNfcipMode == PH_ON) &&
      ...
      if ((pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXDATARATE_FRAMING] == PHHAL_HW_RF_DATARATE_106) &&
        ...
        PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Rc663_GetTxBuffer(pDataParams, PH_ON, &pTmpBuffer,
    ...
    if (pDataParams->bJewelActivated == PH_ON) {
      ...
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Rc663_SetConfig(pDataParams, PHHAL_HW_CONFIG_PARITY,
              PH_OFF));

      /* Make sure CRC is OFF */
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Rc663_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXCRC,
              PH_OFF));
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Rc663_SetConfig(pDataParams, PHHAL_HW_CONFIG_RXCRC,
              PH_OFF));
    ...
    if (0U != (pDataParams->bCRC5Enabled)) {
       ...
       */
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Rc663_GetConfig(pDataParams, PHHAL_HW_CONFIG_TXCRC,
              &wIndex));
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Rc663_GetConfig(pDataParams, PHHAL_HW_CONFIG_RXCRC,
              &wVerifyRxCrc5));
      ...
      if (0U != (wIndex)) {
        ...
        PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Rc663_GetConfig(pDataParams, PHHAL_HW_CONFIG_TXLASTBITS,
                &wIndex));

        /* check condition is added to fix only QAC. Always wTmpBufferLen will be greater the 1 */
        if (wTmpBufferLen > 1U) {
          /* Get bit length */
          wIndex = ((wIndex > 0U) ? (((uint16_t)(wTmpBufferLen - 1U) << 3U) + wIndex) : ((uint16_t)(
                          wTmpBufferLen << 3U)));
        }
        ...
        pTmpBuffer[wTmpBufferLen - 1U] = (uint8_t)(pTmpBuffer[wTmpBufferLen - 1U] & (uint8_t)(0xFFU <<
                    (uint8_t)(8U - bBitlen)));
        ...
        /*  last Tx-Byte. */
        PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXLASTBITS,
                bBitlen));
      }
    }
  } while (FALSE);
  ...
  if (statusTmp == PH_ERR_SUCCESS) {
    ...
    do {
      ...
      if ((wTmpBufferLen == 0U) &&
        ...
        PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
        ...
        PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
                PHHAL_HW_RC663_REG_TXDATANUM, bRegister));
      } else {
        /* Reset NoData flag */
        bNoData = PH_OFF;
      ...
      if (pDataParams->bJewelActivated == PH_ON) {
        ...
        PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Rc663_TransmitJewelCmd(pDataParams, wTmpBufferLen,
                pTmpBuffer));
      ...
      wNumPrecachedBytes = (wTmpBufferLen > pDataParams->wMaxPrecachedBytes) ?
          pDataParams->wMaxPrecachedBytes : wTmpBufferLen;
  ...
  if (pDataParams->bPollGuardTimeFlag == PH_ON) {
    ...
    /* retrieve content of Timer Control register */
    PH_CHECK_SUCCESS_FCT(status, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_TCONTROL,
            &bRegister));
    ...
    if ((0U != ((bRegister & PHHAL_HW_RC663_BIT_T1RUNNING))) &&
      ...
      PH_CHECK_SUCCESS_FCT(status, phhalHw_Rc663_WaitIrq(pDataParams, PH_ON, PH_OFF, 0x00,
              PHHAL_HW_RC663_BIT_TIMER1IRQ, NULL, &bIrq1Reg));
    }

    /* Stop Timer T0 and T1 used for Poll guard time. */
    bRegister &= (uint8_t)~(uint8_t)(PHHAL_HW_RC663_BIT_T0RUNNING | PHHAL_HW_RC663_BIT_T1RUNNING);
    PH_CHECK_SUCCESS_FCT(status, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_TCONTROL,
            bRegister));
    /* Clear Timer Interrupt */
    PH_CHECK_SUCCESS_FCT(status, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_IRQ1,
            (uint8_t)~(uint8_t)PHHAL_HW_RC663_BIT_SET));
  ...
  /* start the command */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_COMMAND, PHHAL_HW_RC663_CMD_TRANSCEIVE));
  ...
  if ((0U == (bIrq0Reg & PHHAL_HW_RC663_BIT_IDLEIRQ))
    ...
    if (0U != (pDataParams->bRxMultiple)) {
      ...
      bIrq0WaitFor = PHHAL_HW_RC663_BIT_RXIRQ | PHHAL_HW_RC663_BIT_HIALERTIRQ |
  ...
  if (pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXLASTBITS] != 0U) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXLASTBITS,
            0x00U));
  }
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_GetRxBuffer(pDataParams, PH_ON, &pTmpBuffer,
          &wTmpBufferLen, &wTmpBufferSize));
  ...
  if (status == PH_ERR_SUCCESS) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
            PHHAL_HW_RC663_REG_RXBITCTRL, &bRegister));
  ...
  do {
    ...
    if (((pDataParams->bJewelActivated) == PH_ON) && ((status == PH_ERR_SUCCESS) ||
      ...
      if (wTmpBufferLen >= 4U) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phTools_ComputeCrc_B(*ppRxBuffer, (uint32_t)wTmpBufferLen - 2U,
                aCrc));

        if ((aCrc[0] != ((*ppRxBuffer)[wTmpBufferLen - 2U])) ||
            (aCrc[1] != ((*ppRxBuffer)[wTmpBufferLen - 1U]))) {
    ...
    if ((0U != (wVerifyRxCrc5)) && ((*pRxLength - pDataParams->wRxBufStartPos) != 0U)) {
      ...
      bBitlen = (uint8_t)((pDataParams->wAdditionalInfo > 0U) ?
              (((uint16_t)((*pRxLength - pDataParams->wRxBufStartPos) - 1U) << 3U) +
                  pDataParams->wAdditionalInfo) :
      ...
      *pRxLength = (pDataParams->wAdditionalInfo > 0U) ? (((uint16_t)bBitlen >> 3U) + 1U) : ((
                  uint16_t)bBitlen >> 3U);
...
)
{
  ...
  if ((pDataParams->bNfcipMode == PH_ON) &&
    ...
    if ((pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXDATARATE_FRAMING] == PHHAL_HW_RF_DATARATE_106) &&
        (pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO14443A)) {
      /* retrieve transmit buffer */
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_GetTxBuffer(pDataParams, PH_ON, &pTmpBuffer,
              &wTmpBufferLen, &wTmpBufferSize));
  ...
  /* start the command */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_COMMAND, PHHAL_HW_RC663_CMD_TRANSMIT));
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_RXWAIT,
          &bRegRxWait));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXWAIT, 0U));
  ...
  /* start the command */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_COMMAND, PHHAL_HW_RC663_CMD_RECEIVE));
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
          PHHAL_HW_RC663_REG_RXWAIT, bRegRxWait));
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_Int_ProtocolSupport(pDataParams,
          pDataParams->bCardType));
  ...
  switch (pDataParams->bCardType) {
    ...
    case PHHAL_HW_CARDTYPE_I18000P3M3:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetConfig(pDataParams, PHHAL_HW_CONFIG_SYMBOL_START,
  ...
  if (bMode != PHHAL_HW_CARDTYPE_CURRENT) {
    if (pDataParams->bPollGuardTimeFlag == PH_OFF) {
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(
              ...
              PHHAL_HW_RC663_BIT_TAUTORESTARTED | PHHAL_HW_RC663_BIT_TSTOP_RX | PHHAL_HW_RC663_BIT_TSTART_TX |
              PHHAL_HW_RC663_VALUE_TCLK_1356_MHZ));
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
            PHHAL_HW_RC663_REG_T2RELOADHI, 0x03U));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
            PHHAL_HW_RC663_REG_T2RELOADLO, 0xFFU));
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
            PHHAL_HW_RC663_REG_RXBITCTRL, PHHAL_HW_RC663_BIT_VALUESAFTERCOLL));

    /* Init RxSOFD register */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
            PHHAL_HW_RC663_REG_RXSOFD, 0x00U));
  ...
  for (wIndex = 0; wIndex < wShadowCount; ++wIndex) {
    ...
    switch (wConfig) {
      ...
      case PHHAL_HW_CONFIG_TIMEOUT_VALUE_US:
        if (pDataParams->bTimeoutUnit == PHHAL_HW_TIME_MICROSECONDS) {
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetConfig(pDataParams, wConfig,
...
)
{
  ...
  switch (wConfig) {
    ...
    case PHHAL_HW_CONFIG_RXALIGN:
      ...
      if (bValue != bRegister) {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
                PHHAL_HW_RC663_REG_RXBITCTRL, bValue));
      }
    ...
    case PHHAL_HW_CONFIG_TXWAIT_MS:
      ...
      /* Set TxWait */
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_Wait(pDataParams, PHHAL_HW_TIME_MILLISECONDS,
              wValue));
      break;
    ...
    case PHHAL_HW_CONFIG_TXDATARATE_FRAMING:
      ...
      if (wShadowCount > 0U) {
        for (wIndex = 0; wIndex < wShadowCount; ++wIndex) {
          ...
          if ((wConfigShadow != PHHAL_HW_CONFIG_TXDATARATE_FRAMING) &&
            ...
            /* Use internal SetConfig */
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetConfig_Int(pDataParams, wConfigShadow,
                    pShadowDefault[(wIndex << 1U) + 1U]));
          }
        }
      }
      ...
      if ((wShadowCount > 0U) ||
          (wValue != pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXDATARATE_FRAMING]) ||
          (wDataRate != pDataParams->wCfgShadow[PHHAL_HW_CONFIG_RXDATARATE_FRAMING])) {
        /* Evaluate hardware settings */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetCardMode(
                pDataParams,
    ...
    case PHHAL_HW_CONFIG_RXDATARATE_FRAMING:
      ...
      if ((wShadowCount > 0U) ||
          (wValue != pDataParams->wCfgShadow[PHHAL_HW_CONFIG_RXDATARATE_FRAMING]) ||
          (wDataRate != pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXDATARATE_FRAMING])) {
        /* Evaluate hardware settings */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_SetCardMode(
                pDataParams,
    ...
    case PHHAL_HW_CONFIG_CRCTYPE:
      ...
      bValue = 0;
      if (0U == (wValue & (PHHAL_HW_CRCTYPE_CRC5 | PHHAL_HW_CRCTYPE_CRC16 |
    ...
    case PHHAL_HW_CONFIG_DISABLE_MF_CRYPTO1:
      ...
      if (wValue != PH_OFF) {
        ...
        if (bValue != bRegister) {
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
                  PHHAL_HW_RC663_REG_STATUS, bValue));
        }
      }
      break;
    ...
    case PHHAL_HW_CONFIG_RXBUFFER_STARTPOS:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_GetRxBuffer(pDataParams, PH_ON, &pBuffer,
    ...
    case PHHAL_HW_CONFIG_TXBUFFER_LENGTH:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_GetTxBuffer(pDataParams, PH_ON, &pBuffer,
              &wBufferLen, &wBufferSize));
    ...
    case PHHAL_HW_CONFIG_POLL_GUARD_TIME_US:
      ...
      if (pDataParams->bCardType == PHHAL_HW_CARDTYPE_UNKNOWN) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
                PHHAL_HW_RC663_REG_T0CONTROL,
                bRegister | PHHAL_HW_RC663_BIT_TAUTORESTARTED | PHHAL_HW_RC663_BIT_TSTOP_RX |
                PHHAL_HW_RC663_BIT_TSTART_TX | PHHAL_HW_RC663_VALUE_TCLK_1356_MHZ));

        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
                PHHAL_HW_RC663_REG_T1CONTROL, &bRegister));
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
                PHHAL_HW_RC663_REG_T1CONTROL,
                bRegister | PHHAL_HW_RC663_BIT_TSTOP_RX | PHHAL_HW_RC663_BIT_TSTART_TX |
                PHHAL_HW_RC663_VALUE_TCLK_T0));
      }

      /* Configure and start timer. */
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_Wait(pDataParams, PHHAL_HW_TIME_MICROSECONDS,
              wValue));
      break;
...
)
{
  ...
  if ((pDataParams->bCardType == PHHAL_HW_CARDTYPE_UNKNOWN) &&
      (pDataParams->bPollGuardTimeFlag == PH_OFF)) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
            PHHAL_HW_RC663_REG_T0CONTROL, &bT0Ctrl));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
            PHHAL_HW_RC663_REG_T0CONTROL, bT0Ctrl | PHHAL_HW_RC663_BIT_TAUTORESTARTED |
            PHHAL_HW_RC663_VALUE_TCLK_1356_MHZ));

    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
            PHHAL_HW_RC663_REG_T1CONTROL, &bT1Ctrl));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams,
            PHHAL_HW_RC663_REG_T1CONTROL, bT1Ctrl | PHHAL_HW_RC663_VALUE_TCLK_T0));
  }
  ...
  /* retrieve content of Timer Control register */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams,
          PHHAL_HW_RC663_REG_TCONTROL, &bControlReg));

  /* clear all irq flags */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_IRQ1,
          (uint8_t)~(uint8_t)PHHAL_HW_RC663_BIT_SET));
  ...
  if (pDataParams->bPollGuardTimeFlag == PH_OFF) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WaitIrq(pDataParams, PH_ON, PH_OFF, 0x00,
            PHHAL_HW_RC663_BIT_TIMER1IRQ, NULL, &bIrqRq));
...
)
{
  ...
  phStatus_t  PH_MEMLOC_REM status = PH_ERR_SUCCESS;
  /* Post Abort Event. */
  PH_CHECK_SUCCESS_FCT(status, phOsal_EventPost(&pDataParams->HwEventObj.EventHandle,
          E_OS_EVENT_OPT_NONE, E_PH_OSAL_EVT_ABORT, NULL));
#else
  /* satisfy compiler */
  if (0U != (pDataParams));
#endif
...
)
{
  ...
  *pRxBufferLen = 0;
  ...
  bQ = (uint8_t)((uint8_t)((uint8_t)pBeginRndCmd[1] & 0x7U) << 1U) | ((uint8_t)((
                  uint8_t)pBeginRndCmd[2] & 0x80U) >> 7U);
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(status, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_IRQ0EN,
          &bRegister));
  /* Clear IRQ0 interrupt sources */
  bRegister = 0x00;
  PH_CHECK_SUCCESS_FCT(status, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_IRQ0EN,
          bRegister));

  PH_CHECK_SUCCESS_FCT(status, phhalHw_Rc663_ReadRegister(pDataParams, PHHAL_HW_RC663_REG_IRQ1EN,
          &bRegister));
  /* Clear IRQ1 interrupt sources */
  bRegister = 0x00;
  PH_CHECK_SUCCESS_FCT(status, phhalHw_Rc663_WriteRegister(pDataParams, PHHAL_HW_RC663_REG_IRQ1EN,
          bRegister));
...
static void phhalHw_Rc663_Reset(void)
{
  ...
  /* delay of ~2 ms */
  (void)phDriver_TimerStart(PH_DRIVER_TIMER_MILLI_SECS, PHHAL_HW_RC663_RESET_DELAY_MILLI_SECS,
      NULL);
