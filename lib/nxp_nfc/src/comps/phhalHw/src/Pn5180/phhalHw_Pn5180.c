...
#include "nxp_nfc/phhalHw_Pn5180_Reg.h"
...
struct hal_timer g_poll_guard_timer;
static uint32_t g_pn5180_timer_res = 0;

uint32_t poll_guard_timer_arg = 0xdeadc0de;

uint8_t no_hexdump = 1;
 ...
 *
 * @param The device object associated with this chip
 * @param Argument passed to OS device init, unused
 *
 * @return 0 on success, non-zero error on failure.
 */
int
pn5180_init(struct os_dev *dev, void *arg)
{
  int rc;
  ...
  rc = hal_timer_config(MYNEWT_VAL(PN5180_HAL_TIMER), MYNEWT_VAL(PN5180_HAL_TIMER_FREQ));
  assert(rc == 0);

  g_pn5180_timer_res = hal_timer_get_resolution(MYNEWT_VAL(PN5180_HAL_TIMER));
  assert(g_pn5180_timer_res == (1000000000 / MYNEWT_VAL(PN5180_HAL_TIMER_FREQ)));
...
{
  struct pn5180_itf *itf = arg;

  pn5180_init((struct os_dev *)bnode, itf);
}

int
pn5180_create_spi_dev(struct bus_spi_node *node, const char *name,
    const struct bus_spi_node_cfg *spi_cfg,
    struct pn5180_itf *itf)
{
  ...
  itf->pi_dev = &node->bnode.odev;
  bus_node_set_callbacks((struct os_dev *)node, &cbs);

  rc = bus_spi_node_create(name, node, spi_cfg, itf);
...
static void phhalHw_Pn5180_EventCallback(void *pDataParams)
{
  ...
  /* Post Abort Event. */
  (void)phOsal_EventPost(&pPn5180DataParams->HwEventObj.EventHandle, E_OS_EVENT_OPT_POST_ISR,
...
#define PN5180_LOG_EVT_MEMPOOL_SIZE  \
    OS_MEMPOOL_SIZE(MYNEWT_VAL(PN5180_LOG_EVENTS_MAX), \
                    sizeof(struct log_ctxt))
...
static void
print_exchange(uint8_t *txbuf, uint16_t txlen, uint8_t *rxbuf, uint16_t rxlen)
{
#if MYNEWT_VAL(PN5180_RXTX_HEXDUMP)
  if (no_hexdump) {
    return;
  ...
  if (txlen) {
    sched_log_evt(1, txbuf, txlen);
  }

  if (rxlen) {
    sched_log_evt(0, rxbuf, rxlen);
  }
#endif
}
...
)
{
  ...
  struct pn5180 *pn5180 = (struct pn5180 *)os_dev_lookup("pn5180_0");
  assert(pn5180);
  pn5180->data_params = pDataParams;
...
#if MYNEWT_VAL(PN5180_RXTX_HEXDUMP)
  int rc = os_mempool_init(&pn5180_log_evt_pool,
          MYNEWT_VAL(PN5180_LOG_EVENTS_MAX),
          sizeof(struct log_ctxt), pn5180_log_evt_area,
          "pn5180_log_evts");
  assert(rc == OS_OK);
#endif
  ...
  pin_output_logic = RESET_POWERUP_LEVEL;
  statusTmp = hal_gpio_init_out(PHDRIVER_PIN_RESET, pin_output_logic);
  ...
  if (((phbalReg_Type_t *)pBalDataParams)->bBalType == PHBAL_REG_TYPE_SPI) {
    ...
    pin_output_logic = PH_DRIVER_SET_HIGH;
    statusTmp = hal_gpio_init_out(PHDRIVER_PIN_SSEL, pin_output_logic);
    if (statusTmp != PH_ERR_SUCCESS) {
      return PH_ERR_ABORTED;
    }
  }
  ...
  pin_output_logic = PH_DRIVER_SET_LOW;
  statusTmp = hal_gpio_init_out(PHDRIVER_PIN_DWL, pin_output_logic);
  if (statusTmp != PH_ERR_SUCCESS) {
    return PH_ERR_ABORTED;
  }
  ...
  if (((phbalReg_Type_t *)pBalDataParams)->bBalType != PHBAL_REG_TYPE_KERNEL_SPI) {
    ...
    pin_pull_up = PHDRIVER_PIN_BUSY_PULL_CFG;
    statusTmp = hal_gpio_init_in(PHDRIVER_PIN_BUSY, pin_pull_up);
    if (statusTmp != PH_ERR_SUCCESS) {
      return PH_ERR_ABORTED;
    }
  }
  ...
  if (((phbalReg_Type_t *)pBalDataParams)->bBalType == PHBAL_REG_TYPE_SPI) {
    ...
    /* delay of ~2 ms */
    os_cputime_delay_usecs(PHHAL_HW_PN5180_DELAY_TO_CHECK_TESTBUS * 1000);

    if (hal_gpio_read(PHDRIVER_PIN_BUSY) == PH_ON) {
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_ReadE2Prom(pDataParams,
          PHHAL_HW_PN5180_FIRMWARE_VERSION_ADDR, bFirmwareVer, 2U));
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_ReadE2Prom(pDataParams,
          PHHAL_HW_PN5180_TESTBUS_ENABLE_ADDR, &pDataParams->bIsTestBusEnabled, 1U));
  ...
  /* Disable Idle IRQ */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams, IRQ_ENABLE,
          (uint32_t)~IRQ_SET_CLEAR_IDLE_IRQ_CLR_MASK));

  /* Clear all IRQs  */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegister(pDataParams, IRQ_SET_CLEAR,
          PHHAL_HW_PN5180_IRQ_SET_CLEAR_ALL_MASK));

  /* Create the event. */
  pDataParams->HwEventObj.pEvtName = (uint8_t *)bHalEventName;
  pDataParams->HwEventObj.intialValue = 0U;
  PH_CHECK_SUCCESS_FCT(statusTmp, phOsal_EventCreate(&pDataParams->HwEventObj.EventHandle,
          &pDataParams->HwEventObj));

  /*clear previously occurred unwanted events*/
  (void)phOsal_EventClear(&pDataParams->HwEventObj.EventHandle, E_OS_EVENT_OPT_NONE,
      E_PH_OSAL_EVT_RF, NULL);
  ...
  if (pDataParams->wFirmwareVer < 0x304U) {
    ...
    if (pDataParams->wFirmwareVer >= 0x308U) {
      ...
      if (((0U == ((bDigitalDelayCfg & PHHAL_HW_PN5180_DIGITAL_DELAY_ENABLE)))) ||
          (!((bDigitalDelayCfg & PHHAL_HW_PN5180_FDT_TIMER_USED_MASK) == ((PHHAL_HW_PN5180_FDT_TIMER_USED)
                      << 1U)))) {
        /* Clear timer bits. */
        bDigitalDelayCfg &= (uint8_t)~(PHHAL_HW_PN5180_FDT_TIMER_USED_MASK);

        /* Enable FW digital delay and timer 1 for FDT/FWT. */
        bDigitalDelayCfg |= (uint8_t)(PHHAL_HW_PN5180_DIGITAL_DELAY_ENABLE | ((
                        PHHAL_HW_PN5180_FDT_TIMER_USED) << 1U));
    ...
    uint8_t bCardType)
{
  ...
  /* Disable the EMD. */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_SetConfig(pDataParams, PHHAL_HW_CONFIG_SET_EMD,
          PH_OFF));
  ...
  switch (pDataParams->bCardType) {
    ...
    case PHHAL_HW_CARDTYPE_ISO14443A:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_LoadRfConfiguration(pDataParams,
              PHHAL_HW_PN5180_RF_TX_ISO14443A_106_MILLER,
              PHHAL_HW_PN5180_RF_RX_ISO14443A_106_MANCH_SUBC));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_ISO14443B:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_LoadRfConfiguration(pDataParams,
              ...
              PHHAL_HW_PN5180_RF_RX_ISO14443B_106_BPSK));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_FELICA_212:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_LoadRfConfiguration(pDataParams,
              ...
              PHHAL_HW_PN5180_RF_RX_FELICA_212));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_FELICA_424:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_LoadRfConfiguration(pDataParams,
              ...
              PHHAL_HW_PN5180_RF_RX_FELICA_424));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_I18092M_ACTIVE_106:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_LoadRfConfiguration(pDataParams,
              ...
              PHHAL_HW_PN5180_RF_RX_NFC_AI_106));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_I18092M_ACTIVE_212:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_LoadRfConfiguration(pDataParams,
              ...
              PHHAL_HW_PN5180_RF_RX_NFC_AI_212));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_I18092M_ACTIVE_424:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_LoadRfConfiguration(pDataParams,
              ...
              PHHAL_HW_PN5180_RF_RX_NFC_AI_424));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_ISO15693:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_LoadRfConfiguration(pDataParams,
              ...
              PHHAL_HW_PN5180_RF_RX_ISO15693_26_1OF4_SC));  /* Masking RxDataRate with Minimum Configuration Index */
    ...
    case PHHAL_HW_CARDTYPE_I18000P3M3:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_LoadRfConfiguration(pDataParams,
              ...
              PHHAL_HW_PN5180_RF_RX_ISO180003M3_MANCH424_2_PERIOD));  /* Masking RxDataRate with Minimum Configuration Index */
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams, PHHAL_HW_CONFIG_TIMEOUT_VALUE_MS,
              5U));
      break;
  ...
  if (0U != bUseDefaultShadow) {
    ...
    (void)memset(pDataParams->wCfgShadow, 0x00U,
        PHHAL_HW_PN5180_SHADOW_COUNT * sizeof(pDataParams->wCfgShadow[0]));
    ...
    for (wIndex = 0U; wIndex < wShadowCount; ++wIndex) {
      ...
      if ((wConfig == PHHAL_HW_CONFIG_TXWAIT_US) && (pDataParams->bActiveMode) &&
          (pDataParams->wTargetMode == PH_OFF)) {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_SetConfig(pDataParams, wConfig,
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams, PHHAL_HW_CONFIG_DISABLE_MF_CRYPTO1,
          PH_ON));
...
)
{
  ...
  do {
    ...
    /* Check options */
    if (0U != (wOption & (uint16_t)~(uint16_t)(PH_EXCHANGE_BUFFERED_BIT |
    ...
    PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_GetTxBuffer(pDataParams, PH_ON, &pTmpBuffer,
    ...
    if (pDataParams->bJewelActivated == PH_ON) {
      ...
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_SetConfig(pDataParams, PHHAL_HW_CONFIG_PARITY,
              PH_OFF));

      /* Make sure CRC is OFF */
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXCRC,
              PH_OFF));
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_SetConfig(pDataParams, PHHAL_HW_CONFIG_RXCRC,
              PH_OFF));
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXLASTBITS,
              7U));

      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterOrMask(pDataParams,
              TRANSCEIVER_CONFIG, TRANSCEIVER_CONFIG_TX_FRAMESTEP_ENABLE_MASK));
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXWAIT_US,
              29));
    }

    if ((pDataParams->wTxBufStartPos == 0U)) {
      pTmpBuffer[pDataParams->wTxBufStartPos++] =
          PHHAL_HW_PN5180_SET_INSTR_SEND_DATA; /* Reserved 0th Location of buffer for Command code */
      pTmpBuffer[pDataParams->wTxBufStartPos++] =
          0U; /* Reserved 1st Location of buffer for Command code */
    ...
    if (wTxLength != 0U) {
      ...
      /* copy data */
      (void)memcpy(&pTmpBuffer[pDataParams->wTxBufStartPos + pDataParams->wTxBufLen], pTxBuffer,
          wTxLength);
    ...
    if (0U != (wOption & PH_EXCHANGE_BUFFERED_BIT)) {
      print_exchange(pDataParams->pTxBuffer, pDataParams->wTxBufLen,
          pDataParams->pRxBuffer, pDataParams->wRxBufLen);
      return PH_ERR_SUCCESS;
    }
    ...
    PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_Int_LoadCommand(pDataParams,
            PHHAL_HW_PN5180_SYSTEM_TRANSEIVE_CMD));

    /*Set wait IRQ */
    if (pDataParams->bRxMultiple == PH_ON) {
      dwIrqWaitFor =  IRQ_STATUS_TIMER1_IRQ_MASK | IRQ_STATUS_GENERAL_ERROR_IRQ_MASK;
    } else {
      dwIrqWaitFor =  IRQ_STATUS_RX_IRQ_MASK | IRQ_STATUS_TIMER1_IRQ_MASK |
          IRQ_STATUS_GENERAL_ERROR_IRQ_MASK;
    }
    ...
    /* Clear Interrupts  */
    PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegister(pDataParams, IRQ_SET_CLEAR,
            PHHAL_HW_PN5180_IRQ_SET_CLEAR_ALL_MASK));
    ...
    /* Enable IRQ sources */
    PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegister(pDataParams, IRQ_ENABLE,
            dwIrqWaitFor));
    ...
    if (pDataParams->bRxMultiple == PH_ON) {
      ...
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegister(pDataParams, TIMER1_CONFIG,
              dwValue));
    }
    if ((pDataParams->bOpeMode != RD_LIB_MODE_EMVCO) && (pDataParams->bRxMultiple == PH_OFF)) {
      dwValue = (TIMER1_CONFIG_T1_START_ON_TX_ENDED_MASK | TIMER1_CONFIG_T1_STOP_ON_RX_STARTED_MASK |
              TIMER1_CONFIG_T1_ENABLE_MASK);
      if (pDataParams->bTimeoutUnit == PHHAL_HW_TIME_MILLISECONDS) {
        dwValue |= PHHAL_HW_PN5180_MS_TIMEOUT_PRESCALAR;
      }
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegister(pDataParams, TIMER1_CONFIG,
    ...
    *(pTmpBuffer + 1U) = (uint8_t)pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXLASTBITS];
    ...
    if (pDataParams->bActiveMode) {
      PH_CHECK_FAILURE_FCT(statusTmp, phhalHw_Pn5180_Instr_ReadRegister(pDataParams, RF_STATUS,
              &dwValue));
  ...
  if (pDataParams->bPollGuardTimeFlag == PH_ON) {
    ...
    if (statusTmp == PH_ERR_SUCCESS) {
      ...
      (void)phOsal_EventPend(xEventHandle,
      ...
      if (E_PH_OSAL_EVT_ABORT == (dwEventFlags & E_PH_OSAL_EVT_ABORT)) {
        statusTmp = PH_ERR_ABORTED;
        console_printf("ABORTED!");
      }
    }
    rc = hal_timer_stop(&g_poll_guard_timer);
    if (rc) {
      print_exchange(pDataParams->pTxBuffer, pDataParams->wTxBufLen,
          pDataParams->pRxBuffer, pDataParams->wRxBufLen);
      return PH_ERR_ABORTED;
  ...
  print_exchange(pTmpBuffer, pDataParams->wTxBufLen,
      NULL, 0);
  ...
  print_exchange(NULL, 0, pReceivedData, wDataLenTmp);
  ...
  if (pDataParams->wCfgShadow[PHHAL_HW_CONFIG_TXLASTBITS] != 0U) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_SetConfig(pDataParams, PHHAL_HW_CONFIG_TXLASTBITS,
            0x00U));
  }
  ...
  if (pDataParams->bOpeMode != RD_LIB_MODE_EMVCO) {
    statusTmp = phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams, TIMER1_CONFIG,
            (uint32_t)(~TIMER1_CONFIG_T1_ENABLE_MASK));
  }
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterOrMask(pDataParams,
          IRQ_SET_CLEAR, dwRegister));

  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_RfOn(pDataParams,
          bRFONconfig));      /* Field ON using Collision Avoidance option */

  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_ReadRegister(pDataParams, RF_STATUS,
          &dwRegister));
...
)
{
  ...
  if (bUnit == PHHAL_HW_TIME_MILLISECONDS) {
    ...
    while (wTimeout > PHHAL_HW_PN5180_MAX_TIME_DELAY_MS) {
      ...
      dwLoadValue = (uint32_t)(PHHAL_HW_PN5180_MAX_TIME_DELAY_MS * (wFreq  /
  ...
  switch (wConfig) {
    ...
    case PHHAL_HW_CONFIG_RXALIGN:
      if ((pDataParams->wCfgShadow[wConfig] != wValue) ||
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterMultiple(pDataParams,
    ...
    case PHHAL_HW_CONFIG_CLEARBITSAFTERCOLL:
      ...
      if (pDataParams->wCfgShadow[wConfig] != wValue) {
        ...
        if (wValue == PH_OFF) {
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterOrMask(pDataParams,
                  CRC_RX_CONFIG, CRC_RX_CONFIG_VALUES_AFTER_COLLISION_MASK));
        } else {
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams,
                  CRC_RX_CONFIG, (0xFFFFFFFFU ^ CRC_RX_CONFIG_VALUES_AFTER_COLLISION_MASK)));
        }
        pDataParams->wCfgShadow[wConfig] = wValue;
      }
      break;
    ...
    case PHHAL_HW_CONFIG_TXWAIT_MS:
      ...
      if (0U != dwTemp) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_WriteRegister(pDataParams, TX_WAIT_CONFIG,
                dwValue));
      } else {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_WriteRegister(pDataParams, TX_WAIT_CONFIG,
                dwValue));
    ...
    case PHHAL_HW_CONFIG_RXWAIT_US:
      ...
      if ((pDataParams->wCfgShadow[wConfig] != wValue) ||
        ...
        if (0U != dwTemp) {
          ...
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegister(pDataParams, RX_WAIT_CONFIG,
                  dwValue));
        } else {
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegister(pDataParams, RX_WAIT_CONFIG,
                  dwValue));
    ...
    case PHHAL_HW_CONFIG_MODINDEX:
      ...
      if ((pDataParams->wCfgShadow[wConfig] != wValue) ||
        ...
        /*Set the new value  */
        dwTemp = (uint32_t)((wValue << ANA_RF_CONTROL_TX_TX_RESIDUAL_CARRIER_POS) &
                ANA_RF_CONTROL_TX_TX_RESIDUAL_CARRIER_MASK);
        wRegTypeValueSets[wSizeOfRegTypeValueSets++] = ANA_RF_CONTROL_TX;
        wRegTypeValueSets[wSizeOfRegTypeValueSets++] = PHHAL_HW_PN5180_WRITE_MULTIPLE_TYPE_WRITE_OR_MASK;
        wRegTypeValueSets[wSizeOfRegTypeValueSets++] = (uint8_t)(dwTemp);
        wRegTypeValueSets[wSizeOfRegTypeValueSets++] = (uint8_t)(dwTemp >> 8U);
    ...
    case PHHAL_HW_CONFIG_DISABLE_MF_CRYPTO1:
      ...
      if ((wValue != PH_OFF) && (pDataParams->bMfcCryptoEnabled == PH_ON)) {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams,
                SYSTEM_CONFIG, (uint32_t)(~(SYSTEM_CONFIG_MFC_CRYPTO_ON_MASK))));
        pDataParams->bMfcCryptoEnabled = PH_OFF;
      }
      break;
    ...
    case PHHAL_HW_CONFIG_RXBUFFER_STARTPOS:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_GetRxBuffer(pDataParams, PH_ON, &pBuffer,
    ...
    case PHHAL_HW_CONFIG_TXBUFFER_LENGTH:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_GetTxBuffer(pDataParams, PH_ON, &pBuffer,
              &wBufferLen, &wBufferSize));
    ...
    case PHHAL_HW_CONFIG_TXDATARATE_FRAMING:
      ...
      if ((pDataParams->wCfgShadow[wConfig] != wValue) ||
        ...
        if ((pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO15693) &&
            (wTmpValue == PHHAL_HW_RF_TX_DATARATE_1_OUT_OF_4)) {
          PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_WriteRegister(pDataParams,
                  TX_SYMBOL23_MOD_RFU, (uint32_t)PHHAL_HW_15693_TX26_SYMBOL23));
          PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_WriteRegister(pDataParams, TX_DATA_MOD_RFU,
                  (uint32_t)PHHAL_HW_15693_TX26_DATA_MOD));
        } else if ((pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO15693) &&
            (wTmpValue == PHHAL_HW_RF_I15693_53KBPS_DATARATE)) {
          PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_WriteRegister(pDataParams,
                  TX_SYMBOL23_MOD_RFU, (uint32_t)PHHAL_HW_15693_TX53_SYMBOL23));
          PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_WriteRegister(pDataParams, TX_DATA_MOD_RFU,
                  (uint32_t)PHHAL_HW_15693_TX53_DATA_MOD));
          pDataParams->wCfgShadow[wConfig] = wTmpValue;
        } else if ((pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO15693) &&
            (wTmpValue == PHHAL_HW_RF_DATARATE_106)) {
          PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_WriteRegister(pDataParams,
                  TX_SYMBOL23_MOD_RFU, (uint32_t)PHHAL_HW_15693_TX106_SYMBOL23));
          PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_WriteRegister(pDataParams, TX_DATA_MOD_RFU,
                  (uint32_t)PHHAL_HW_15693_TX106_DATA_MOD));
          pDataParams->wCfgShadow[wConfig] = wTmpValue;
        } else if ((pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO15693) &&
            (wTmpValue == PHHAL_HW_RF_DATARATE_212)) {
          PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_WriteRegister(pDataParams,
                  TX_SYMBOL23_MOD_RFU, (uint32_t)PHHAL_HW_15693_TX212_SYMBOL23));
          PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_WriteRegister(pDataParams, TX_DATA_MOD_RFU,
                  (uint32_t)PHHAL_HW_15693_TX212_DATA_MOD));
          pDataParams->wCfgShadow[wConfig] = wTmpValue;
        } else {
    ...
    case PHHAL_HW_CONFIG_RXDATARATE_FRAMING:
      ...
      if ((pDataParams->wCfgShadow[wConfig] != wValue) ||
        ...
        if ((pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO15693) &&
            (wTmpValue == PHHAL_HW_RF_DATARATE_106)) {
          PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_ReadRegister(pDataParams, SIGPRO_CONFIG,
                  &dwTemp));
          dwTemp = dwTemp & PHHAL_HW_15693_RX106_BAUDRATE_MASK;
          dwTemp = dwTemp | PHHAL_HW_15693_RX106_BAUDRATE_VALUE;
          PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_WriteRegister(pDataParams, SIGPRO_CONFIG,
                  (uint32_t)dwTemp));

          PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_ReadRegister(pDataParams, SIGPRO_RM_CONFIG,
                  &dwTemp));
          dwTemp = dwTemp & PHHAL_HW_15693_RX106_CORRSPEED;
          PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_WriteRegister(pDataParams, SIGPRO_RM_CONFIG,
                  (uint32_t)dwTemp));
    ...
    case PHHAL_HW_CONFIG_SUBCARRIER:
      ...
      if ((pDataParams->wCfgShadow[wConfig] != wValue) ||
        ...
        if ((pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO15693) &&
            (wValue != PHHAL_HW_SUBCARRIER_SINGLE)) {
    ...
    case PHHAL_HW_CONFIG_SET_EMD:
      ...
      if (wValue == 0x1U) {
        if (pDataParams->bOpeMode != RD_LIB_MODE_ISO) {
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_WriteRegister(pDataParams, EMD_CONTROL,
                  PHHAL_HW_PN5180_EMVCO_NFC_EMD));
        } else {
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_WriteRegister(pDataParams, EMD_CONTROL,
                  PHHAL_HW_PN5180_ISO_EMD));
        }
      } else {
        /* Clear EMD Enable bit in EMD Control Register */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams,
                EMD_CONTROL, (uint32_t)~EMD_CONTROL_EMD_ENABLE_MASK));
      }
      break;
    ...
    case PHHAL_HW_CONFIG_RXMULTIPLE:
      ...
      if (wValue == PH_ON) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterOrMask(pDataParams,
                TRANSCEIVER_CONFIG, TRANSCEIVER_CONFIG_RX_MULTIPLE_ENABLE_MASK));
      } else {
        pDataParams->bRxMultiple = PH_OFF;
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams,
                TRANSCEIVER_CONFIG, (uint32_t)~(TRANSCEIVER_CONFIG_RX_MULTIPLE_ENABLE_MASK)));
      }
      break;
    ...
    case PHHAL_HW_CONFIG_RFON_INTERRUPT:
      ...
      if (wValue == PH_ON) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegister(pDataParams, IRQ_ENABLE,
                IRQ_ENABLE_RFON_DET_IRQ_SET_ENABLE_MASK));
      } else {
        /* Disable RF-ON Interrupt*/
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams, IRQ_ENABLE,
                ~(uint32_t)IRQ_ENABLE_RFON_DET_IRQ_SET_ENABLE_MASK));
      }
      break;
    ...
    case PHHAL_HW_CONFIG_SET_SYMBOL_SEND:
      ...
      if (wValue == PH_ON) {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams, TX_CONFIG,
                ~(uint32_t)TX_CONFIG_TX_DATA_ENABLE_MASK));
      }
      /* PH_OFF implies Setting Bit-3 (DataEn Bit in TxDataNum). */
      else {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterOrMask(pDataParams, TX_CONFIG,
                TX_CONFIG_TX_DATA_ENABLE_MASK));
      }
      break;
    ...
    case PHHAL_HW_CONFIG_CRCTYPE:
      ...
      */
      ...
      if ((0U != ((wValue & PHHAL_HW_CRCTYPE_CRC5))) &&
          (pDataParams->bCardType == PHHAL_HW_CARDTYPE_I18000P3M3)) {
    ...
    case PHHAL_HW_CONFIG_MFHALTED:
      ...
      if (wValue == PH_ON) {
        PH_CHECK_SUCCESS_FCT(statusTmp,
            phhalHw_Pn5180_Instr_WriteRegisterOrMask(pDataParams, SYSTEM_CONFIG,
                SYSTEM_CONFIG_AUTOCOLL_STATE_A_MASK));
        pDataParams->bCardMode  = PH_ON;
      } else {
        PH_CHECK_SUCCESS_FCT(statusTmp,
            phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams, SYSTEM_CONFIG,
                (uint32_t)~SYSTEM_CONFIG_AUTOCOLL_STATE_A_MASK));
        pDataParams->bCardMode  = PH_OFF;
    ...
    case PHHAL_HW_CONFIG_POLL_GUARD_TIME_US:
      ...
      rc = hal_timer_set_cb(MYNEWT_VAL(PN5180_HAL_TIMER), &g_poll_guard_timer,
              phhalHw_Pn5180_hal_timer_cb,
              &poll_guard_timer_arg);
      assert(rc == 0);
      rc = hal_timer_start(&g_poll_guard_timer, (wValue * 1000) / g_pn5180_timer_res);
      assert(rc == 0);
      break;
    ...
    case PHHAL_HW_PN5180_CONFIG_SET_FELICA_EMD:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp,
          phhalHw_Pn5180_SetConfig_FelicaEmdRegBit(pDataParams, wValue,
              (uint32_t)FELICA_EMD_CONTROL_FELICA_EMD_ENABLE_MASK));
      break;
    ...
    case PHHAL_HW_PN5180_CONFIG_SET_FELICA_EMD_LEN_CHECK:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp,
          phhalHw_Pn5180_SetConfig_FelicaEmdRegBit(pDataParams, wValue,
              (uint32_t)FELICA_EMD_CONTROL_FELICA_EMD_LEN_CHECK_ENABLE_MASK));
      break;
    ...
    case PHHAL_HW_PN5180_CONFIG_SET_FELICA_EMD_RC_CHECK:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp,
          phhalHw_Pn5180_SetConfig_FelicaEmdRegBit(pDataParams, wValue,
              (uint32_t)FELICA_EMD_CONTROL_FELICA_EMD_RC_CHECK_ENABLE_MASK));
      break;
    ...
    case PHHAL_HW_PN5180_CONFIG_SET_FELICA_EMD_PROTOCOL_ERR_CHECK:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp,
          phhalHw_Pn5180_SetConfig_FelicaEmdRegBit(pDataParams, wValue,
              (uint32_t)FELICA_EMD_CONTROL_FELICA_EMD_PROTOCOL_ERR_CHECK_ENABLE_MASK));
      break;
    ...
    case PHHAL_HW_PN5180_CONFIG_SET_FELICA_EMD_INTEGRITY_ERR_CHECK:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp,
          phhalHw_Pn5180_SetConfig_FelicaEmdRegBit(pDataParams, wValue,
              (uint32_t)FELICA_EMD_CONTROL_FELICA_EMD_INTEGRITY_ERR_CHECK_ENABLE_MASK));
      break;
...
)
{
  ...
  /* Check options */
  if (wOption & (uint16_t)~(uint16_t)(PH_TRANSMIT_BUFFERED_BIT | PH_TRANSMIT_LEAVE_BUFFER_BIT |
          PH_TRANSMIT_PREV_FRAME)) {
    return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
  }

  /* Re-send option should not be combined with buffer options. */
  if ((0U != ((wOption & PH_TRANSMIT_PREV_FRAME))) &&
      (wOption & (uint16_t)~(uint16_t)(PH_TRANSMIT_PREV_FRAME))) {
    return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
  }
  ...
  *  since the data to transmit would get overwritten by received data. */
  ...
  if ((pDataParams->wTargetMode != PH_OFF)) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_ReadRegister(pDataParams, RF_STATUS,
            &dwRegValue));
    dwTrasceiveState = (dwRegValue & RF_STATUS_TRANSCEIVE_STATE_MASK) >>
        RF_STATUS_TRANSCEIVE_STATE_POS;
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_GetTxBuffer(pDataParams, PH_ON, &pTmpBuffer,
          &wTmpBufferLen, &wTmpBufferSize));
  ...
  if (wTxLength != 0U) {
    if (wTxLength > (wTmpBufferSize - wTmpBufferLen -
            bOffset /* Offset Reserved for Transmit Data Command and Detect sync if TYPE A if present */)) {
      pDataParams->wTxBufLen = 0U;
      pDataParams->wTxBufStartPos = 0U;
      return PH_ADD_COMPCODE_FIXED(PH_ERR_BUFFER_OVERFLOW, PH_COMP_HAL);
    }
  ...
  if ((pDataParams->wTargetMode == PH_OFF)) {
    ...
    dwTrasceiveState = (dwRegValue & RF_STATUS_TRANSCEIVE_STATE_MASK) >>
        RF_STATUS_TRANSCEIVE_STATE_POS;
    if ((dwTrasceiveState > PHHAL_HW_PN5180_TRANCEIVE_STATE_WAITTRANSMIT) ||
        (dwTrasceiveState == PHHAL_HW_PN5180_TRANCEIVE_STATE_IDLE)) {
      /*load idle command*/
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Int_IdleCommand(pDataParams));
      /*Execute the Tranceive Command*/
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Int_LoadCommand(pDataParams,
              PHHAL_HW_PN5180_SYSTEM_TRANSEIVE_CMD));

    }
  }

  dwIrqWaitFor =  IRQ_STATUS_TX_IRQ_MASK | IRQ_STATUS_GENERAL_ERROR_IRQ_MASK |
      IRQ_STATUS_TIMER1_IRQ_MASK ;
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_WriteRegister(pDataParams, IRQ_SET_CLEAR,
          PHHAL_HW_PN5180_IRQ_SET_CLEAR_ALL_MASK));
  ...
  if (status != PH_ERR_SUCCESS) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Int_IdleCommand(pDataParams));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams,
            TIMER1_CONFIG, (uint32_t)(~TIMER1_CONFIG_T1_ENABLE_MASK)));
  }
...
)
{
  ...
  dwIrqWaitFor =  IRQ_STATUS_GENERAL_ERROR_IRQ_MASK | IRQ_STATUS_RX_IRQ_MASK |
      IRQ_STATUS_TIMER1_IRQ_MASK;
  ...
  do {
    ...
    if (pDataParams->bTransmited == PH_OFF) {
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_ReadRegister(pDataParams, RF_STATUS,
              &dwRegister));
      dwTranceiveState = ((dwRegister & RF_STATUS_TRANSCEIVE_STATE_MASK) >>
              RF_STATUS_TRANSCEIVE_STATE_POS);
      ...
      if ((dwTranceiveState == PHHAL_HW_PN5180_TRANCEIVE_STATE_IDLE) \
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams,
                TRANSCEIVER_CONFIG, (uint32_t)(~TRANSCEIVER_CONFIG_INITIATOR_MASK)));
      ...
      if ((pDataParams->bActiveMode == PH_ON) && (pDataParams->wTargetMode != PH_OFF)) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams,
                SYSTEM_CONFIG, (uint32_t)~SYSTEM_CONFIG_ACTIVE_MODE_TX_RF_ENABLE_MASK));

        status = phOsal_EventPend((volatile phOsal_Event_t *)(&pDataParams->HwEventObj.EventHandle),
                E_OS_EVENT_OPT_PEND_SET_ANY, 400U, E_PH_OSAL_EVT_RF, NULL);

        if ((status & PH_ERR_MASK) == PH_ERR_IO_TIMEOUT) {
          status = PH_ADD_COMPCODE_FIXED(PH_ERR_EXT_RF_ERROR, PH_COMP_HAL);
        } else {
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterOrMask(pDataParams,
                  SYSTEM_CONFIG, SYSTEM_CONFIG_ACTIVE_MODE_TX_RF_ENABLE_MASK));
        }
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_ReadRegister(pDataParams, IRQ_STATUS,
              &dwRegister));
    ...
    if ((PH_ERR_SUCCESS == status) && (pDataParams->bActiveMode == PH_OFF) &&
        (pDataParams->wTargetMode != PH_OFF)) {
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_ReadRegister(pDataParams, RF_STATUS,
              &dwRegister));
...
{
  phStatus_t  PH_MEMLOC_REM statusTmp;
  struct os_dev *dev = os_dev_lookup("pn5180_0");

  assert(dev);
  if (((phbalReg_Type_t *)(pDataParams->pBalDataParams))->bBalType == PHBAL_REG_TYPE_KERNEL_SPI) {
    int rc = bus_node_simple_write_read_transact(dev, pTxBuffer, wTxLength, pRxBuffer,
            *pRxLength);
    if (rc) {
    ...
    while (hal_gpio_read(PHDRIVER_PIN_BUSY));
#endif
    ...
    int rc = bus_node_simple_write(dev, pTxBuffer, wTxLength);
    if (rc) {
    ...
    if (0U != wRxBufSize) {
      ...
      int rc = bus_node_simple_read(dev, pRxBuffer, wRxBufSize);
      if (rc) {
        statusTmp = PH_ERR_INTERNAL_ERROR;
        return statusTmp;
      }
...
)
{
  ...
  *pRxLength = 0U;
  ...
  /*Go to autocoll mode*/
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_SwitchModeAutocoll(pDataParams,
          bRfTechnology, bAutocollMode));
  ...
  if (0U != (dwIrqReg & IRQ_STATUS_GENERAL_ERROR_IRQ_MASK)) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_ReadRegisterMultiple(pDataParams, pRegisters,
            4U, &pTmpBuffer));
    ...
    bFraming = (uint8_t)((dwRegValue & SIGPRO_CM_CONFIG_RX_FRAMING_MASK) >>
            SIGPRO_CM_CONFIG_RX_FRAMING_POS);
    ...
    if (0U != (dwIrqReg & IRQ_STATUS_CARD_ACTIVATED_IRQ_MASK)) {
      ...
      if ((bSpeed == PHHAL_HW_RF_DATARATE_106) &&
          (**ppRxBuffer == PHHAL_HW_PN5180_I18092MT_START_BYTE)) {
    ...
    if ((pDataParams->bActiveMode == PH_ON) && ((wMode & ACTIVE_A_MODE) != ACTIVE_A_MODE) &&
        (bSpeed == PHHAL_HW_RF_DATARATE_106)) {
      /* Terminate Autocoll command before existing. */
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_SwitchModeNormal(pDataParams));
      return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_HAL);
    }
    ...
    phhalHw_Pn5180_DataParams_t *pDataParams)
{
  ...
  switch (pDataParams->bLpcdMode) {
    case PHHAL_HW_PN5180_LPCD_MODE_DEFAULT:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_ReadRegister(pDataParams, AGC_REF_CONFIG,
              &dwRegister));

      PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_ReadE2Prom(pDataParams,
              PHHAL_HW_PN5180_LPCD_REFERENCE_VALUE_ADDR, bEPromData,  5U));

      PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_ReadE2Prom(pDataParams,
              PHHAL_HW_PN5180_OCPROT_CONTROL_ADDR, bOcProtControl,  1U));
      ...
      pDataParams->wLpcdReference = (uint16_t)(dwRegister & (AGC_REF_CONFIG_AGC_GEAR_MASK |
                  AGC_REF_CONFIG_AGC_VALUE_MASK));

      /*If TXOCP is enabled, Then we need to compare gear*/
      if ((bOcProtControl[0] & 0x01U) &&
          (dwRegister & AGC_REF_CONFIG_AGC_GEAR_MASK) != (dwAgcRefVal & AGC_REF_CONFIG_AGC_GEAR_MASK)) {
        status = PH_ERR_SUCCESS;
      } else {
...
)
{
  ...
  return phhalHw_Pn5180_Instr_WriteE2Prom(pDataParams, PHHAL_HW_PN5180_SET_LISTEN_EEPROM_ADDR, aCmd,
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(status,
      phOsal_EventPost(&pDataParams->HwEventObj.EventHandle, E_OS_EVENT_OPT_NONE, E_PH_OSAL_EVT_ABORT,
          NULL));
...
)
{
  ...
  /* Clear the Rf Event. */
  PH_CHECK_SUCCESS_FCT(status, phOsal_EventClear(&pDataParams->HwEventObj.EventHandle,
          E_OS_EVENT_OPT_NONE, E_PH_OSAL_EVT_RF, NULL));

  /* Enable IRQ sources */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterOrMask(pDataParams, IRQ_ENABLE,
          dwIrqWaitFor));
  ...
  if (0U != (dwIrqReg & IRQ_STATUS_RX_IRQ_MASK)) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_EpcGen2RetrieveResultSize(pDataParams,
            &ResultSize));
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_EpcGen2RetrieveResult(pDataParams,
            ppRxBuffer, ResultSize));
    ...
    uint32_t dwEventTimeout)
{
  return phOsal_EventPend((volatile phOsal_Event_t *)(&pDataParams->HwEventObj.EventHandle),
          E_OS_EVENT_OPT_PEND_SET_ANY, dwEventTimeout,
...
static void phhalHw_Pn5180_Reset(void)
{
  ...
  hal_gpio_write(PHDRIVER_PIN_RESET, RESET_POWERUP_LEVEL);

  /* delay of ~2 ms */
  os_cputime_delay_usecs(PHHAL_HW_PN5180_RESET_DELAY_MILLI_SECS * 1000);

  hal_gpio_write(PHDRIVER_PIN_RESET, RESET_POWERDOWN_LEVEL);
