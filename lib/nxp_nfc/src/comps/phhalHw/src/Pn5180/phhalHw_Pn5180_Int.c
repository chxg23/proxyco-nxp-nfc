...
)
{
  ...
  /* Normal command -> do not overwrite Tx- and Rx-Contents */
    ...
    /* Redundant code */
    return phhalHw_Pn5180_GetRxBuffer(pDataParams, bIsExchange, pTxBuffer, pTxBufferLen,
            pTxBufferSize);
  }
  ...
  switch (wConfig) {
    ...
    case PHHAL_HW_CONFIG_ASK100:
      ...
      /* switch on 100% ASK */
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams,
                ANA_RF_CONTROL_TX, ~(uint32_t)ANA_RF_CONTROL_TX_TX_RESIDUAL_CARRIER_MASK));
      }
...
)
{
  ...
  if (wValue != PH_OFF) {
    ...
    if (pDataParams->dwFelicaEmdReg & dwMaskValue) {
      /* Update FELICA_EMD_CONTROL Register */
      PH_CHECK_SUCCESS_FCT(statusTmp,
          phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams, FELICA_EMD_CONTROL,
              (uint32_t)~dwMaskValue));
      /* Update shadow register */
    ...
    uint16_t wSubcarrier)
{
  ...
  if ((PHHAL_HW_PN5180_CURRENT_RF_CONFIGURATION_INDEX != bTxConfig) ||
      (PHHAL_HW_PN5180_CURRENT_RF_CONFIGURATION_INDEX != bRxConfig)) {
    PH_CHECK_SUCCESS_FCT(statusTmp,
...
)
{
  ...
  if (bUnit == PHHAL_HW_TIME_MICROSECONDS) {
    ...
    if (wTimeout <= PHHAL_HW_PN5180_MAX_TIME_DELAY_MS) {
      dwLoadValue = (uint32_t)(wTimeout * (PHHAL_HW_PN5180_MIN_FREQ  /
    ...
    uint32_t wPrescaler)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegister(pDataParams, IRQ_SET_CLEAR,
          IRQ_SET_CLEAR_TIMER0_IRQ_CLR_MASK));

  phOsal_EventClear(&pDataParams->HwEventObj.EventHandle, E_OS_EVENT_OPT_NONE, E_PH_OSAL_EVT_RF,
      NULL);

  /* Enable IRQ source */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegister(pDataParams, IRQ_ENABLE,
          IRQ_ENABLE_TIMER0_IRQ_SET_ENABLE_MASK));
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_WaitIrq(pDataParams, PH_ON, PH_OFF,
          IRQ_STATUS_TIMER0_IRQ_MASK, &dwIrqStatusReg));
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegisterAndMask(pDataParams,
          TIMER0_CONFIG, (uint32_t)(~TIMER0_CONFIG_T0_ENABLE_MASK)));
...
)
{
  ...
  /* Get T1 reload value */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_ReadRegister(pDataParams, TIMER1_RELOAD,
          &bRegister));
  ...
  if ((wExchangeStatus & PH_ERR_MASK) != PH_ERR_IO_TIMEOUT) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_ReadRegister(pDataParams, TIMER1_STATUS,
            &bRegister));
...
)
{
  ...
  } else if ((pDataParams->bRxMultiple == PH_ON) &&
      (0U != ((dwIrqReg & IRQ_STATUS_TIMER1_IRQ_MASK)))) {
  ...
  else if ((pDataParams->bActiveMode == PH_ON) &&
      (0U != ((dwIrqReg & IRQ_STATUS_RF_ACTIVE_ERROR_IRQ_MASK)))) {
    bRfActiveErrCause = (uint8_t)((dwRegister & RF_STATUS_RF_ACTIVE_ERROR_CAUSE_MASK) >>
            RF_STATUS_RF_ACTIVE_ERROR_CAUSE_POS);
  ...
  if ((0U != ((dwIrqReg & IRQ_STATUS_RX_IRQ_MASK))) && (status == PH_ERR_SUCCESS)) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_GetRxBuffer(pDataParams, PH_ON, &pTmpBuffer,
...
#ifndef _WIN32
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_Instr_WriteRegister(pDataParams, IRQ_SET_CLEAR,
          IRQ_SET_CLEAR_IDLE_IRQ_CLR_MASK));
#endif
...
phhalHw_Pn5180_AutoSyncByte(phhalHw_Pn5180_DataParams_t *pDataParams)
{
  ...
  if ((pDataParams->wTargetMode == PH_OFF) && (dwValue == 4U)) {
    ...
    if (pDataParams->bActiveMode == PH_ON) {
      ...
      dwTemp = (0x21FU << SIGPRO_RM_CONFIG_RFU_SYNC_VAL_POS) | (0x09U <<
              SIGPRO_RM_CONFIG_RFU_SYNC_LEN_POS);
      wRegTypeValueSets[wSizeOfRegTypeValueSets++] = SIGPRO_RM_CONFIG_RFU;
      wRegTypeValueSets[wSizeOfRegTypeValueSets++] = PHHAL_HW_PN5180_WRITE_MULTIPLE_TYPE_WRITE_OR_MASK;
      wRegTypeValueSets[wSizeOfRegTypeValueSets++] = (uint8_t)(dwTemp);
      wRegTypeValueSets[wSizeOfRegTypeValueSets++] = (uint8_t)(dwTemp >> 8U);
