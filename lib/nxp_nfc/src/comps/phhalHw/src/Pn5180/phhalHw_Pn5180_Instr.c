...
)
{
  ...
  * wExpectedResponseLength is not > 260 bytes*/
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_GetRxBuffer(pDataParams, PH_ON,
          &pReceivedDataBuffer, &wTmpBufferLen, &wTmpBufferSize));
...
)
{
  ...
  * wSizeOfRegTypeValueSets for modulus of 6. One set of write register is of 6 byte*/
  ...
  for (bReg_offset = 0U; bReg_offset < wSizeOfRegTypeValueSets;
      bReg_offset += PHHAL_HW_PN5180_MIN_REGISTER_TYPE_VALUE_SET) {
    /* Check for read-only registers */
    PH_CHECK_SUCCESS_FCT(statusTmp,
        phhalHw_Pn5180_Check_Reg_Readonly(*(pRegTypeValueSets + bReg_offset)));
...
)
{
  ...
  */
  if ((NULL == pDataParams) \
      ...
      || ((bWakeupControlMask & (PHHAL_HW_PN5180_WAKEUP_CONTROL_MASK_WUP_ON_COUNTER |
                  PHHAL_HW_PN5180_WAKEUP_CONTROL_MASK_WUP_ON_EXT_FIELD)) == 0U) \
...
)
{
  ...
  */
  ...
  if (0U != (aRspBuffer[0] & 0x01U)) {
    ...
    /* Check auth success */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn5180_ReadRegister(pDataParams, SYSTEM_CONFIG,
            &dwRegister));
...
)
{
  ...
  */
  ...
  /* Copy the Instruction payload and update the buffer length*/
  (void)memcpy(&pTmpBuffer[wBufferLength], pBeginRoundCommand,
...
)
{
  ...
  switch (wConfig) {
    ...
    case PHHAL_HW_CONFIG_LPCD_MODE:
      if ((wValue == PHHAL_HW_PN5180_LPCD_MODE_DEFAULT) ||
    ...
    case PHHAL_HW_CONFIG_LPCD_REF:
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_ReadE2Prom(pDataParams,
              PHHAL_HW_PN5180_LPCD_GPO_REFVAL_CONTROL_ADDR, bEPromData, 1U));

      if ((bEPromData[0] &  LPCD_REFVAL_CONTROL_MASK) == 0U) {
        bEPromData[0] = (uint8_t)wValue;
        bEPromData[1] = (uint8_t)(wValue >> 8U);

        PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_WriteE2Prom(pDataParams,
                PHHAL_HW_PN5180_LPCD_REFERENCE_VALUE_ADDR, bEPromData, 2U));
      } else {
        PH_CHECK_SUCCESS_FCT(statusTmp,  phhalHw_Pn5180_Instr_WriteRegister(pDataParams, AGC_REF_CONFIG,
                (uint32_t)wValue));
      }
      break;
