...
{
  phStatus_t PH_MEMLOC_REM statusTmp = PH_OSAL_SUCCESS;
  uint32_t   PH_MEMLOC_REM dwRegister;
  phOsal_EventBits_t PH_MEMLOC_REM tReceivedEvents = 0;
  ...
  if ((bEnableIrq & PHHAL_HW_CHECK_IRQ_PIN_MASK) != PH_OFF) {
    ...
    {
      statusTmp = phOsal_EventPend((volatile phOsal_Event_t *)(&pDataParams->HwEventObj.EventHandle),
              E_OS_EVENT_OPT_PEND_SET_ANY, PHOSAL_MAX_DELAY,
