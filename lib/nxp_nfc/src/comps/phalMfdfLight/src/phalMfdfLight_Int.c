    ...
    uint16_t *pDataLen)
{
  ...
  switch (bIns) {
    case PHAL_MFDFLIGHT_CMD_ISO7816_SELECT_FILE:
      ...
      statusTmp = phalMfdfLight_Int_ISOSelectFile(pDataParams, pPalMifareDataParams, bOption, bCmdBuff,
              wCmdLen,
              (uint8_t)bLc, bExtendedLenApdu, pDataIn, (uint8_t)bLe, ppDataOut, pDataLen);
      break;

    case PHAL_MFDFLIGHT_CMD_ISO7816_UPDATE_BINARY:
      statusTmp = phalMfdfLight_Int_ISOUpdateBinary(pDataParams, pPalMifareDataParams, bOption,
              bCmdBuff, wCmdLen,
    ...
    uint16_t *pDataLen)
{
  ...
  */
  ...
  if ((statusTmp & PH_ERR_MASK) == PHAL_MFDFLIGHT_ERR_DF_7816_GEN_ERROR) {
    PH_CHECK_SUCCESS_FCT(status, phalMfdfLight_GetConfig(pDataParams, PHAL_MFDFLIGHT_ADDITIONAL_INFO,
            &wVal));
  }
