  ...
  */
  ...
  if (bAuthOption == PHAL_MFDFLIGHT_AUTHFIRST_NON_LRP) {
    ...
    (void)memcpy(pDataParams->bTi, &bCmdBuff[0],
        PHAL_MFDFLIGHT_SIZE_TI); /* Store the Transaction Identifier */
    ...
    if ((bCommOption & PHAL_MFDFLIGHT_MAC_DATA_INCOMPLETE) != PHAL_MFDFLIGHT_MAC_DATA_INCOMPLETE) {
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phCryptoSym_Encrypt(
              ...
              PH_CRYPTOSYM_CIPHER_MODE_CBC | (((wDataLen1 == 0) &&
                      (bCmdBuff[0] != PHAL_MFDFLIGHT_RESP_ADDITIONAL_FRAME)) ? PH_EXCHANGE_DEFAULT :
  ...
  */
  ...
  if (wTmp <= (wFrameLen - wCmdLen)) {
    wApduLen = ((wCmdLen == 0x01) &&
            (bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE)) ? 0 : PHAL_MFDFLIGHT_WRAP_HDR_LEN;
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phpalMifare_ExchangeL4(
            ...
            (((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE) &&
                    (bCommOption & PHAL_MFDFLIGHT_MAC_DATA_INCOMPLETE)) ? PH_EXCHANGE_TXCHAINING : 0),
            pApdu,
            wApduLen,
            &pRecv,
            &wRxlen));
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phpalMifare_ExchangeL4(
            ...
            (uint16_t)((pDataParams->dwPayLoadLen > 0xFE) &&
                (bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE)) ? 0x02U : 0x01U,
            &pRecv,
            &wRxlen));
  ...
  if (pDataParams->bAuthMode == PHAL_MFDFLIGHT_AUTHENTICATEEV2) {
    ...
    */
    ...
    memcpy(&pDataParams->pUnprocByteBuff[pDataParams->bNoUnprocBytes], pDataParams->bTi,
        PHAL_MFDFLIGHT_SIZE_TI); /* PRQA S 3200 */
    pDataParams->bNoUnprocBytes += PHAL_MFDFLIGHT_SIZE_TI;
    ...
    uint8_t *pData, uint16_t wDataLen)
{
  ...
  if (wTotalLen == 0x0000) {
    ...
    if (wTmp <= (wFrameLen - wCmdLen)) {
      ...
      if (pDataParams->bWrappedMode) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phpalMifare_ExchangeL4(
                pDataParams->pPalMifareDataParams,
                PH_EXCHANGE_BUFFER_LAST | (((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE) &&
                        (bCommOption & PHAL_MFDFLIGHT_MAC_DATA_INCOMPLETE)) ? PH_EXCHANGE_TXCHAINING : 0),
                &pApdu[2],
                (uint16_t)((pDataParams->dwPayLoadLen > 0xFE) &&
                    (bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE)) ? 0x02U : 0x01U,
  ...
  if ((status == PH_ERR_SUCCESS) &&
      ((bCommOption & PHAL_MFDFLIGHT_AUTHENTICATE_RESET) == PHAL_MFDFLIGHT_AUTHENTICATE_RESET)) {
    /* Reset authentication status */
    if (pDataParams->bAuthMode == PHAL_MFDFLIGHT_AUTHENTICATEEV2) {
      phalMfdfLight_Sw_Int_ResetAuthStatus(pDataParams);
    }
...
void phalMfdfLight_Sw_Int_ResetAuthStatus(phalMfdfLight_Sw_DataParams_t *pDataParams)
{
  ...
  (void)phTMIUtils_ActivateTMICollection((phTMIUtils_t *)pDataParams->pTMIDataParams,
    ...
    uint16_t *pRespLen)
{
  ...
  do {
    ...
    wLeFieldLen = ((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE) &&
    ...
    if (pDataParams->bWrappedMode && (bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE) &&
    ...
    if (pDataParams->bWrappedMode) {
      ...
      if (bExtended7816 && (bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE)) {
        ...
        pApdu[4] = (uint8_t)(wTemLen + ((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE) ?
                    pDataParams->dwPayLoadLen  : (wCopyDataLen + wCopyLastChunkLen)));
      }

      wApduHeaderLen = ((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE) &&
              (wTemLen == 0)) ? 0 : wApduHeaderLen;

      PH_CHECK_SUCCESS_FCT(statusTmp, phpalMifare_ExchangeL4(
              pDataParams->pPalMifareDataParams,
              PH_EXCHANGE_BUFFER_FIRST | ((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE &&
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp,  phpalMifare_ExchangeL4(
              pDataParams->pPalMifareDataParams,
              PH_EXCHANGE_BUFFER_CONT | ((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE &&
                      bCommOptionTmp) ? PH_EXCHANGE_TXCHAINING : 0),
              &pCmd[wIndexCmdLen],
              wTemLen,
              &pRecv,
    ...
    if (bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE && bCommOptionTmp) {
      bStatusByte = (uint8_t)((pRecv[0] & 0xF0) == 0xA0) ? PHAL_MFDFLIGHT_RESP_ADDITIONAL_FRAME :
  ...
  do {
    if (pDataParams->bWrappedMode) {
      ...
      if (bIsExtended7816 && (bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE_MAPPED)) {
        ...
        pApdu[4] = (uint8_t)(wCmdLen + ((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE_MAPPED) ? wDataLenTotal
                    + wAddDataLenTotal : wDataLen + wAddDataLen));
      }
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phpalMifare_ExchangeL4(
              pDataParams->pPalMifareDataParams,
              PH_EXCHANGE_BUFFER_FIRST | ((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE_MAPPED &&
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp,  phpalMifare_ExchangeL4(
              pDataParams->pPalMifareDataParams,
              PH_EXCHANGE_BUFFER_CONT | ((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE_MAPPED &&
                      wMoreDataToTransmit) ? PH_EXCHANGE_TXCHAINING : 0),
              &pCmd[1 + wCmdIdx],
              wCmdLen,
              &pRecv,
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phpalMifare_ExchangeL4(
              pDataParams->pPalMifareDataParams,
              PH_EXCHANGE_BUFFER_LAST | ((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE_MAPPED &&
                      wMoreDataToTransmit) ? PH_EXCHANGE_TXCHAINING : 0),
              &pLe[wTrailerIdx],
    ...
    uint32_t *pBytesRead)
{
  ...
  if (status == PH_ERR_SUCCESS) {
    ...
    if (pDataParams->bAuthMode == PHAL_MFDFLIGHT_NOT_AUTHENTICATED ||
        (pDataParams->bAuthMode == PHAL_MFDFLIGHT_AUTHENTICATE &&
            ((wOption & PH_EXCHANGE_CUSTOM_BITS_MASK) == PHAL_MFDFLIGHT_COMMUNICATION_PLAIN))) {
      *pBytesRead -= 2;
      return phalMfdfLight_Int_ComputeErrorResponse(pDataParams, statusTmp);
    }
