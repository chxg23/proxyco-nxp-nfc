    ...
    void *pTMIDataParams, void *pHalDataParams)
{
  ...
  pDataParams->bShortLenApdu =
      0x01; /* Since Prime variant supports only Short Length APDU, setting this to 0x01 always */
    ...
    uint8_t *pPcdCapsIn, uint8_t *pPcdCapsOut, uint8_t *pPdCapsOut)
{
  ...
  if ((bAuthOption == PHAL_MFDFLIGHT_AUTHFIRST_NON_LRP) ||
      (bAuthOption == PHAL_MFDFLIGHT_AUTHNONFIRST_NON_LRP)) {
  ...
  * using the below flag variable */
  if ((bOption == PHAL_MFDFLIGHT_SET_CONFIG_OPTION5) && (pData[4] == 0x02)) {
    ...
    pDataParams->bCryptoMethod =
        PH_CRYPTOSYM_CONFIG_LRP; /*  This is just a value. Same to be checked inside phalMfdfLight_Sw_Int_Write_Enc API */
    bEnableLrp = 0x01;
  }
    ...
    uint8_t *pUidLength)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phalMfdfLight_SetConfig(pDataParams,
          PHAL_MFDFLIGHT_ADDITIONAL_INFO, (uint16_t)(*pUidLength)));
    ...
    uint8_t *pVerInfo, uint8_t *pVerLength)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phalMfdfLight_Sw_Int_ReadData_Plain(
          pDataParams,
          (pDataParams->bAuthMode == PHAL_MFDFLIGHT_AUTHENTICATEEV2) ? PHAL_MFDFLIGHT_COMMUNICATION_MACD :
   ...
   */
  if (((wRxlen != PHAL_MFDFLIGHT_DEF_VERSION_LENGTH) &&
          (wRxlen != PHAL_MFDFLIGHT_VERSION_LENGTH_WITH_FABKEY_ID)) &&
  ...
  *pVerLength = (uint8_t)(wRxlen);
  ...
  phalMfdfLight_Sw_SetConfig((phalMfdfLight_Sw_DataParams_t *)pDataParams,
      PHAL_MFDFLIGHT_ADDITIONAL_INFO, wRxlen);
  ...
  */
  ...
  if (memcmp(pDataParams->pAid, bAppId, 3) == 0x00) {
    ...
    if ((bKeyNo > PHAL_MFDFLIGHT_ORIGINALITY_KEY_LAST) ||
        (bKeyNo < PHAL_MFDFLIGHT_ORIGINALITY_KEY_FIRST)) {
      return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_AL_MFDFLIGHT);
    }
  } else {
    /* At App level, 0,1,2,3,4 are valid ones. */
  ...
  if ((bOption & 0x0FU) == PH_EXCHANGE_RXCHAINING) {
    ...
    if (dwTMIStatus == PH_ON) {
      PH_CHECK_SUCCESS_FCT(statusTmp, phTMIUtils_CollectTMI((phTMIUtils_t *)pDataParams->pTMIDataParams,
              (uint8_t)((dwDataLen == 0) ? (PH_TMIUTILS_READ_INS | PH_TMIUTILS_ZEROPAD_CMDBUFF) :
  ...
  if (pDataParams->bAuthMode == PHAL_MFDFLIGHT_NOT_AUTHENTICATED) {
    ...
    if ((bCommOption & 0xF0U) == PHAL_MFDFLIGHT_COMMUNICATION_MACD) {
      pDataParams->dwPayLoadLen = pDataParams->dwPayLoadLen + ((pDataParams->bAuthMode ==
                  PHAL_MFDFLIGHT_AUTHENTICATE) ? 0x04 : 0x08);
    }
    if ((bCommOption & 0xF0U) == PHAL_MFDFLIGHT_COMMUNICATION_ENC) {
      if (pDataParams->bAuthMode == PHAL_MFDFLIGHT_AUTHENTICATEEV2) {
        ...
        pDataParams->dwPayLoadLen  = pDataParams->dwPayLoadLen  + ((pDataParams->bAuthMode ==
                    PHAL_MFDFLIGHT_AUTHENTICATE) ? 0x02 : 0x04);
        bIvLen = (pDataParams->bAuthMode == PHAL_MFDFLIGHT_AUTHENTICATEAES) ?
            PH_CRYPTOSYM_AES_BLOCK_SIZE : PH_CRYPTOSYM_DES_BLOCK_SIZE;
    ...
    uint8_t *pRecCount, uint8_t *pRecSize, uint8_t **ppRxdata, uint16_t *pRxdataLen)
{
  ...
  if ((bCommOption & 0x0FU) ==  PH_EXCHANGE_RXCHAINING) {
    ...
    if (dwTMIStatus == PH_ON) {
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phTMIUtils_CollectTMI((phTMIUtils_t *)pDataParams->pTMIDataParams,
              (uint8_t)((dwNumRec == 0) ? (PH_TMIUTILS_READ_INS | PH_TMIUTILS_ZEROPAD_CMDBUFF) :
                  PH_TMIUTILS_ZEROPAD_CMDBUFF),
              bCmdBuff,
              wCmdLen,
              NULL,
    ...
    uint8_t *pTMRI, uint8_t *pEncTMRI)
{
  ...
  */
  ...
  if ((bOption == PHAL_MFDFLIGHT_COMMUNICATION_PLAIN) && (wRxlen != 0)) {
    return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_AL_MFDFLIGHT);
  } else if ((bOption == PHAL_MFDFLIGHT_COMMUNICATION_MACD) &&
      (wRxlen != 16)) {  /* Response should be 16 bytes long */
    return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_AL_MFDFLIGHT);
  }
    ...
    uint8_t bDFnameLen, uint8_t bExtendedLenApdu, uint8_t **ppFCI, uint16_t *pwFCILen)
{
  ...
  if ((statusTmp == PH_ERR_SUCCESS) ||
      (wVal == PHAL_MFDFLIGHT_ISO7816_ERR_LIMITED_FUNCTIONALITY_INS)) {
...
phStatus_t phalMfdfLight_Sw_ResetAuthentication(phalMfdfLight_Sw_DataParams_t *pDataParams)
{
  ...
  status = phalMfdfLight_Sw_GetConfig((phalMfdfLight_Sw_DataParams_t *)pDataParams,
          PHAL_MFDFLIGHT_ADDITIONAL_INFO, &pValue);
  if (pValue == 0xFFFF) {
