...
)
{
  ...
  if (!((bNumTimeSlots == PHPAL_FELICA_NUMSLOTS_1) ||
    ...
    wTimeOutUs = FDTF_LISTEN_SENSF_REQ((((uint16_t)bNumTimeSlots) + 1U),
            wTR1ValueUs) - wTR1ValueUs + PHPAL_FELICA_SW_T_DELTA_F_POLL_US;
  }
  ...
  if (bNumTimeSlots > PHPAL_FELICA_NUMSLOTS_1) {
    ...
    *pRxLength = wRxLengthTmp;
    ...
    if ((wRxLengthTmp == (PHPAL_FELICA_ATQC_LENGTH + PHPAL_FELICA_RD_LENGTH + 1U)) &&
        (pDataParams->bRequestCode == 0U)) {
      return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_PAL_FELICA);
    }
    /* Store IDm and PMm */
    ...
    *pRxLength = (uint16_t)(((uint16_t)pRxBufferTmp[0]) - (PHPAL_FELICA_SW_LEN_BYTE_SIZE +
...
)
{
  ...
  *pwStatus = PH_ERR_SUCCESS;
  ...
  /* Check length */
  if (((pResponseBuffer[bStartPos + PHHAL_HW_STATUS_FRAME_LEN_BYTE_POS] !=
              (PHPAL_FELICA_ATQC_LENGTH + PHPAL_FELICA_SW_LEN_BYTE_SIZE)) &&
          (pResponseBuffer[bStartPos + PHHAL_HW_STATUS_FRAME_LEN_BYTE_POS] != (PHPAL_FELICA_ATQC_LENGTH +
                  PHPAL_FELICA_RD_LENGTH + PHPAL_FELICA_SW_LEN_BYTE_SIZE))) ||
  ...
  /* RC = 0 and card response with RD information */
  if ((pResponseBuffer[bStartPos + PHHAL_HW_STATUS_FRAME_LEN_BYTE_POS] ==
          (PHPAL_FELICA_ATQC_LENGTH + PHPAL_FELICA_RD_LENGTH + PHPAL_FELICA_SW_LEN_BYTE_SIZE)) &&
      (pDataParams->bRequestCode == 0U)) {
    return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_PAL_FELICA);
  }

  /* Extract error from Status byte */
  bError = pResponseBuffer[(bStartPos) + PHHAL_HW_STATUS_FRAME_ERR_BYTE_POS ] &
  ...
  if (bError != 0x00U) {
    ...
    *ppID = &pResponseBuffer[bStartPos + (PHPAL_FELICA_SW_LEN_BYTE_SIZE +
                          PHPAL_FELICA_SW_RESP_REQC_SIZE)];
    *pLen = pResponseBuffer[bStartPos + PHHAL_HW_STATUS_FRAME_LEN_BYTE_POS] -
        (PHPAL_FELICA_SW_LEN_BYTE_SIZE + PHPAL_FELICA_SW_RESP_REQC_SIZE);
  }
...
)
{
  ...
  */
  if ((0U == ((wOption & PH_EXCHANGE_LEAVE_BUFFER_BIT))) || (pDataParams->bLength < 2U)) {
    ...
    /* Prepare header */
    aHeader[0] =
        0x00;          /* This is just a dummy which is updated with the correct value later on,
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
          PHHAL_HW_CONFIG_TXBUFFER, (uint16_t)pDataParams->bLength));
