...
phStatus_t phalMfdf_Sam_NonX_Int_ValidateResponse(void * pDataParams, uint16_t wStatus, uint16_t wPiccRetCode)
{
		...
		((wStatus & PH_ERR_MASK) == (PH_ERR_CUSTOM_BEGIN + 23) /* DESFire General Errors */))
	{
		...
		PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Int_ComputeErrorResponse(pDataParams, (uint16_t) (wPiccRetCode & 0x00FF)));
	}
	else
	{
	...
	uint16_t wDataLen, uint8_t ** ppResponse, uint16_t * pRespLen, uint8_t * pPiccErrCode)
{
	...
	else
	{
			...
			(bCmdOption & PHALMFDF_SAM_NONX_CMD_OPTION_COMPLETE))
		{
			...
			/* Add the status code. */
			*pPiccErrCode = pResponse[(bCmdOption & PHALMFDF_SAM_NONX_PICC_STATUS_WRAPPED) ? (wRespLen - 1) : 0];
	...
	uint8_t * pDivInput, uint8_t bDivInputLen)
{
	...
	PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_Int_SetAuthMode(pDataParams, PHAL_MFDF_NOT_AUTHENTICATED));
	...
	if(bAuthMode != PHAL_MFDF_NOT_AUTHENTICATED)
	{
		...
		if (bCommMode == PHAL_MFDF_COMMUNICATION_ENC)
		{
			if(!bIsReadCmd)
			{
				...
				/* Set the buffering flag to Default. */
				wBuffOption = (uint16_t) ((bAuthMode != PHAL_MFDF_AUTHENTICATE) ? PH_EXCHANGE_BUFFER_FIRST : PH_EXCHANGE_DEFAULT);
		...
		else
		{
			...
			if(bIsWriteCmd || (bAuthMode != PHAL_MFDF_AUTHENTICATE))
			{
				...
				if(bAuthMode != PHAL_MFDF_AUTHENTICATE)
				{
					...
					#ifdef NXPBUILD__PHAL_MFDF_SAMAV2_NONX
					if((PH_GET_COMPID(pDataParams) == PHAL_MFDF_SAMAV2_ID) && (wBuffOption & PH_EXCHANGE_TXCHAINING))
					{
	...
	uint8_t bPiccStat, uint8_t * pRespMac, uint16_t wRespMacLen, uint8_t ** ppOutBuffer, uint16_t * pOutBufLen)
{
	...
	if(bAuthMode != PHAL_MFDF_NOT_AUTHENTICATED)
	{
		...
		if(bCommMode == PHAL_MFDF_COMMUNICATION_ENC)
		{
			...
			bExchangeStatus = (uint8_t) (((wOption & PH_EXCHANGE_CUSTOM_BITS_MASK) != PHALMFDF_SAM_NONX_EXCHANGE_PICC_STATUS) ? PH_OFF : bExchangeStatus);
	...
	uint8_t * pCmdBuff, uint16_t wCmdLen, uint8_t ** ppResponse, uint16_t * pRespLen)
{
	...
	if(((wOption & 0xFF0F) == PH_EXCHANGE_DEFAULT) && !(wOption & PHALMFDF_SAM_NONX_EXCHANGE_WITHOUT_SM))
	{
	...
	bExchangeMac = (uint8_t) ((bAuthMode == PHAL_MFDF_AUTHENTICATE) ? PHALMFDF_SAM_NONX_MAC_ON_CMD : PHALMFDF_SAM_NONX_NO_MAC_ON_CMD);
	...
	bOption = (uint8_t) ((wBuffOption == PH_EXCHANGE_RXCHAINING) ? PHALMFDF_SAM_NONX_CMD_OPTION_COMPLETE : PHALMFDF_SAM_NONX_CMD_OPTION_NONE);
	...
	do
	{
		...
		if((wBuffOption != PH_EXCHANGE_DEFAULT) && ((wBuffOption != PH_EXCHANGE_RXCHAINING)))
		{
			...
			bOption = (uint8_t) (PHALMFDF_SAM_NONX_CMD_OPTION_COMPLETE | (wOption & PH_EXCHANGE_CUSTOM_BITS_MASK));
		...
		else
		{
			...
			if(!(wOption & PHALMFDF_SAM_NONX_EXCHANGE_WITHOUT_SM))
			{
				...
				do
				{
					...
					{
						wMacLen = (uint16_t) (((bAuthMode == PHAL_MFDF_AUTHENTICATE) || (bAuthMode == PHAL_MFDF_NOT_AUTHENTICATED) ||
						...
						if(bIsDataCmd)
						{
							...
							wMacLen = (uint8_t) (((bAuthMode == PHAL_MFDF_AUTHENTICATE) && (bResp_ComMode == PHAL_MFDF_COMMUNICATION_PLAIN)) ? 0 : wMacLen);
						}
					}
					...
					if((wStatus == PH_ERR_SUCCESS) || ((wStatus & PH_ERR_MASK) == PH_ERR_SUCCESS_CHAINING))
					{
						if(bResp_ComMode == PHAL_MFDF_COMMUNICATION_ENC)
						{
							...
							if(bFirstFrame || ((wOption & PH_EXCHANGE_CUSTOM_BITS_MASK) == PHALMFDF_SAM_NONX_RETURN_CHAINING_STATUS))
							{
								*pRespLen = 0;
							}
					...
					if(pRespLen != NULL)
					{
						*pRespLen -= (uint16_t) ((bLastFrame && (bResp_ComMode != PHAL_MFDF_COMMUNICATION_ENC)) ? wMacLen : 0);
					}
		...
		/* Set finished flag. */
		if((wStatus1 == PH_ERR_SUCCESS) || ((wOption & PH_EXCHANGE_CUSTOM_BITS_MASK) == PHALMFDF_SAM_NONX_RETURN_CHAINING_STATUS))
		{
			bFinished = PH_ON;
		}
	}while(!bFinished);
	...
	uint8_t * pCmdBuff, uint16_t wCmdLen, uint8_t * pData, uint32_t dwDataLen, uint8_t ** ppResponse, uint16_t * pRespLen)
{
	...
	do
	{
		...
		if(!(wOption & PHALMFDF_SAM_NONX_EXCHANGE_WITHOUT_SM) && (bAuthMode != PHAL_MFDF_NOT_AUTHENTICATED))
		{
			...
			else
			{
				if(wStatus == PH_ERR_SUCCESS)
				{
					...
					if(wSMDataLen < wPICCFrameLen)
					{
						...
						/* Set if Mac on command is required. */
						bExchangeMac = (uint8_t) ((bCmd_ComMode == PHAL_MFDF_COMMUNICATION_MACD) ? PHALMFDF_SAM_NONX_MAC_ON_CMD :
		...
		if(!bPiccExchangeComplete)
		{
			do
			{
				...
				/* Frame the total length. */
				wTotLen = 0;
				wTotLen = (uint16_t) (wCmdLen_Tmp + ((bCmd_ComMode == PHAL_MFDF_COMMUNICATION_ENC) ? bDataLen : dwDataLen));
				...
				bCmdOption = (uint8_t) ((wBuffOption_PICC == PH_EXCHANGE_BUFFER_LAST) ? PHALMFDF_SAM_NONX_CMD_OPTION_COMPLETE :
					PHALMFDF_SAM_NONX_CMD_OPTION_NONE);
				...
				wStatus1 = phalMfdf_Sam_NonX_Int_CardExchange(
					...
					(uint16_t) ((bCmd_ComMode == PHAL_MFDF_COMMUNICATION_ENC) ? bDataLen : ((bLastFrame && bExchangeMac) ? wSMDataLen : 0)),
					&pCardResponse,
					&wCardRespLen,
					&bPiccErrCode);
	...
	/* Perform Secure messaging verification only if required. */
	if(!(wOption & PHALMFDF_SAM_NONX_EXCHANGE_WITHOUT_SM) && !(wOption & PHALMFDF_SAM_NONX_RETURN_CHAINING_STATUS))
	{
		/* Reset the Authentication. */
			...
			else
			{
				if(bResp_ComMode != PHAL_MFDF_COMMUNICATION_PLAIN)
				{
					PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_Int_VerifySM(
						...
						bPiccErrCode,
						((pCmdBuff[0] == PHAL_MFDF_CMD_COMMIT_TXN) && (wCmdLen == 2)) ? &pCardResponse[12] : pCardResponse,
						(uint16_t) (((pCmdBuff[0] == PHAL_MFDF_CMD_COMMIT_TXN) && (wCmdLen == 2)) ? (wCardRespLen - 12) : wCardRespLen),
						ppResponse,
						pRespLen));
		...
		/* Evaluate the response. */
		PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_Int_ValidateResponse(pDataParams, wStatus1, bPiccErrCode));
	}
	else
	{
		if(pRespLen != NULL)
