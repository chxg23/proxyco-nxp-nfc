...
#ifdef NXPBUILD__PHAL_MFP_SAMAV2_NONX
phStatus_t phalMfdf_SamAV2_Init(phalMfdf_SamAV2_DataParams_t * pDataParams, uint16_t wSizeOfDataParams, phhalHw_SamAV2_DataParams_t * pHalSamDataParams,
...
#ifdef NXPBUILD__PHAL_MFP_SAMAV3_NONX
phStatus_t phalMfdf_SamAV3_NonX_Init(phalMfdf_SamAV3_NonX_DataParams_t * pDataParams, uint16_t wSizeOfDataParams, phhalHw_SamAV3_DataParams_t * pHalSamDataParams,
	...
	uint8_t bDivInputLen)
{
	...
	PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_Int_SetAuthMode(pDataParams, PHAL_MFDF_AUTHENTICATE));
	...
	uint8_t bDivInputLen)
{
	...
	PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_Int_SetAuthMode(pDataParams, PHAL_MFDF_AUTHENTICATEISO));
	PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_Int_SetKeyNo(pDataParams, bKeyNoCard));
	...
	uint8_t bDivInputLen)
{
	...
	PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_Int_SetAuthMode(pDataParams, PHAL_MFDF_AUTHENTICATEAES));
	PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_Int_SetKeyNo(pDataParams, bKeyNoCard));
...
phStatus_t phalMfdf_Sam_NonX_GetKeySettings(void * pDataParams, uint8_t * pKeySettings)
{
	...
	bComMode = (uint8_t) ((bAuthMode == PHAL_MFDF_AUTHENTICATE) ? PHAL_MFDF_COMMUNICATION_MACD : PHAL_MFDF_COMMUNICATION_PLAIN);
...
phStatus_t phalMfdf_Sam_NonX_GetDFNames(void * pDataParams, uint8_t bOption, uint8_t * pDFBuffer, uint8_t * bSize)
{
	phStatus_t	PH_MEMLOC_REM wStatus = 0;
	uint8_t		PH_MEMLOC_REM aCmdBuff[1];
	uint8_t		PH_MEMLOC_REM bComMode = 0;
	...
	/* Frame the command information. */
	aCmdBuff[0] = (uint8_t) (((bOption & 0x0FU) == PH_EXCHANGE_DEFAULT) ? PHAL_MFDF_CMD_GET_DF_NAMES : PHAL_MFDF_RESP_ADDITIONAL_FRAME);
...
phStatus_t phalMfdf_Sam_NonX_GetVersion(void * pDataParams, uint8_t * pVerInfo)
{
	...
	PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_SetConfig(pDataParams, PHAL_MFDF_ADDITIONAL_INFO, wRespLen));
	...
	uint16_t * pRespLen)
{
	...
	bCmd_ComMode = (uint8_t) (((bAuthMode != PHAL_MFDF_AUTHENTICATE) || (bCmd_ComMode == PHAL_MFDF_COMMUNICATION_MACD)) ?
		PHAL_MFDF_COMMUNICATION_MACD : PHAL_MFDF_COMMUNICATION_PLAIN);
	bCmd_ComMode = (uint8_t) (((bAuthMode == PHAL_MFDF_AUTHENTICATE) && ((bOption & 0xF0) == PHAL_MFDF_COMMUNICATION_PLAIN)) ?
...
phStatus_t phalMfdf_Sam_NonX_WriteData(void * pDataParams, uint8_t bOption, uint8_t bFileNo, uint8_t * pOffset, uint8_t * pData, uint8_t * pDataLen)
{
	...
	/* Frame the SM to be applied for command. */
	bResp_ComMode = (uint8_t) ((bAuthMode == PHAL_MFDF_AUTHENTICATE) ? PHAL_MFDF_COMMUNICATION_PLAIN : PHAL_MFDF_COMMUNICATION_MACD);
	bResp_ComMode = (uint8_t) (((bAuthMode == PHAL_MFDF_AUTHENTICATE) && ((bOption & 0xF0) == PHAL_MFDF_COMMUNICATION_PLAIN)) ?
...
phStatus_t phalMfdf_Sam_NonX_GetValue(void * pDataParams, uint8_t bCommOption, uint8_t bFileNo, uint8_t * pValue)
{
	...
	bCmd_ComMode = (uint8_t) (((bAuthMode != PHAL_MFDF_AUTHENTICATE) || (bCmd_ComMode == PHAL_MFDF_COMMUNICATION_MACD)) ?
		PHAL_MFDF_COMMUNICATION_MACD : PHAL_MFDF_COMMUNICATION_PLAIN);
	bCmd_ComMode = (uint8_t) (((bAuthMode == PHAL_MFDF_AUTHENTICATE) && (bCommOption == PHAL_MFDF_COMMUNICATION_PLAIN)) ?
		PHAL_MFDF_COMMUNICATION_PLAIN : bCmd_ComMode);
...
phStatus_t phalMfdf_Sam_NonX_Credit(void * pDataParams, uint8_t bCommOption, uint8_t bFileNo, uint8_t * pValue)
{
	...
	/* Frame the SM to be applied for command. */
	bResp_ComMode = (uint8_t) ((bAuthMode == PHAL_MFDF_AUTHENTICATE) ? PHAL_MFDF_COMMUNICATION_PLAIN : PHAL_MFDF_COMMUNICATION_MACD);
	bResp_ComMode = (uint8_t) (((bAuthMode == PHAL_MFDF_AUTHENTICATE) && (bCommOption == PHAL_MFDF_COMMUNICATION_PLAIN)) ?
		PHAL_MFDF_COMMUNICATION_PLAIN : bResp_ComMode);
	...
	uint8_t ** ppFCI, uint16_t * pFCILen)
{
	...
	PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_Int_WriteData(
		pDataParams,
		(uint16_t) (PH_EXCHANGE_DEFAULT | PHALMFDF_SAM_NONX_EXCHANGE_WITHOUT_SM | PHALMFDF_SAM_NONX_PICC_STATUS_WRAPPED),
		PH_OFF,
		PHAL_MFDF_COMMUNICATION_PLAIN,
		PHAL_MFDF_COMMUNICATION_PLAIN,
		PH_OFF,
	...
	uint8_t ** ppResponse, uint16_t * pBytesRead)
{
	...
	wStatus = phalMfdf_Sam_NonX_Int_ReadData(
		pDataParams,
		(uint16_t) (wOption | PHALMFDF_SAM_NONX_EXCHANGE_WITHOUT_SM | PHALMFDF_SAM_NONX_PICC_STATUS_WRAPPED),
		PH_ON,
		PHAL_MFDF_COMMUNICATION_PLAIN,
		PHAL_MFDF_COMMUNICATION_PLAIN,
		0,
...
phStatus_t phalMfdf_Sam_NonX_IsoAuthenticate(void * pDataParams, uint16_t wKeyNo, uint16_t wKeyVer, uint8_t bKeyNoCard, uint8_t bIsPICCkey)
{
	...
	PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_IsoGetChallenge(pDataParams, wKeyNo, wKeyVer, bRndLen, aRnd));
	...
	PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_IsoExternalAuthenticate(pDataParams, aData, bDataLen, aRnd,
		&bDataLen));
	...
	PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_IsoInternalAuthenticate(pDataParams, aData, bDataLen, NULL, NULL));
...
phStatus_t phalMfdf_Sam_NonX_SetConfig(void * pDataParams, uint16_t wConfig, uint16_t wValue)
{
	...
	switch (wConfig)
    {
		...
		case PHAL_MFDF_WRAPPED_MODE:
			PH_CHECK_SUCCESS_FCT(wStatus, phalMfdf_Sam_NonX_Int_SetWrappedMode(pDataParams, (uint8_t) wValue));
			break;
