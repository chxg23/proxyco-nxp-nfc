...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
          PHHAL_HW_CONFIG_TXLASTBITS, bTxLastBits));
...
)
{
  ...
  /* Parameter check */
  if ((bTarget > 0x07U) || (bAction > 0x07U) || (bMemBank > 0x03U) || (bPointerLength > 3U) ||
      (bTruncate > 1U)) {
    return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_PAL_I18000P3M3);
  }
  ...
  /* Build Command frame */
  bCmdLength = 0;
  bCmd[bCmdLength++] = (PHPAL_I18000P3M3_CMD_SELECT << 4U) | (bTarget << 1U) |
      (bAction >> 2U);  /* 0 bits left */
  bCmd[bCmdLength]   = (bAction << 6U) | (bMemBank <<  4U) | (bPointerLength <<
          2U);             /* 2 bits left */
  ...
  /* set Tx-CRC as CRC16  */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
          PHHAL_HW_CONFIG_CRCTYPE, PHHAL_HW_CRCTYPE_CRC16 | PHHAL_HW_CRCOPTION_INVERTED));
...
)
{
  ...
  /* Parameter check */
  if ((bDr > 0x01U) || (bM < 0x02U) || (bM > 0x03U) || (bTRext > 1U) || (bSel > 0x03U) ||
      (bSession > 0x03U) || (bRfu > 0x01U) || (bQ > 0x0FU)) {
    return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_PAL_I18000P3M3);
  }
  ...
  /* Build Command frame */
  aCmd[0] = (PHPAL_I18000P3M3_CMD_BEGIN_ROUND << 4U) | (bDr << 3U) | (bM << 1U) |
      (bTRext);  /* 0 bits left */
  aCmd[1] = (bSel << 6U) | (bSession << 4U) | (bRfu << 3U) | (bQ >>
          1U);                      /* 0 bits left */
  aCmd[2] = (bQ <<
          7U);                                                                    /* 7 bits left */
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(
          ...
          PHPAL_I18000P3M3_SW_DELTA_MIN_US));
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
          PHHAL_HW_CONFIG_CRCTYPE, PHHAL_HW_CRCTYPE_CRC5));
...
)
{
  ...
  /* Build Command frame */
  aCmd[0] = (PHPAL_I18000P3M3_CMD_RESIZE_ROUND << 4U) | (pDataParams->bSession << 2U) |
      (bUpDn >> 1U);   /* 0 bits left */
  aCmd[1] = (bUpDn <<
          7U);                                                                             /* 7 bits left */
...
)
{
  ...
  /* Build Command frame */
  aCmd[0] = (PHPAL_I18000P3M3_CMD_NEXT_SLOT << 4U) | (pDataParams->bSession <<
          2U); /* 2 bits left */
...
)
{
  ...
  /* Build Command frame */
  bCmdLength = 0;
  *(pSelectCmd + bCmdLength++) = (PHPAL_I18000P3M3_CMD_SELECT << 4U) | (bTarget << 1U) |
      (bAction >> 2U);  /* 0 bits left */
  *(pSelectCmd + bCmdLength)   = (bAction << 6U) | (bMemBank <<  4U) |
      (bPointerLength << 2U);             /* 2 bits left */
...
)
{
  ...
  /* Build Command frame */
  *pBeginRnd = (PHPAL_I18000P3M3_CMD_BEGIN_ROUND << 4U) | (bDr << 3U) | (bM << 1U) |
      (bTRext);  /* 0 bits left */
  *(pBeginRnd + 1U) = (bSel << 6U) | (bSession << 4U) | (bRfu << 3U) |
      (bQ >> 1U);                      /* 0 bits left */
  *(pBeginRnd + 2U) = (bQ <<
          7U);                                                                    /* 7 bits left */
