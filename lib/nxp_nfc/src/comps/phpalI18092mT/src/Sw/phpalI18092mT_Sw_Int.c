  ...
  if (bExpectedResponseCode == PHPAL_I18092MT_SW_CMD_DEP_REQ) {
    ...
    if ((0U == ((pDataParams->bStateNow & PHPAL_I18092MT_SW_STATE_CHAINING_BIT))) &&
      ...
      if (0U != (pDataParams->bNadEnabled)) {
        ...
        pDataParams->bNadVal = (uint8_t)(pDataParams->bNadVal | ((uint8_t)(pFrame[bOffset++] &
...
)
{
  ...
  if (0U != (pDataParams->bStateNow & PHPAL_I18092MT_SW_STATE_CHAINING_BIT)) {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
            PHHAL_HW_CONFIG_TXBUFFER_OFFSET, (uint16_t)(bLengthPos + 3U)));
...
)
{
  ...
  /* Write frame header */
  PH_CHECK_SUCCESS_FCT(statusTmp, phpalI18092mT_Sw_WriteFrameHeader(pDataParams,
          PH_EXCHANGE_BUFFER_FIRST, PHPAL_I18092MT_SW_CMD_DEP_RES, bDataLength));
    ...
    uint8_t bPfbMask)
{
  ...
  /* Write frame header */
  PH_CHECK_SUCCESS_FCT(status, phpalI18092mT_Sw_WriteFrameHeader(pDataParams,
          PH_EXCHANGE_BUFFER_FIRST, PHPAL_I18092MT_SW_CMD_DEP_RES, bCmdLength));
