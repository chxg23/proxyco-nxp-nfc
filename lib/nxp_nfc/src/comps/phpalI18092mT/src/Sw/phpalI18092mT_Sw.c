...
)
{
  ...
  wStatus = phOsal_EventCreate(&pDataParams->Pal18092mTEventObj.EventHandle,
          &pDataParams->Pal18092mTEventObj);
  ...
  if ((wTargetFraming & PHHAL_HW_RF_DATARATE_OPTION_MASK) != a18092T_DrTable[bDrt]) {
    ...
    if ((a18092T_DrTable[bDrt] == PHHAL_HW_RF_DATARATE_106) || (bNFCMode == PH_ON)) {
      PH_CHECK_SUCCESS_FCT(status, phhalHw_SetConfig(pDataParams->pHalDataParams,
              PHHAL_HW_CONFIG_NFCIP_STARTBYTE, PH_ON));
    } else {
      PH_CHECK_SUCCESS_FCT(status, phhalHw_SetConfig(pDataParams->pHalDataParams,
              PHHAL_HW_CONFIG_NFCIP_STARTBYTE, PH_OFF));
    }
  }
  ...
  */
  ...
  if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS) {
    ...
    if (0U == (pDataParams->bRtoxDisabled)) {
      /* XXX There is no active timer here, so no need to stop */
    }
  }
...
)
{
  ...
  * shall disable to receive a subsequent ATR_REQ. */
  if ((((wDataRate & PHHAL_HW_RF_FRAMING_OPTION_MASK) == PHHAL_HW_RF_TYPE_ACTIVE_FRAMING) &&
          (pDataParams->bCmdtype == PHPAL_I18092MT_SW_CMD_WUP_REQ)) ||
  ...
  do {
    ...
    if (status == PH_ERR_SUCCESS) {
      ...
      if ((wDataRate & PHHAL_HW_RF_FRAMING_OPTION_MASK) == PHHAL_HW_RF_TYPE_F_FRAMING) {
        if (memcmp(&pFrame[PHPAL_I18092MT_SW_ATR_REQ_NFCID3_POS + 1U], &pAtrRes[0],
                (PHPAL_I18092MT_NFCID3_LENGTH - 2)) != 0) {
      ...
      if ((pFrame[PHPAL_I18092MT_SW_ATR_REQ_DID_POS] > PHPAL_I18092MT_DID_MAX) &&
  ...
  */
  ...
  (void)memmove(pDataParams->aNfcid3i, &pFrame[PHPAL_I18092MT_SW_ATR_REQ_NFCID3_POS],
      PHPAL_I18092MT_NFCID3_LENGTH);

  /* Copy the DID, BSI and BRI */
  pDataParams->bDid = pFrame[PHPAL_I18092MT_SW_ATR_REQ_DID_POS];
  pDataParams->bBsi = pFrame[PHPAL_I18092MT_SW_ATR_REQ_BSI_POS] & ((uint8_t)(~
              (uint8_t)PHPAL_I18092MT_BIT_RATE_MASK));
  pDataParams->bBri = pFrame[PHPAL_I18092MT_SW_ATR_REQ_BRI_POS] & ((uint8_t)(~
              (uint8_t)PHPAL_I18092MT_BIT_RATE_MASK));
  ...
  */
  /* copy LRi */
  pDataParams->bLri = (pFrame[PHPAL_I18092MT_SW_ATR_REQ_PPI_POS] >>
          PHPAL_I18092MT_SW_ATR_PPT_LRT_BIT) & 0x03U;
  ...
  if (0U != (pFrame[PHPAL_I18092MT_SW_ATR_REQ_PPI_POS] &
          PHPAL_I18092MT_SW_ATR_REQ_PPI_GT_AVAILABLE)) {
  ...
  */
  ...
  pDataParams->bFsl = (pDataParams->bLrt < pDataParams->bLri) ? pDataParams->bLrt :
      pDataParams->bLri;
...
)
{
  ...
  if (wOption == PH_RECEIVE_CHAINING) {
    ...
    /* Stop timer */
    if (0U == (pDataParams->bRtoxDisabled)) {
      /* XXX There is no timer here, so no need to stop */
    }
  }
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
          PHHAL_HW_CONFIG_RXBUFFER_STARTPOS, 0x00));
  ...
  *pTransportDataBytesLength = (uint16_t)(wRxLen - bProtLength);
  ...
  if ((!pDataParams->bRtoxDisabled) &&
    ...
    os_cputime_delay_usecs(PHPAL_I18092MT_SW_CALCULATE_RWT_US(pDataParams->bTo) *
        pDataParams->bRtoxExtnVal);
...
)
{
  ...
  do {
    ...
    if (((uint16_t)bHeaderLength + wTxBufferLen + wBytesToSend) > wMaxFrameSize) {
      ...
      if ((0U == ((pDataParams->bStateNow & PHPAL_I18092MT_SW_STATE_CHAINING_BIT)))) {
        ...
        pDataParams->bStateNow |= (PHPAL_I18092MT_SW_STATE_CHAINING_BIT |
                PHPAL_I18092MT_SW_STATE_CHAINING_FIRST_BIT);
      }
    }
    ...
    if ((wOption & PH_TRANSMIT_MODE_MASK) == PH_TRANSMIT_TXCHAINING) {
      ...
      if ((wBytesToSend == wTransportDataBytesLength) &&
          (0U == ((wOption & PH_TRANSMIT_BUFFERED_BIT)))) {
    ...
    if (((0U == ((wOption & PH_TRANSMIT_LEAVE_BUFFER_BIT)))) ||
      ...
      * Header should be only be updated once again from next Chained packet. */
      if ((0U == ((wRxOption & PH_TRANSMIT_LEAVE_BUFFER_BIT)))) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phpalI18092mT_Sw_UpdatePduHeader(pDataParams,
                (uint8_t)(wBytesToSend + wTxBufferLen)));
    ...
    /* Clean-up state */
    pDataParams->bStateNow &= (uint8_t)~(uint8_t)(PHPAL_I18092MT_SW_STATE_CHAINING_FIRST_BIT |
            PHPAL_I18092MT_SW_STATE_CHAINING_LAST_BIT);

    /* Chaining is active */
    if (pDataParams->bStateNow == (PHPAL_I18092MT_SW_STATE_TX |
            PHPAL_I18092MT_SW_STATE_CHAINING_BIT)) {
  ...
  while ((pDataParams->bStateNow & PHPAL_I18092MT_SW_STATE_MASK) !=
      PHPAL_I18092MT_SW_STATE_INFORMATION_PDU_RX);
    ...
    case PHPAL_I18092MT_CONFIG_NAD:
      if (0U != (wValue & PHPAL_I18092MT_MASK_NAD_ENABLE)) {
        ...
        if ((wValue & PHPAL_I18092MT_MASK_NAD_VALUE) <= PHPAL_I18092MT_NAD_MAX) {
          pDataParams->bNadVal = (uint8_t)(pDataParams->bNadVal | ((uint8_t)(wValue &
