...
)
{
  ...
  status = phhalHw_Exchange(pDataParams->pHalDataParams, PH_EXCHANGE_DEFAULT, cmd, 2, &pResp,
          &wRespLength);
...
)
{
  ...
  /* Copy valid UID bits */
  wSndBytes = (uint16_t)(((((uint16_t)bNvbUidIn) & 0xF0U) >> 4U) + (((bNvbUidIn & 0x0FU) != 0U) ?
              1U : 0U));
    ...
    *pNvbUidOut = (uint8_t)(((bBitCount >> 3U) << 4U) + (bBitCount & 0x07U));
    ...
    if (wRcvBytes > 0U) {
      ...
      if (0U != (bNvbUidIn & 0x07U)) {
        bCmdBuffer[2U + bUidStartIndex] = (bCmdBuffer[2U + bUidStartIndex] & (uint8_t)((1U <<
                        (bNvbUidIn & 0x07U)) - 1U)) |
      ...
      (void)memcpy(&bCmdBuffer[2U + bUidStartIndex + 1U], &pRcvBuffer[1],
          (size_t)(((uint32_t)wRcvBytes) - 1u));
    }
  ...
  *pMoreCardsAvailable = PH_OFF;
  ...
  if (pDataParams->bOpeMode == RD_LIB_MODE_EMVCO) {
    ...
    while (((status & PH_ERR_MASK) == PH_ERR_IO_TIMEOUT) &&
        (bRetryCount < PH_NXPNFCRDLIB_CONFIG_EMVCO_RETRYCOUNT)) {
      bRetryCount++;
  ...
  for (bCascadeIndex = 0; bCascadeIndex < 3U; bCascadeIndex++) {
    ...
    while (bNvbUid != 0x40U) {
      ...
      status = phpalI14443p3a_Sw_Anticollision(pDataParams, bCascadeLevel, bUid, bNvbUid, bUid,
              &bNvbUid);
    ...
    if (pDataParams->bOpeMode == RD_LIB_MODE_EMVCO) {
      bRetryCount = 0;
      while (((statusTmp & PH_ERR_MASK) == PH_ERR_IO_TIMEOUT) &&
          (bRetryCount < PH_NXPNFCRDLIB_CONFIG_EMVCO_RETRYCOUNT)) {
        bRetryCount++;
        /* Wait for at least Tmin retransmission delay. */
        PH_CHECK_SUCCESS_FCT(status, phhalHw_Wait(
...
)
{
  ...
  /* Send ReqA command */
  cmd[0] = bReqCode;
  statusTmp = phhalHw_Exchange(pDataParams->pHalDataParams, PH_EXCHANGE_DEFAULT, cmd, 1, &pResp,
          &wRespLength);
