    ...
    uint8_t                    bDevType)
{
  ...
  if (phlnLlcp_Sw_Int_CheckGenBytes(pRemoteGnrlBytes, bRemoteGnrBytesLen,
          &(pDataParams->sRemoteLMParams)) != PH_ERR_SUCCESS) {
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp,
      phlnLlcp_Sw_Int_VersionAgreement(pDataParams->sLocalLMParams.bVersion,
          pDataParams->sRemoteLMParams.bVersion,
          &(pDataParams->bAgreedVersion)));

  /* Initialize MAC layer. */
  PH_CHECK_SUCCESS_FCT(statusTmp, phlnLlcp_Sw_MacInit(pDataParams, bDevType, &wSymmTime,
          &wLtoTime));
  ...
  (void)phlnLlcp_Sw_Transport_Socket_Init(pDataParams, &gsphlnLlcp_Socket,
      PHLN_LLCP_TRANSPORT_SERVER_CONNECTIONORIENTED, bLlcpBuffer, bLlcpBuflen);

  /* Register LLCP Link Management socket.*/
  PH_CHECK_SUCCESS_FCT(statusTmp, phlnLlcp_Sw_Int_Transport_Socket_Register(&gsphlnLlcp_Socket,
          PHLN_LLCP_TRANSPORT_SERVER_CONNECTIONORIENTED, 0, NULL, PHLN_LLCP_DEFAULTLINK_SOCKET));
  gsphlnLlcp_Socket.bState = PHLN_LLCP_SOCKET_INFO_EX;

  /* Post LLCP Activated Event to Threads Blocked for LLCP Event. */
  PH_CHECK_SUCCESS_FCT(statusTmp, phOsal_EventPost(&pDataParams->LlcpEventObj.EventHandle,
          E_OS_EVENT_OPT_NONE, E_PH_OSAL_EVT_LLCP_ACTIVATED, NULL));
  ...
  while (TRUE) {
    ...
    switch (psMsgQueue->bType) {
      ...
      case PH_TOOLS_Q_RX_DATA:
        ...
        wStatus = phlnLlcp_Sw_Int_Pdu_Process(pDataParams, psMsgQueue->pbData, psMsgQueue->dwLength,
                epType, &psQueuedSocket);
        ...
        statusTmp = phlnLlcp_Sw_Int_PostEvents(wStatus, epType, psQueuedSocket, pTempBuffer,
                pDataParams->bAgreedVersion);
...
phlnLlcp_Sw_Deactivate(phlnLlcp_Sw_DataParams_t *pDataParams)
{
  ...
  wLength = phlnLlcp_Sw_Int_Pdu_FrameHeader(PHLN_LLCP_PTYPE_DISC, gsphlnLlcp_Socket.bRsap,
          gsphlnLlcp_Socket.bLsap, 0,
          0, baDisc);
  ...
  PH_CHECK_SUCCESS_FCT(wStatus, phTools_Q_SendFront(psMsgQueue, (uint32_t)0xFFFFFFFFU,
          PH_TRANSMIT_DEFAULT));
    ...
    uint32_t dwSocketRxBufferSize)
{
  ...
  bRetstatus = phOsal_SemCreate(&psSocket->xSema.SemHandle, &psSocket->xSema,
          E_OS_SEM_OPT_SIGNALLING_SEM);
  ...
  bRetstatus = phOsal_SemCreate(&psSocket->xRxSema.SemHandle, &psSocket->xRxSema,
          E_OS_SEM_OPT_SIGNALLING_SEM);
  if (bRetstatus != PH_ERR_SUCCESS) {
    return (PH_ERR_RESOURCE_ERROR | PH_COMP_LN_LLCP);
  }
    ...
    uint8_t *pUri)
{
  ...
  return phlnLlcp_Sw_Int_Transport_Socket_Register(pSocket, eSocketType, bLsap, pUri,
...
)
{
  ...
  if ((pSocket->eSocketType == PHLN_LLCP_TRANSPORT_CLIENT_CONNECTIONORIENTED) &&
      (pDataParams->bCurrentClientCnt != 0U)) {
...
phlnLlcp_Sw_Transport_Socket_UnblockAll(phlnLlcp_Sw_DataParams_t *pDataParams)
{
  ...
  while (psSockets != NULL) {
    ...
    PH_CHECK_SUCCESS_FCT(bRetstatus, phOsal_SemPost(&psSockets->xRxSema.SemHandle,
            E_OS_SEM_OPT_NONE));
...
phlnLlcp_Sw_DeInit(phlnLlcp_Sw_DataParams_t *pDataParams)
{
  ...
  PH_CHECK_SUCCESS_FCT(wStatus, phOsal_EventClear(&pDataParams->LlcpEventObj.EventHandle,
          E_OS_EVENT_OPT_NONE, E_PH_OSAL_EVT_LLCP_ACTIVATED, NULL));
