    ...
    phlnLlcp_LMDataParams_t *pLMBytes)
{
  ...
  return phlnLlcp_Sw_Int_ParseGenBytes((uint8_t *)(pGenBytes + 3U), wLength - (uint16_t)3U,
          pLMBytes);
}
    ...
    uint8_t bIgnoreRsap)
{
  ...
  while ((pSockets != NULL) &&
      (((pSockets->bLsap != bLsap)) || ((bIgnoreRsap != (uint8_t)1U) && ((pSockets->bRsap != bRsap) &&
                  (pSockets->bRsap != 0x01U))))) {
  ...
  if (epType == PHLN_LLCP_PTYPE_AGF) {
    ...
    if ((bLsap != 0U) || (bRsap != 0U)) {
      ...
      * in AGF PDU and this will be handled by thread Processing the Message Queue. */
      ...
      PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_Sw_Int_HandleMsgQueue(gphlnLlcp_AGFBuffer, (uint16_t)3U,
...
phStatus_t phlnLlcp_Sw_Int_Send(phTools_Q_t *psMsgQueue, uint8_t *pbPerformRx)
{
  ...
  *pbPerformRx = (uint8_t) 1U;
  ...
  if (psMsgQueue->bLlcpData == PH_ON) {
    ...
    return phlnLlcp_MacTransmit(PH_TRANSMIT_DEFAULT, psMsgQueue->bLlcpBuf,
            (uint16_t)psMsgQueue->dwLength);
  } else {
    ...
    * bLsap & bRsap are zeros if it is internal. */
    if ((psSocket->bLsap == 0U) && (psSocket->bRsap == 0U)) {
      ...
      return phlnLlcp_MacTransmit(PH_TRANSMIT_DEFAULT, psMsgQueue->pbData,
              (uint16_t)psMsgQueue->dwLength);
    } else if (psSocket->eSocketType == PHLN_LLCP_TRANSPORT_CONNECTIONLESS) {
      if (psSocket->bState == PHLN_LLCP_SOCKET_SNL) {
        wStatus = phlnLlcp_Sw_Int_Pdu_SnlEx(psSocket, psMsgQueue->pbData, (uint16_t) psMsgQueue->dwLength,
                psMsgQueue->wFrameOpt);
        psMsgQueue->wFrameOpt = 0;
      } else {
        /* Send UI PDU. */
        wStatus = phlnLlcp_Sw_Int_Pdu_ConnectionLess_InfoEx(psSocket, psMsgQueue->pbData,
                (uint16_t)psMsgQueue->dwLength,
                psMsgQueue->wFrameOpt, pbPerformRx);

        if ((psMsgQueue->wFrameOpt == PH_TRANSMIT_DEFAULT) ||
            (psMsgQueue->wFrameOpt == PH_TRANSMIT_BUFFER_LAST)) {
      ...
      switch (psSocket->bState) {
        case PHLN_LLCP_SOCKET_INFO_SEND_EX :
          wStatus = phlnLlcp_Sw_Int_Pdu_InfoEx(psSocket, psMsgQueue->pbData, (uint16_t)psMsgQueue->dwLength,
                  psMsgQueue->wFrameOpt,
                  pbPerformRx);
          break;
        ...
        case PHLN_LLCP_SOCKET_CONN_PEND  :
          /* Send connect PDU */
          wStatus = phlnLlcp_Sw_Int_Pdu_CcOrC(psSocket, PHLN_LLCP_PTYPE_CONNECT, psMsgQueue->pbData,
                  (uint8_t)psMsgQueue->dwLength);
          break;
  ...
  if ((bLsap == 1U) && ((epType == PHLN_LLCP_PTYPE_CONNECT) || (epType == PHLN_LLCP_PTYPE_SNL))) {
    if ((pRxBuffer[2] == PHLN_LLCP_TLV_TYPE_SDRES) && (pDataParams->bSdpClientSAP != 0U)) {
      ...
      * and also check if SDREQ TLV is present in SNL PDU. */
      pUri = phlnLlcp_Sw_Int_GetUri(&pRxBuffer[2], (uint16_t)(dwLength - 2u), &bUriLen, NULL,
              &bSDREQPresent);
    ...
    psSocket  = phlnLlcp_Transport_Socket_Search(bLsap, bRsap, ((epType == PHLN_LLCP_PTYPE_CONNECT) ||
                (epType == PHLN_LLCP_PTYPE_CC)) ? (uint8_t)1U : (uint8_t)0U);
  }
  *ppsSocket = psSocket;
  ...
  * return SNL PDU with SDRES parameter with SAP address as Zero. */
  ...
  switch (epType) {
          ...
          if ((*ppsSocket)->fReady == (uint8_t)1U) {
            ...
            * Information Error and send FRMR PDU to Peer. */
            if ((dwLength - PHLN_LLCP_HEADER_SIZE) <= (*ppsSocket)->dwBufLen) {
              ...
              (void)memcpy((*ppsSocket)->pbRxBuffer, (uint8_t *)(pRxBuffer + PHLN_LLCP_HEADER_SIZE),
                  (*ppsSocket)->dwLength);
    ...
    case PHLN_LLCP_PTYPE_SNL:
      if ((*ppsSocket != NULL) && (pRxBuffer[2] == PHLN_LLCP_TLV_TYPE_SDRES) &&
          (pDataParams->bSdpClientSAP == (*ppsSocket)->bLsap)) {
        if ((dwLength - PHLN_LLCP_SNL_HEADER_SIZE) <= (*ppsSocket)->dwBufLen) {
          (*ppsSocket)->dwLength = dwLength - PHLN_LLCP_SNL_HEADER_SIZE;
          (void)memcpy((*ppsSocket)->pbRxBuffer, (uint8_t *)(pRxBuffer + PHLN_LLCP_SNL_HEADER_SIZE),
              (*ppsSocket)->dwLength);
        }
    ...
    case PHLN_LLCP_PTYPE_UI:
      ...
      if ((*ppsSocket)->fReady == (uint8_t)1U) {
        ...
        if ((dwLength - PHLN_LLCP_UI_HEADER_SIZE) <= (*ppsSocket)->dwBufLen) {
          (void)memcpy((*ppsSocket)->pbRxBuffer, (uint8_t *)(pRxBuffer + PHLN_LLCP_UI_HEADER_SIZE),
              dwLength);
  ...
  switch (epType) {
    ...
    case PHLN_LLCP_PTYPE_DISC:
      if (wProcessStatus == PH_ERR_SUCCESS) {
        if ((psSocket->bLsap == 0U) && (psSocket->bRsap == 0U)) {
          ...
          /* Send DM PDU. */
          PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_Sw_Int_Pdu_DiscOrDm(psSocket, PHLN_LLCP_PTYPE_DM,
    ...
    case PHLN_LLCP_PTYPE_INFO:
      switch (wProcessStatus) {
        ...
        case PH_ERR_LLCP_PDU_INFO_ERR:
          /* Send FRMR PDU. */
          wStatus = phlnLlcp_Sw_Int_Pdu_Frmr(psSocket, epType, bLsap, bRsap, bSeq, (uint8_t)wProcessStatus,
                  PH_OFF);
          break;
    ...
    case PHLN_LLCP_PTYPE_AGF:
      ...
      if (pRxData[2] == PH_ERR_LLCP_PDU_INVALID) {
        PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_Sw_Int_Pdu_Frmr(psSocket, epType, 0, 0, 0,
                PH_ERR_LLCP_PDU_INVALID, PH_ON));
    ...
    case PHLN_LLCP_PTYPE_RNR:
      if (wProcessStatus == PH_ERR_LLCP_PDU_RX_SEQ_ERR) {
        /* Send FRMR PDU. */
        wStatus = phlnLlcp_Sw_Int_Pdu_Frmr(psSocket, epType, bLsap, bRsap, bSeq,
                PH_ERR_LLCP_PDU_RX_SEQ_ERR, PH_OFF);
      }
      break;
    ...
    default:
      ...
      PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_Sw_Int_Pdu_Frmr(psSocket, epType, bLsap, bRsap, bSeq,
              PH_ERR_LLCP_PDU_INVALID, PH_OFF));
      break;
  }
  return wStatus;
}
    ...
    uint8_t bVersion)
{
  ...
  if ((wProcessStatus == PH_ERR_LLCP_SOCKET_NOT_REGISTERED) ||
      (wProcessStatus == PH_ERR_LLCP_BUSY)) {
    return wTxFlag;
  } else if ((wProcessStatus == PH_ERR_LLCP_PDU_RX_SEQ_ERR) ||
      (wProcessStatus == PH_ERR_LLCP_PDU_TX_SEQ_ERR) ||
    ...
    switch (epType) {
      ...
      case PHLN_LLCP_PTYPE_SNL:
        ...
        *  Also if SNL PDU with no SDREQ TLV is received then not need to send SNL PDU back. */
        if ((bVersion < PHLN_LLCP_SNL_SUPPORTED_VERSION_VALUE) ||
            (wProcessStatus == PH_ERR_LLCP_NO_SDREQ)) {
          wTxFlag = PH_OFF;
        }
        break;
