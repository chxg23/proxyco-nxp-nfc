    ...
    uint16_t *wSymmTime, uint16_t *wLtoTime)
{
  ...
  if (pDataParams->sRemoteLMParams.bLto == 0U) {
    ...
    *wLtoTime = ((uint16_t)pDataParams->sRemoteLMParams.bLto * 10U) + (((
                    uint16_t)pDataParams->sRemoteLMParams.bLto * PHLN_LLCP_LTO_DELTA_PERCENTAGE) / 10U) ;
  }
  ...
  if (gphlnLlcp_MacType == PHLN_LLCP_INITIATOR) {
    ...
    do {
      ...
      if (wStatus == PH_ERR_SUCCESS) {
        wStatus = phpalI18092mT_Receive(gphlnLlcp_MacDataParams, PH_RECEIVE_DEFAULT, &pRxBuff,
                &wRxBuffLen);
      }
