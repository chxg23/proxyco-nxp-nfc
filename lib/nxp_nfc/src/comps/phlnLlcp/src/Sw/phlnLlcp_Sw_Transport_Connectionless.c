...
)
{
  ...
  if ((pClientSocket == NULL) || (pTxBuffer == NULL)) {
    ...
    wStatus = phlnLlcp_Sw_Int_Socket_SendInt(pClientSocket, pTxBuffer, dwTxBufferSize, wFrameOpt,
    ...
    uint8_t *pbPerformRx)
{
  ...
  switch (wFrameOpt) {
    case PH_TRANSMIT_BUFFER_FIRST:
      /* Frame the UI PDU */
      PHLN_LLCP_PDU_FRAME_MANDATORY_HEADER(psSocket->bRsap, PHLN_LLCP_PTYPE_UI, psSocket->bLsap,
          baLlcpHeader);
