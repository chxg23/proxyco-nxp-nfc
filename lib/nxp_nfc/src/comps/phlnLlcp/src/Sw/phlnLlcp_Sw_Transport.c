    ...
    uint8_t bReason)
{
  ...
  wLength = phlnLlcp_Sw_Int_Pdu_FrameHeader(bPtype, psSocket->bRsap, psSocket->bLsap, 0, 0,
          (uint8_t *)pDiscPdu);
...
phStatus_t phlnLlcp_Sw_Int_Pdu_Snl(uint8_t bTid, uint8_t bSap)
{
  ...
  /* Frame the Disconnect pdu */
  wLength = phlnLlcp_Sw_Int_Pdu_FrameHeader(PHLN_LLCP_PTYPE_SNL, 0x01, 0x01, 0, 0,
          (uint8_t *)pSnlPdu);
    ...
    uint8_t bPtype)
{
  ...
  wLength = phlnLlcp_Sw_Int_Pdu_FrameHeader(bPtype, psSocket->bRsap, psSocket->bLsap,
          psSocket->sSeq.bRxState_Vr,
          0, pResPdu);
