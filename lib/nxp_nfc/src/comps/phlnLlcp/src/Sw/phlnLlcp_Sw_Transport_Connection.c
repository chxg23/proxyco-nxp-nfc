...
)
{
  ...
  return phlnLlcp_Sw_Int_Socket_SendInt(pClientSocket, NULL, 0, PH_TRANSMIT_DEFAULT,
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(bRetstatus, phOsal_SemPend(&pServerSocket->xSema.SemHandle,
          timePeriodToWait));
  ...
  if ((pConnServerSocket != NULL) && ((pServerSocket->wStatus & PH_ERR_MASK) == PH_ERR_SUCCESS)) {
    ...
    PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_Sw_Int_Transport_Socket_Register(pConnServerSocket,
            pServerSocket->eSocketType, pServerSocket->bLsap, pServerSocket->pUri,
            PHLN_LLCP_CONNECTIONSERVER_SOCKET));
...
)
{
  ...
  PH_UNUSED_VARIABLE(pDataParams);
  return phlnLlcp_Sw_Int_Socket_SendInt(pClientSocket, pTxBuffer, dwTxBufferSize, wFrameOpt,
...
)
{
  ...
  wStatus = phlnLlcp_Sw_Int_Socket_SendInt(pClientSocket, NULL, 0, PH_TRANSMIT_DEFAULT,
          PHLN_LLCP_SOCKET_DISC_PEND);

  if (((wStatus & PH_ERR_MASK) == PH_ERR_LLCP_DEACTIVATED) ||
      (pClientSocket->bState == PHLN_LLCP_SOCKET_DISC)) {
    ...
    uint16_t wNoURI)
{
  ...
  for (; bCount < wNoURI; bCount++) {
    ...
    if (bCount == (wNoURI - 1u)) {
      /* Send Last URI. */
      PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_MacTransmit(PH_TRANSMIT_BUFFER_LAST, &pbTxData[bOffset],
              bSDREQLen));
    } else {
      /* Send URI. */
      PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_MacTransmit(PH_TRANSMIT_BUFFER_CONT, &pbTxData[bOffset],
              bSDREQLen));
    }
    ...
    uint8_t *pbPerformRx)
{
  ...
  switch (wFrameOpt) {
    case PH_TRANSMIT_BUFFER_FIRST:
      /* frame I PDU */
      PHLN_LLCP_PDU_FRAME_HEADER(psSocket->bRsap, PHLN_LLCP_PTYPE_INFO, psSocket->bLsap,
          psSocket->sSeq.bSendState_Vs,
