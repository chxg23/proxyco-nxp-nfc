...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_Transport_Socket_Init(pDataParams->plnLlcpDataParams,
          pDataParams->psSocket,
          PHLN_LLCP_TRANSPORT_CLIENT_CONNECTIONORIENTED, pbSnepRxBuffer, dwSnepRxBufferSize));

  PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_Transport_Socket_Register(pDataParams->plnLlcpDataParams,
          pDataParams->psSocket,
  ...
  if (eServerType == phnpSnep_Default_Server) {
    PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_Transport_Socket_ConnectViaUri(
...
phStatus_t phnpSnep_Sw_ClientDeInit(phnpSnep_Sw_DataParams_t *pDataParams)
{
  ...
  wStatus = phlnLlcp_Transport_Socket_Disconnect(pDataParams->plnLlcpDataParams,
          pDataParams->psSocket);
...
)
{
  ...
  return phlnLlcp_Transport_Socket_WaitForConnection(pDataParams->plnLlcpDataParams,
...
)
{
  ...
  /* SNEP Version check. */
  if ((bRxVersion & PHNP_SNEP_VERSION_MAJOR_MASK) != (PHNP_SNEP_VER  &
          PHNP_SNEP_VERSION_MAJOR_MASK)) {
    ...
    PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_Transport_Socket_Disconnect(pDataParams->plnLlcpDataParams,
            pDataParams->psSocket));
  ...
  if ((bClientReq > (uint8_t)PHNP_SNEP_REQ_PUT) && (bClientReq < (uint8_t)PHNP_SNEP_REQ_REJECT)) {
    ...
    return (PH_ERR_SNEP_FAILED | PH_COMP_NP_SNEP);
  } else if ((bClientReq == (uint8_t)PHNP_SNEP_REQ_GET) &&
      (pDataParams->eServerType != phnpSnep_NonDefault_Server)) {
...
)
{
  ...
  if ((bClientRequest == (uint8_t)PHNP_SNEP_REQ_GET) &&
      (pDataParams->eServerType == phnpSnep_NonDefault_Server)) {
  ...
  do {
    ...
    switch (bClientRequest) {
      case PHNP_SNEP_REQ_PUT:
        ...
        dwMsgLen = ((uint32_t)pLocSocket->pbRxBuffer[2] << 24U) | ((uint32_t)pLocSocket->pbRxBuffer[3] <<
                16U) |
      ...
      case PHNP_SNEP_REQ_GET:
        ...
        dwMsgLen = ((uint32_t)pLocSocket->pbRxBuffer[6] << 24U) | ((uint32_t)pLocSocket->pbRxBuffer[7] <<
                16U) |
        ...
        if (dwMsgLen < dwRespDataLen) {
          ...
          PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_Transport_Socket_Send(pDataParams->plnLlcpDataParams,
                  pLocSocket,
                  baResLen, PHNP_SNEP_HEADER_SIZE, PH_TRANSMIT_BUFFER_FIRST));

          dwLength = (((dwRespDataLen + PHNP_SNEP_HEADER_SIZE) <= ((uint32_t)(pLocSocket->wRMiu)))) ?
              dwRespDataLen : ((uint32_t)pLocSocket->wRMiu - PHNP_SNEP_HEADER_SIZE);
...
)
{
  ...
  if ((pLocSocket->pbRxBuffer[0] == PHNP_SNEP_VER) &&
      (pLocSocket->pbRxBuffer[1] == (uint8_t)PHNP_SNEP_RES_EXCESS_DATA)
  ...
  if (pLocSocket->dwLength > PHNP_SNEP_HEADER_SIZE) {
    ...
    dwLength = ((uint32_t)pLocSocket->pbRxBuffer[2] << 24U) | ((uint32_t)pLocSocket->pbRxBuffer[3] <<
            16U) |
        ((uint32_t)pLocSocket->pbRxBuffer[4] << 8U) | pLocSocket->pbRxBuffer[5];
...
)
{
  ...
  return phlnLlcp_Transport_Socket_Send(pDataParams->plnLlcpDataParams, pLocSocket, baHeader,
          PHNP_SNEP_HEADER_SIZE, PH_TRANSMIT_DEFAULT);
}
