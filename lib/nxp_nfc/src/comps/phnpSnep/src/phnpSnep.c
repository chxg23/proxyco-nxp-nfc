...
)
{
  ...
  PH_LOG_HELPER_ADDPARAM_BUFFER(PH_LOG_LOGTYPE_DEBUG, pbSnepRxBuffer_log, pbSnepRxBuffer,
      dwSnepRxBufferSize);
  PH_LOG_HELPER_EXECUTE(PH_LOG_OPTION_CATEGORY_ENTER);
  PH_ASSERT_NULL(pDataParams);
  if (0U != (dwSnepRxBufferSize)) {
    PH_ASSERT_NULL(pbSnepRxBuffer);
  }
