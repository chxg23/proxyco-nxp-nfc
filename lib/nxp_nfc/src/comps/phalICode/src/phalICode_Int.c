    ...
    uint16_t wNumBlocks, uint8_t *pData, uint16_t *pDataLen)
{
  ...
  if (0U != (bEnableBuffering)) {
     ...
     * value.
     */
    wMaxNoBlocks = (uint8_t)((bOption != 0U) ? PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_SET :
            PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_NOT_SET);
    ...
    do {
      ...
      if ((bCmdCode == PHAL_ICODE_CMD_EXTENDED_READ_MULTIPLE_BLOCKS) ||
          (bCmdCode == PHAL_ICODE_CMD_EXTENDED_GET_MULTIPLE_BLOCK_SECURITY_STATUS) ||
      ...
      *pDataLen += wRespLen;
      ...
      switch (bCmdCode) {
        ...
        case PHAL_ICODE_CMD_EXTENDED_FAST_READ_MULTIPLE_BLOCKS:
          if ((!(bOption > 0U) && ((wNumBlocks * 4U) == *pDataLen)) || ((bOption > 0U) &&
                  (((wNumBlocks * 4U) + wNumBlocks) == *pDataLen))) {
            bAllBlocksRead = 1U;
          }
          break;
    ...
    uint8_t *pData, uint16_t *pDataLen)
{
  ...
  if ((((phpalSli15693_Sw_DataParams_t *) pPalSli15693DataParams)->bFlags &
          PHAL_ICODE_FLAG_DATA_RATE) > 0U) {
    if (((phpalSli15693_Sw_DataParams_t *) pPalSli15693DataParams)->bFlags &
    ...
    uint8_t *pData, uint16_t wDataLen)
{
  ...
  if (bEnableBuffering) {
     ...
     * value.
     */
    bMaxNoBlocks = (uint8_t)(bOption ? PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_SET :
            PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_NOT_SET);
