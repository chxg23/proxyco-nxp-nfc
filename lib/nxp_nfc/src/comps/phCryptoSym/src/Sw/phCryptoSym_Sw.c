...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phCryptoSym_Sw_GetConfig(pDataParams,
          PH_CRYPTOSYM_CONFIG_BLOCK_SIZE, &wBlockSize));
    ...
    switch ((uint8_t)(wOption)) {
      case PH_CRYPTOSYM_MAC_MODE_CMAC:
        ...
        if (pDataParams->bCMACSubKeysInitialized == 0x00U) {
          ...
          PH_CHECK_SUCCESS_FCT(statusTmp, phCryptoSym_Sw_CMAC_GenerateK1K2(pDataParams, pSubKey1,
                  pSubKey2));
...
#else
  /* satisfy compiler */
  if (pDataParams || wKeyNo || wKeyVersion || pDiversifiedKey || bLenDivInput || pDivInput ||
      wOption);
  return PH_ADD_COMPCODE_FIXED(PH_ERR_UNSUPPORTED_COMMAND, PH_COMP_CRYPTOSYM);
#endif  /* NXPBUILD__PH_KEYSTORE */
}
      ...
      switch (wKeyType) {
        ...
        case PH_CRYPTOSYM_KEY_TYPE_2K3DES:
          ...
          if (0U != (wOption & PH_CRYPTOSYM_DIV_OPTION_2K3DES_HALF)) {
            ...
            (void)memcpy(&pDiversifiedKey[PH_CRYPTOSYM_DES_KEY_SIZE], &pDiversifiedKey[0],
                PH_CRYPTOSYM_DES_KEY_SIZE);
          }
          ...
          PH_CHECK_SUCCESS_FCT(statusTmp, phCryptoSym_Sw_Des_EncodeVersion(pDiversifiedKey, wKeyVersion,
                  wKeyType, pDiversifiedKey));
