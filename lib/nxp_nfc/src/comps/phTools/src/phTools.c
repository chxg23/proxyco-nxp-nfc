...
#include <nxp_nfc/NxpNfcRdLib_Ver.h>
...
)
{
  ...
  /* Parameter check */
  if (((bOption != PH_TOOLS_PARITY_OPTION_EVEN) && (bOption != PH_TOOLS_PARITY_OPTION_ODD)) ||
      (bInBufferBits > 7U)) {
  ...
  for (; wByteIndexIn < wInBufferLength; ++wByteIndexIn) {
    ...
    if ((wByteIndexOut + 1U) < (*pOutBufferLength)) {
      ...
      if ((bInBufferBits == 0U) || ((wByteIndexIn + 1U) < wInBufferLength)) {
        pOutBuffer[wByteIndexOut + 1U] |= (uint8_t)(phTools_CalcParity(pInBuffer[wByteIndexIn],
                    bOption) << (7U - bBitPosition));
      }
    }
...
)
{
  ...
  *pOutBufferBits = (uint8_t)((8U - (((8U + ((*pOutBufferLength) % 8U)) - bInBufferBits) % 8U)) %
          8U);
  ...
  for (; wByteIndexOut < (*pOutBufferLength); ++wByteIndexOut, ++wByteIndexIn, --bBitPosition) {
    ...
    if ((wByteIndexIn + /* */ 1U) < wInBufferLength) {
      ...
      if ((*pOutBufferBits == 0U)
        ...
        if ((pInBuffer[wByteIndexIn + 1U] & (uint8_t)(1U << (7U - bBitPosition))) != (bParity <<
                (7U - bBitPosition))) {
...
static void phTools_UpdateCrc_B(uint8_t bCh, uint16_t *pLpwCrc)
{
  ...
  bCh = (bCh ^ (bCh << 4U));
  *pLpwCrc = (*pLpwCrc >> 8U) ^ ((uint16_t)bCh << 8U) ^ ((uint16_t)bCh << 3U) ^ ((
              uint16_t)bCh >> 4U);
}
    ...
    uint16_t *pVersionStringLen)
{
  ...
  *pPatch_Dev = NxpNfcRdLib_VER_DEV;
  ...
  wCopyLen = (uint16_t)(sizeof(szNxpNfcRdLib_VER_FILEDESCRIPTION) / sizeof(
              szNxpNfcRdLib_VER_FILEDESCRIPTION[0]));
