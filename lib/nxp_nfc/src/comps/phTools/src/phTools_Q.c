...
phStatus_t phTools_Q_Init(void)
{
  ...
  PH_CHECK_SUCCESS_FCT(bRetstatus, phOsal_SemCreate(&gphTools_Q_MsgSema.SemHandle,
          &gphTools_Q_MsgSema, E_OS_SEM_OPT_COUNTING_SEM));
  if (gphTools_Q_MsgSema.SemHandle == NULL) {
    return (PH_ERR_RESOURCE_ERROR | PH_COMP_TOOLS);
  }

  PH_CHECK_SUCCESS_FCT(bRetstatus, phOsal_MutexCreate(&gphTools_Q_Mutex.MutexHandle,
          &gphTools_Q_Mutex));
  if (gphTools_Q_Mutex.MutexHandle == NULL) {
    return (PH_ERR_RESOURCE_ERROR | PH_COMP_TOOLS);
  }

  PH_CHECK_SUCCESS_FCT(bRetstatus, phOsal_MutexCreate(&gphTools_Q_MsgMutex.MutexHandle,
          &gphTools_Q_MsgMutex));
  if (gphTools_Q_Mutex.MutexHandle == NULL) {
    return (PH_ERR_RESOURCE_ERROR | PH_COMP_TOOLS);
  }
    ...
    uint32_t dwBlockTime, uint8_t bPriority)
{
  ...
  if ((bPriority == PH_OFF) &&
      (bQCount >= (PH_TOOLS_Q_MAX_ELEMENTS - PH_TOOLS_Q_RESERVED_ELEMENTS)) &&
      (pMutex == gphTools_Q_Mutex.MutexHandle)) {
    return NULL;
  }
...
phStatus_t phTools_Q_Release(phTools_Q_t *psMsgQueue, uint32_t dwBlockTime)
{
  ...
  PH_CHECK_SUCCESS_FCT(bRetstatus, phOsal_MutexLock(&gphTools_Q_Mutex.MutexHandle,
          timePeriodToWait));
    ...
    uint16_t wFrameOpt)
{
  ...
  if ((wFrameOpt == PH_TRANSMIT_BUFFER_FIRST) || (wFrameOpt == PH_TRANSMIT_DEFAULT)) {
    bNoOfMsgs = 1;
    PH_CHECK_SUCCESS_FCT(bRetstatus, phOsal_MutexLock(&gphTools_Q_MsgMutex.MutexHandle,
            timePeriodToWait));
  } else {
    bNoOfMsgs++;
  }
  ...
  if ((wFrameOpt == PH_TRANSMIT_BUFFER_LAST) || (wFrameOpt == PH_TRANSMIT_DEFAULT)) {
    ...
    for (i = 0; i < bNoOfMsgs; i++) {
      PH_CHECK_SUCCESS_FCT(bRetstatus, phOsal_SemPost(&gphTools_Q_MsgSema.SemHandle,
              E_OS_SEM_OPT_NONE));
    }
