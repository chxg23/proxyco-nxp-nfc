...
)
{
  ...
  wCommandLen = (bBitCount == 0U) ? (((uint16_t)bMaskBitLength) >> 3U) : ((((
                      uint16_t)bMaskBitLength) >> 3U) + 1U);
  ...
  wCommandBitLen = (bBitCount == 0U) ? (wCommandLen << 3U) : (((uint16_t)(
                  wCommandLen - 1u) << 3U) + bBitCount);
  ...
  /* Parse response */
  PH_CHECK_SUCCESS_FCT(statusTmp, phpalEpcUid_Sw_ParseResponse(pDataParams, pResponse,
          wResponseLen));
...
)
{
  ...
  if (pDataParams->bLabelType == PHPAL_EPCUID_LABEL_EPC) {
    /* Copy EPC */
    (void)memcpy(&pDataParams->LabelInfo.bEpc[pDataParams->bMaskBitLength >> 3U], pRxBuffer,
        (((uint32_t)(wRxLength)) - 2u));
    ...
    /* Copy IDD */
    (void)memcpy(&pDataParams->LabelInfo.bIdd[pDataParams->bMaskBitLength >> 3U], pRxBuffer,
        (((uint32_t)(wRxLength)) - 2u));
  ...
  /* Check CRC */
  if (((uint8_t)(wCrc16 >> 8U) != pRxBuffer[wRxLength - 2u]) ||
      ((uint8_t)(wCrc16) != pRxBuffer[wRxLength - 1u])) {
    pDataParams->bUidValid = PH_OFF;
