      ...
      case PHPAL_I14443P4MC_STATE_TX_R:
        ...
        for (bIndex = 0; bIndex < 2 ; bIndex ++) {
          PH_CHECK_SUCCESS_FCT(status, phhalHw_SetConfig(pDataParams->pHalDataParams,
                  PHHAL_HW_CONFIG_TXBUFFER_OFFSET, (uint16_t)bIndex));

          /* Backup header byte  */
          PH_CHECK_SUCCESS_FCT(status, phhalHw_GetConfig(pDataParams->pHalDataParams,
        ...
        for (bIndex = 0; bIndex < 2 ; bIndex ++) {
          ...
          /* Restore the TxBuffer  */
          PH_CHECK_SUCCESS_FCT(status, phhalHw_SetConfig(pDataParams->pHalDataParams,
                  PHHAL_HW_CONFIG_TXBUFFER, (uint16_t) bBackUpTxBuff[bIndex]));
        }
        break;
...
)
{
  ...
  do {
    ...
    if (0U == (pDataParams->bDisableWtx)) {
      /* XXX There is no timer here, so no need to stop one */
    }
...
)
{
  switch (wConfig) {
    ...
    case PHPAL_I14443P4MC_CONFIG_WTX_VALUE:
      if ((wValue < PHPAL_I14443P4MC_SW_MIN_WTX_VALUE) ||
          (wValue > PHPAL_I14443P4MC_SW_MAX_WTX_VALUE)) {
        return (PH_ERR_INVALID_PARAMETER | PH_COMP_PAL_I14443P4MC);
      }
