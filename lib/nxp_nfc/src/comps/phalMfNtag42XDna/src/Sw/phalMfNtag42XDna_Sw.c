    ...
    uint8_t *pPcdCapsOut, uint8_t *pPdCapsOut)
{
  ...
  if ((bAuthOption == PHAL_MFNTAG42XDNA_AUTHFIRST_NON_LRP) ||
      (bAuthOption == PHAL_MFNTAG42XDNA_AUTHNONFIRST_NON_LRP)) {
  ...
  * using the below flag variable */
  if (bOption == PHAL_MFNTAG42XDNA_SET_CONFIG_OPTION5 && (pData[4] == 0x02)) {
    ...
    pDataParams->bCryptoMethod =
        PH_CRYPTOSYM_CONFIG_LRP; /*  This is just a value. Same to be checked inside phalMfNtag42XDna_Sw_Int_Write_Enc API */
    bEnableLrp = 0x01;
  }
    ...
    uint8_t  *pVerInfo, uint8_t *pVerLength)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phalMfNtag42XDna_Sw_Int_ReadData_Plain(
          ...
          PHAL_MFNTAG42XDNA_COMMUNICATION_MACD : PHAL_MFNTAG42XDNA_COMMUNICATION_PLAIN,
          bCmdBuff,
          1,
          &pRecv,
          &wRxlen
   ...
   */
  if (((wRxlen != PHAL_MFNTAG42XDNA_DEF_VERSION_LENGTH) &&
          (wRxlen != PHAL_MFNTAG42XDNA_VERSION_LENGTH_WITH_FABKEY_ID)) &&
  ...
  *pVerLength = (uint8_t)(wRxlen);
  ...
  phalMfNtag42XDna_Sw_SetConfig((phalMfNtag42XDna_Sw_DataParams_t *)pDataParams,
      PHAL_MFNTAG42XDNA_ADDITIONAL_INFO, wRxlen);
    ...
    uint8_t  *pUid, uint8_t *pUidLength)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phalMfNtag42XDna_SetConfig(pDataParams,
          PHAL_MFNTAG42XDNA_ADDITIONAL_INFO, (uint16_t)(*pUidLength)));
  ...
  */
  ...
  if (memcmp(pDataParams->pAid, bAppId, 3) == 0x00) {
    ...
    if ((bKeyNo > PHAL_MFNTAG42XDNA_ORIGINALITY_KEY_LAST) ||
        (bKeyNo < PHAL_MFNTAG42XDNA_ORIGINALITY_KEY_FIRST)) {
      return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_AL_MFNTAG42XDNA);
    }
  } else {
    /* At App level, 0,1,2,3,4 are valid ones. */
...
phStatus_t phalMfNtag42XDna_Sw_ChangeFileSettingsSDM(phalMfNtag42XDna_Sw_DataParams_t
    *pDataParams, uint8_t bOption, uint8_t bFileNo,
    ...
    uint8_t *pPICCDataOffset, uint8_t *pTTPermStatusOffset, uint8_t *pSDMMACInputOffset,
    uint8_t *pSDMENCOffset, uint8_t *pSDMENCLen,
  ...
  if (pDataParams->bAuthMode == PHAL_MFNTAG42XDNA_NOT_AUTHENTICATED) {
    ...
    if ((bCommOption & 0xF0U) == PHAL_MFNTAG42XDNA_COMMUNICATION_MACD) {
      pDataParams->dwPayLoadLen = pDataParams->dwPayLoadLen + ((pDataParams->bAuthMode ==
                  PHAL_MFNTAG42XDNA_AUTHENTICATE) ? 0x04 : 0x08);
    }
    if ((bCommOption & 0xF0U) == PHAL_MFNTAG42XDNA_COMMUNICATION_ENC) {
      if (pDataParams->bAuthMode == PHAL_MFNTAG42XDNA_AUTHENTICATEEV2) {
        ...
        pDataParams->dwPayLoadLen  = pDataParams->dwPayLoadLen  + ((pDataParams->bAuthMode ==
                    PHAL_MFNTAG42XDNA_AUTHENTICATE) ? 0x02 : 0x04);
        bIvLen = (pDataParams->bAuthMode == PHAL_MFNTAG42XDNA_AUTHENTICATEAES) ?
