  ...
  for (bIndex = 0; bIndex < PHAC_DISCLOOP_ACT_POLL_MAX_TECHS_SUPPORTED; bIndex++) {
    switch (pDataParams->bActPollTechCfg & (PH_ON << bIndex)) {
      ...
      case PHAC_DISCLOOP_ACT_POS_BIT_MASK_424:
        ...
        PH_CHECK_SUCCESS_FCT(status, phhalHw_Wait(pDataParams->pHalDataParams, PHHAL_HW_TIME_MICROSECONDS,
                pDataParams->wActPollGTimeUs));
        ...
        /* Form NFCID3 */
        (void)memcpy(aNfcId3, pDataParams->sTypeFTargetInfo.aTypeFTag[0].aIDmPMm,
...
)
{
  ...
  if (pDataParams->bPollState == PHAC_DISCLOOP_POLL_STATE_COLLISION_RESOLUTION) {
    ...
    if ((status & PH_ERR_MASK) == PHAC_DISCLOOP_DEVICE_RESOLVED) {
      /* Activation */
      status = phacDiscLoop_Sw_Int_ActivateDevice(pDataParams, (bTechType - (uint8_t)1U),
              (uint8_t)0x00U);
    }
    /* Return if status is aborted. */
    PH_CHECK_ABORT(status);
...
)
{
  ...
  for (bTechLoopIndex = 0; bTechLoopIndex < PHAC_DISCLOOP_PASS_POLL_MAX_TECHS_SUPPORTED;
      bTechLoopIndex++) {
...
#ifdef NXPBUILD__PHAC_DISCLOOP_SW_ECP
    /* Is Polling for Type VAS is enable*/
    if ((0U != ((bDetectConfig & PHAC_DISCLOOP_POS_BIT_MASK_VAS))) && (bStopTechDetection == 0U) &&
        (bNumOfTechsFound == 0U)) {
      if (0U == (pDataParams->bDetectedTechs & PHAC_DISCLOOP_POS_BIT_MASK_VAS)) {
        /* Apply Guard time. */
        PH_CHECK_SUCCESS_FCT(status, phhalHw_SetConfig(pDataParams->pHalDataParams,
                PHHAL_HW_CONFIG_POLL_GUARD_TIME_US,
        ...
        PH_CHECK_SUCCESS_FCT(status, phhalHw_ApplyProtocolSettings(
                pDataParams->pHalDataParams,
                (uint8_t)(gPasTechTypeMapTable[6] & 0xFFU)));

        /* Perform Tech detection. */
        status = pfDetTechs[6](pDataParams);
        if ((status & PH_ERR_MASK) == PHAC_DISCLOOP_TECH_DETECTED) {
          /*Set the corresponding detected bit. */
          pDataParams->bDetectedTechs |= PHAC_DISCLOOP_POS_BIT_MASK_VAS;
          bNumOfTechsFound++;
        } else {
          PH_CHECK_ABORT(status);
        }
      ...
      PH_CHECK_SUCCESS_FCT(statustmp, phhalHw_FieldOff(pDataParams->pHalDataParams));

      PH_CHECK_SUCCESS_FCT(statustmp, phhalHw_Wait(
              pDataParams->pHalDataParams,
              PHHAL_HW_TIME_MICROSECONDS,
              PH_NXPNFCRDLIB_CONFIG_EMVCO_OTHER_TECH_POLLING_FIELD_OFF_DELAY_US)
      );

      PH_CHECK_SUCCESS_FCT(statustmp, phhalHw_FieldOn(pDataParams->pHalDataParams));
    }
#endif /* NXPBUILD__PHAC_DISCLOOP_SW_ECP */
