...
)
{
    ...
    while (wIndex < wRxBufferLen) {
      if (pRxBuffer[wIndex] == 0U) {
        ...
        if (pRxBuffer[wIndex + 1U] > (bPcLength + bPacketCrcLen)) {
          ...
          if ((pRxBuffer[wIndex + 1U] - (bPcLength + bPacketCrcLen)) <= (uint8_t)sizeof(
            ...
            pDataParams->sI18000p3m3TargetInfo.aI18000p3m3[pDataParams->bNumOfCards].wUiiLength =
                (uint16_t)(((uint16_t)pRxBuffer[wIndex + 1U] - ((uint16_t)bPcLength +
                            (uint16_t)bPacketCrcLen)) * 8U);
          } else {
  ...
  } while (((bMoreCardsPresent != PH_OFF) && (bQ <= 0x0FU)) ||
...
)
{
  ...
  if ((bI18000p3m3TagIdx >= pDataParams->sI18000p3m3TargetInfo.bTotalTagsFound) ||
      (bI18000p3m3TagIdx >= PHAC_DISCLOOP_CFG_MAX_VICINITY_CARDS_SUPPORTED)) {
  ...
  if (pDataParams->sI18000p3m3TargetInfo.aI18000p3m3[bI18000p3m3TagIdx].wUiiLength > 255U) {
    ...
    bMaskLength = (uint8_t)
        pDataParams->sI18000p3m3TargetInfo.aI18000p3m3[bI18000p3m3TagIdx].wUiiLength;
  }
