...
)
{
  ...
  if ((status & PH_ERR_MASK) == PH_ERR_IO_TIMEOUT) {
    ...
    if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS) {
      ...
      do {
        while (pDataParams->sTypeVTargetInfo.bTotalTagsFound <
              ...
              for (bSlotCount = bCount[bLevel]; bSlotCount < 16U; bSlotCount++) {
                if (0U != (wCollisionLogReg[bLevel] & ((uint16_t)1U << bSlotCount))) {
                  ...
                  /* Partial byte is valid */
                    ...
                    aMaskBuffer[bMaskBitLength >> (uint8_t)3U] &= (uint8_t)((uint8_t)0xFF >> ((
                                    uint8_t)8U  - bUidLengthDummy));
...
)
{
  ...
  if ((bTypeVTagIdx >= pDataParams->sTypeVTargetInfo.bTotalTagsFound) ||
      (bTypeVTagIdx >= PHAC_DISCLOOP_CFG_MAX_VICINITY_CARDS_SUPPORTED)) {
    /* Out of range */
