...
)
{
  ...
  phStatus_t PH_MEMLOC_REM wStatustmp;
  ...
  if (0u != (phacDiscLoop_Sw_Int_IsValidPollStatus(wStatus))) {
    ...
    if ((pDataParams->bVASPollMode == PHAC_DISCLOOP_VAS_IN_COMPATIBILITY_MODE) &&
      ...
      PH_CHECK_ABORT_FCT(wStatustmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
              PHHAL_HW_CONFIG_TIMEOUT_VALUE_US,
              1U));

      /* Send VAS command to mimic the behavior of PN7150. */
      (void)phpalI14443p3a_VASUpA(
          pDataParams->pPal1443p3aDataParams,
  ...
  if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS) {
    ...
    if (pDataParams->sTypeATargetInfo.aTypeA_I3P3[0].aAtqa[0] == 0x00U) {
      ...
      PH_CHECK_SUCCESS_FCT(status, phhalHw_SetConfig(pDataParams->pHalDataParams,
              PHHAL_HW_CONFIG_JEWEL_MODE, PH_ON));
  ...
  while (bDeviceCount <= pDataParams->baPasConDevLim[PHAC_DISCLOOP_TECH_TYPE_A]) {
    if (bTypeATagIdx <= pDataParams->baPasConDevLim[PHAC_DISCLOOP_TECH_TYPE_A]) {
        ...
        while (bTypeANvbUid != 0x40U) {
          ...
          if ((status & PH_ERR_MASK) == PH_ERR_COLLISION_ERROR) {
            ...
            if ((bTypeANvbUid & ((uint8_t)0x07U)) < (uint8_t)7U) {
              ...
              aTypeAUid[(uint8_t)((bTypeANvbUid & 0xF0U) >> 4U)] |= (uint8_t)(0x01 << ((bTypeANvbUid & ((
                                      uint8_t)0x07U)) - 1U));
            } else {
      ...
      if (status == PH_ERR_SUCCESS) {
        ...
        /*Symbol 20*/
        if ((0U != ((pDataParams->bCollPend & PHAC_DISCLOOP_POS_BIT_MASK_A))) &&
            (bTypeATagIdx < pDataParams->baPasConDevLim[PHAC_DISCLOOP_TECH_TYPE_A])) {
          /* Symbol 21 */
