...
phStatus_t phNfcLib_Mifare_Transmit(uint8_t *pTxBuffer, uint16_t wTxLength)
{
  ...
  /* Disable RxCrc */
  (void) phhalHw_SetConfig(gphNfcLib_Params.sDiscLoop.pHalDataParams, PHHAL_HW_CONFIG_RXCRC,
      PH_OFF);
  ...
  if ((wStatus & PH_ERR_MASK) == PH_ERR_SUCCESS_INCOMPLETE_BYTE) {
    /* Retrieve bitcount */
    PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_GetConfig(gphNfcLib_Params.sDiscLoop.pHalDataParams,
            PHHAL_HW_CONFIG_RXLASTBITS, &wValidBits));

    /* Check for protocol error and check for ack*/
    if ((wValidBits != 4U) || (gphNfcLib_State.wRxLength != 1U) ||
        (( *&gphNfcLib_State.pRxBuffer)[0] != 0x0AU)) {
