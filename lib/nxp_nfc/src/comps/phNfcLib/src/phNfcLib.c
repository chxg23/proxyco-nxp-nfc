...
static phStatus_t phNfcLib_Configure_DiscLoop_EMVCo(uint16_t wTechnologyMask)
{
  ...
  if (wTechnologyMask != (PH_NFCLIB_TECHNOLOGY_INITIATOR_ISO_14443_A |
...
static phStatus_t phNfcLib_Activate_EMVCo_Profile(uint16_t wTechnologyMask)
{
  ...
  gphNfcLib_Params.sDiscLoop.sTypeATargetInfo.sTypeA_I3P4.pAts =
      gphNfcLib_State.pPeerInfo->uRi.uInitiator.tIso14443_4.aAts;

  do {
    /* Start Polling, Function will return once card is activated or any other error has occurred */
    wtmpStatus = phacDiscLoop_Run(&gphNfcLib_Params.sDiscLoop,
            (uint8_t)PHAC_DISCLOOP_ENTRY_POINT_POLL);
    ...
    if (((wtmpStatus & PH_ERR_MASK) == PHAC_DISCLOOP_NO_TECH_DETECTED) ||
      ...
      if ((wtmpStatus & PH_ERR_MASK) == PHAC_DISCLOOP_MERGED_SEL_RES_FOUND) {
        ...
        if (PHAC_DISCLOOP_CHECK_ANDMASK(wTagsDetected, PHAC_DISCLOOP_POS_BIT_MASK_A)) {
          ...
          wtmpStatus = phacDiscLoop_GetConfig(&gphNfcLib_Params.sDiscLoop,
                  PHAC_DISCLOOP_CONFIG_TYPEA_P2P_ATR_RES_LEN, &wGtLength);
          ...
          /* Get the ATR_RES Length. */
          wtmpStatus = phacDiscLoop_GetConfig(&gphNfcLib_Params.sDiscLoop,
                  PHAC_DISCLOOP_CONFIG_TYPEF_P2P_ATR_RES_LEN, &wGtLength);
        } else {
...
#ifdef NXPBUILD__PHPAL_I14443P3A_SW
      if (PHAC_DISCLOOP_CHECK_ANDMASK(wTagsDetected, PHAC_DISCLOOP_POS_BIT_MASK_A)) {
        gphNfcLib_State.pPeerInfo->uTi.uInitiator.tIso14443_3a.bUidLength =
            gphNfcLib_Params.sDiscLoop.sTypeATargetInfo.aTypeA_I3P3[0].bUidSize;
        (void)memcpy(&gphNfcLib_State.pPeerInfo->uTi.uInitiator.tIso14443_3a.aUid[0],
            &gphNfcLib_Params.sDiscLoop.sTypeATargetInfo.aTypeA_I3P3[0].aUid[0],
            gphNfcLib_Params.sDiscLoop.sTypeATargetInfo.aTypeA_I3P3[0].bUidSize);

        sak_atqa = (uint32_t)((((uint32_t)(
                            gphNfcLib_Params.sDiscLoop.sTypeATargetInfo.aTypeA_I3P3[0].aSak)) << 16U) |
      ...
      if (PHAC_DISCLOOP_CHECK_ANDMASK(wTagsDetected, PHAC_DISCLOOP_POS_BIT_MASK_V)) {
        ...
        gphNfcLib_State.pPeerInfo->uTi.uInitiator.tIso15693.TagIndex[0].pUid =
            &gphNfcLib_Params.sDiscLoop.sTypeVTargetInfo.aTypeV[0].aUid[0];
      ...
      if (PHAC_DISCLOOP_CHECK_ANDMASK(wTagsDetected, PHAC_DISCLOOP_POS_BIT_MASK_18000P3M3)) {
        /* TBD */
        gphNfcLib_State.pPeerInfo->uTi.uInitiator.tIso18000_3_3.TagIndex[0].pUii =
            &gphNfcLib_Params.sDiscLoop.sI18000p3m3TargetInfo.aI18000p3m3[0].aUii[0];
        gphNfcLib_State.pPeerInfo->uTi.uInitiator.tIso18000_3_3.TagIndex[0].wUiiLength =
            gphNfcLib_Params.sDiscLoop.sI18000p3m3TargetInfo.aI18000p3m3[0].wUiiLength;
        gphNfcLib_State.pPeerInfo->dwActivatedType = E_PH_NFCLIB_ISO18000p3m3;
        gphNfcLib_State.wActivatedUIDLength =
            gphNfcLib_Params.sDiscLoop.sI18000p3m3TargetInfo.aI18000p3m3[0].wUiiLength;
        gphNfcLib_State.pActivatedUid =
            &gphNfcLib_Params.sDiscLoop.sI18000p3m3TargetInfo.aI18000p3m3[0].aUii[0];
        break;
      }
...
#ifdef NXPBUILD__PH_NFCLIB_ISO_15693
      if (PHAC_DISCLOOP_CHECK_ANDMASK(wTagsDetected, PHAC_DISCLOOP_POS_BIT_MASK_V)) {
        gphNfcLib_State.pPeerInfo->uTi.uInitiator.tIso15693.bTotalTagsFound =
            gphNfcLib_Params.sDiscLoop.sTypeVTargetInfo.bTotalTagsFound;
        for (bIndex = 0; bIndex < gphNfcLib_Params.sDiscLoop.sTypeVTargetInfo.bTotalTagsFound; bIndex++) {
          gphNfcLib_State.pPeerInfo->uTi.uInitiator.tIso15693.TagIndex[bIndex].pUid =
              &gphNfcLib_Params.sDiscLoop.sTypeVTargetInfo.aTypeV[bIndex].aUid[0];
        }
        gphNfcLib_State.pPeerInfo->dwActivatedType = E_PH_NFCLIB_ISO15693;
        break;
      }
...
#ifdef NXPBUILD__PH_NFCLIB_ISO_18000
      if (PHAC_DISCLOOP_CHECK_ANDMASK(wTagsDetected, PHAC_DISCLOOP_POS_BIT_MASK_18000P3M3)) {
        gphNfcLib_State.pPeerInfo->uTi.uInitiator.tIso18000_3_3.bTotalTagsFound =
            gphNfcLib_Params.sDiscLoop.sI18000p3m3TargetInfo.bTotalTagsFound;
        for (bIndex = 0; bIndex < gphNfcLib_Params.sDiscLoop.sI18000p3m3TargetInfo.bTotalTagsFound;
            bIndex++) {
          gphNfcLib_State.pPeerInfo->uTi.uInitiator.tIso18000_3_3.TagIndex[bIndex].wUiiLength =
              gphNfcLib_Params.sDiscLoop.sI18000p3m3TargetInfo.aI18000p3m3[bIndex].wUiiLength;
          gphNfcLib_State.pPeerInfo->uTi.uInitiator.tIso18000_3_3.TagIndex[bIndex].pUii =
              &gphNfcLib_Params.sDiscLoop.sI18000p3m3TargetInfo.aI18000p3m3[bIndex].aUii[0];
        }
        gphNfcLib_State.pPeerInfo->dwActivatedType = E_PH_NFCLIB_ISO18000p3m3;
        break;
      }
...
static phStatus_t phNfcLib_EMVCo_ConfigureRemoval_Cycle(void)
{
  ...
  if (PHAC_DISCLOOP_CHECK_ANDMASK(gphNfcLib_Params.sDiscLoop.bDetectedTechs,
    ...
    PH_CHECK_SUCCESS_FCT(wStatus, phhalHw_SetConfig(&gphNfcLib_Params.sHal, PHHAL_HW_CONFIG_RFCA,
            PH_OFF));
      ...
      if ((gphNfcLib_State.pRxBuffer != NULL) && (gphNfcLib_State.wRxLength > 0U)) {
        if (gphNfcLib_State.wRxLength <= *pNumberOfBytes) {
          ...
          *pNumberOfBytes = gphNfcLib_State.wRxLength;
          (void)memcpy((uint8_t *)pRxBuffer, &gphNfcLib_State.pRxBuffer[gphNfcLib_State.wRxDataPoin],
              *pNumberOfBytes);
