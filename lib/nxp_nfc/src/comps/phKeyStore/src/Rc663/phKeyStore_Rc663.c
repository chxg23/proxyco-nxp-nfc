...
#include "nxp_nfc/phKeyStore_Rc663.h"
...
)
{
  ...
  for (i = 0; i < wNoOfKeys; ++i) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phKeyStore_Rc663_SetKeyAtPos(pDataParams, wKeyNo,  i, wNewKeyType,
            &pNewKeys[i * PH_KEYSTORE_KEY_TYPE_MIFARE_SIZE], wKeyVersion));
  }
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams->pHalDataParams,
          PH_KEYSTORE_RC663_REG_FIFOCONTROL, &bRegValue));

  /* Set FlushFifo bit */
  bRegValue |= PH_KEYSTORE_RC663_BIT_FLUSHFIFO;

  /* Write to FIFO Control register */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams->pHalDataParams,
          PH_KEYSTORE_RC663_REG_FIFOCONTROL, bRegValue));

  /* Write the Key number */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams->pHalDataParams,
          PH_KEYSTORE_RC663_REG_FIFODATA, bKeyNumber));

  /* Write each single key byte */
  for (wIndex = 0; wIndex < (bNumKeys * PH_KEYSTORE_KEY_TYPE_MIFARE_SIZE); ++wIndex) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams->pHalDataParams,
            PH_KEYSTORE_RC663_REG_FIFODATA, pKey[wIndex]));
  }

  /* start the command */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(pDataParams->pHalDataParams,
          PH_KEYSTORE_RC663_REG_COMMAND, PH_KEYSTORE_RC663_CMD_STOREKEYE2));

  /* wait for the EE programming to be finished */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Wait(pDataParams->pHalDataParams,
          PHHAL_HW_TIME_MILLISECONDS, PH_KEYSTORE_RC663_EEP_WR_TO_MS * bNumKeys));

  /* take a look  at the IDLE IRQ */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams->pHalDataParams,
          PH_KEYSTORE_RC663_REG_IRQ0, &bRegValue));
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_ReadRegister(pDataParams->pHalDataParams,
          PHHAL_HW_RC663_REG_ERROR, &bRegValue));
