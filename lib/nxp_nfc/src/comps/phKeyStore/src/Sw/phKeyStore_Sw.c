...
)
{
  ...
  for (i = 0; i < pDataParams->wNoOfVersions; ++i) {
    PH_CHECK_SUCCESS_FCT(statusTmp, phKeyStore_Sw_GetKeyValuePtrPos(pDataParams, wKeyNo, i,
            &pKeyVersion));
...
)
{
  phStatus_t statusTmp;
  phKeyStore_Sw_KeyVersionPair_t *pKeyVersion = NULL;
  PH_CHECK_SUCCESS_FCT(statusTmp, phKeyStore_Sw_GetKeyValuePtrVersion(pDataParams, wKeyNo,
          wKeyVersion, &pKeyVersion));
...
)
{
  ...
  phStatus_t statusTmp;
  phKeyStore_Sw_KeyVersionPair_t *pKeyVersion = NULL;
  PH_CHECK_SUCCESS_FCT(statusTmp, phKeyStore_Sw_GetKeyValuePtrPos(pDataParams, wKeyNo, wPos,
          &pKeyVersion));
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phKeyStore_Sw_CheckUpdateKUC(pDataParams,
          pDataParams->pKeyEntries[wKeyNo].wRefNoKUC));
...
)
{
  if (wKeyUsageCtrNumber != PH_KEYSTORE_INVALID_ID) {
    ...
    if (pDataParams->pKUCEntries[wKeyUsageCtrNumber].dwCurVal >=
        pDataParams->pKUCEntries[wKeyUsageCtrNumber].dwLimit) {
