...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phpalI18092mPI_Sw_WriteFrameHeader(pDataParams,
          PH_EXCHANGE_BUFFER_FIRST, PHPAL_I18092MPI_SW_CMD_DEP_REQ, bDataLength));
...
)
{
  ...
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
          PHHAL_HW_CONFIG_TXBUFFER_OFFSET, (uint16_t)bLengthPos));

  /* Write new length */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
          PHHAL_HW_CONFIG_TXBUFFER, (uint16_t)bDataLength));

  /* Prepare access to PFB position */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
          PHHAL_HW_CONFIG_TXBUFFER_OFFSET, (uint16_t)(((uint16_t)bLengthPos) + 3U)));

  /* Read current PFB */
  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_GetConfig(pDataParams->pHalDataParams,
          PHHAL_HW_CONFIG_TXBUFFER, &wPfb));
    ...
    */
    ...
    if ((bResponseReceived == 1U)
      ...
      for (bCounter = 0; bCounter < PHPAL_I18092MPI_SW_MAX_HEADER_LENGTH; ++bCounter) {
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
                PHHAL_HW_CONFIG_TXBUFFER_OFFSET, (uint16_t)bCounter));

        /* Restore header byte */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams,
                PHHAL_HW_CONFIG_TXBUFFER, (uint16_t)abHeaderBackup[bCounter])); /* PRQA S 3321 */
      }
    ...
    if (/*(bSendNack != PH_OFF) || */(bSendAtn != PH_OFF)) {
      ...
      if ((0U != ((pDataParams->bStateNow & PHPAL_I18092MPI_SW_STATE_RETRANSMIT_BIT)))
        ...
        for (bCounter = 0; bCounter < PHPAL_I18092MPI_SW_MAX_HEADER_LENGTH; ++bCounter) {
          ...
          /* Backup header byte  */
          PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_GetConfig(pDataParams->pHalDataParams,
                  PHHAL_HW_CONFIG_TXBUFFER, &wConfig));
...
)
{
  ...
  } while ((0U != (wRetransmit)) || (0U != (bSendNack)) || (0U != (bSendAtn)) ||
      (0U != (bResPending)));
