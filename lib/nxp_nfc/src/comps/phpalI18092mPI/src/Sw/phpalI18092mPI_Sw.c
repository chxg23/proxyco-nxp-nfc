...
)
{
  ...
  do {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phpalI18092mPI_Sw_WriteFrameHeader(pDataParams,
            PH_EXCHANGE_BUFFER_FIRST, PHPAL_I18092MPI_SW_CMD_ATR_REQ, (bCmdLen + bGiLength)));
    ...
    if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS) {
      ...
      if (0U != (pFrame[PHPAL_I18092MPI_SW_ATR_RES_PPT_POS] &
      ...
      *pAtrResLength = (uint8_t)wFrameLength;
      ...
      (void)memcpy(pDataParams->aNfcid3i, &pAtrRes[PHPAL_I18092MPI_SW_ATR_RES_NFCID3_POS],
...
)
{
  ...
  do {
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phpalI18092mPI_Sw_WriteFrameHeader(pDataParams,
            PH_EXCHANGE_BUFFER_FIRST, PHPAL_I18092MPI_SW_CMD_PSL_REQ, 3));
    ...
    if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS) {
      ...
      PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_GetConfig(pDataParams->pHalDataParams,
              PHHAL_HW_CONFIG_ACTIVEMODE, &wActiveMode));
      if (wActiveMode == PH_ON) {
...
)
{
  ...
  do {
    ...
    /* Write frame header and update length field. */
    PH_CHECK_SUCCESS_FCT(statusTmp, phpalI18092mPI_Sw_WriteFrameHeader(pDataParams,
            PH_EXCHANGE_BUFFER_FIRST, bDeselectCommand, bCmdLen));
...
)
{
  ...
  wMaxFrameSize = (wTxBufferSize < aI18092_FsTable[pDataParams->bLrt]) ? wTxBufferSize :
      aI18092_FsTable[pDataParams->bLrt];
  ...
  if (((wOption & PH_EXCHANGE_MODE_MASK) == PH_EXCHANGE_RXCHAINING) ||
    ...
    pDataParams->bStateNow = PHPAL_I18092MPI_SW_STATE_ACK_PDU_TX |
        PHPAL_I18092MPI_SW_STATE_CHAINING_BIT;
  }
  /* PDU transmission */
  else {
    /* Reset to default state if not in Tx-Mode */
    if ((pDataParams->bStateNow & PHPAL_I18092MPI_SW_STATE_MASK) !=
  ...
  *pRxLength = 0;
  ...
  do {
    ...
    switch (pDataParams->bStateNow & PHPAL_I18092MPI_SW_STATE_MASK) {
      case PHPAL_I18092MPI_SW_STATE_INFORMATION_PDU_TX:
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phpalI18092mPI_Sw_GetPduHeaderLength(pDataParams,
                &bHeaderLength));
        ...
        /* PDU Header has already been written */
          ...
          if ((0U == ((wHalExchangeOption & PH_EXCHANGE_BUFFERED_BIT)))) {
            PH_CHECK_SUCCESS_FCT(statusTmp, phpalI18092mPI_Sw_UpdatePduHeader(pDataParams,
                    (uint8_t)(wBytesToSend + wTxBufferLen)));
          }
        }
        break;
    ...
    if (0U != (pDataParams->bStateNow & PHPAL_I18092MPI_SW_STATE_RETRANSMIT_BIT)) {
      ...
      /* Clean-up state */
      pDataParams->bStateNow &= (uint8_t)~(uint8_t)(PHPAL_I18092MPI_SW_STATE_CHAINING_FIRST_BIT |
  ...
  while ((pDataParams->bStateNow & PHPAL_I18092MPI_SW_STATE_MASK) !=
    ...
    /* We're not done yet, chaining is in progress */
      ...
      /* Next chunk of PDU has been received */
        ...
        if (0U == (bBufferOverflow)) {
          ...
          (void)memcpy(&pRawResponse[wRxOverlappedStartPos], aRxOverlapped,
              bHeaderLength); /* PRQA S 3354 */
...
)
{
  ...
  do {
    ...
    if (((0U == ((pDataParams->bStateNow & PHPAL_I18092MPI_SW_STATE_CHAINING_BIT)))) &&
        ((0U == ((wOption & PH_TRANSMIT_BUFFERED_BIT)))) && (wBytesToSend == wTxLength)) {
...
)
{
  ...
  /* Option parameter check */
  if ((uint16_t)0U != (wOption & (uint16_t)~((uint16_t)PH_RECEIVE_CHAINING |
              (uint16_t)PH_RECEIVE_CHAINING_BUFSIZE))) {
    return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_PAL_I18092MPI);
  }
