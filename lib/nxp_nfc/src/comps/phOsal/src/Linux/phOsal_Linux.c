...
phOsal_Init(void)
{
  ...
  status = phOsal_MutexCreate((phOsal_Mutex_t *)(&gsEventMutexObj.MutexHandle),
          (pphOsal_MutexObj_t)(&gsEventMutexObj));
...
phOsal_EventCreate(phOsal_Event_t *eventHandle, pphOsal_EventObj_t eventObj)
{
  ...
  status = phOsal_Linux_GetFreeIndex(&bEventIndex, gdwEventBitMap,
          PH_OSAL_CONFIG_LINUX_MAX_NUM_EVENTS);
    ...
    phOsal_EventBits_t FlagsToWait, phOsal_EventBits_t *pCurrFlags)
{
  ...
  while (1) {
    if (((options & E_OS_EVENT_OPT_PEND_SET_ALL) &&
            (((*((uint32_t *)(*eventHandle))) & FlagsToWait) == FlagsToWait))
        || ((!(options & E_OS_EVENT_OPT_PEND_SET_ALL)) &&
            ((*((uint32_t *)(*eventHandle))) & FlagsToWait))) {
    ...
    if (ticksToWait != PHOSAL_MAX_DELAY) {
      ...
      status = pthread_cond_timedwait(&(((phOsal_LinuxEventGroup_t *)(*eventHandle))->EventCond),
              gsEventMutexObj.MutexHandle, &sTimeSpec);
      if (status != PH_OSAL_SUCCESS) {
        if (status == ETIMEDOUT) {
          status = PH_OSAL_IO_TIMEOUT;
        } else {
      ...
      status = pthread_cond_wait(&(((phOsal_LinuxEventGroup_t *)(*eventHandle))->EventCond),
              gsEventMutexObj.MutexHandle);
      if (status != PH_OSAL_SUCCESS) {
        status = PH_OSAL_FAILURE;
        break;
      }
...
phOsal_TimerStart(phOsal_Timer_t *timerHandle)
{
  ...
  if ((*timerHandle == NULL) || (pTimerObj->opt >= E_OS_TIMER_OPT_MAX) ||
      (pTimerObj->timePeriod.unitPeriod >= E_OS_TIMER_UNIT_MAX)) {
    return (PH_OSAL_ERROR | PH_COMP_OSAL);
  }
...
phOsal_TimerGetCurrent(phOsal_Timer_t *timerHandle, uint32_t *dwGetElapsedTime)
{
  ...
  *dwGetElapsedTime = (sTimerSpec.it_value.tv_sec * 1000) + (sTimerSpec.it_value.tv_nsec /
          (1000 * 1000));
