...
#include <nxp_nfc/phOsal_Config.h>
#include <windows.h>
#include <math.h>
#include <time.h>
...
gphOsalWinOsEventHandles[PH_OSAL_CONFIG_WINOS_MAX_NUM_EVENTS];
...
phOsal_EventCreate(phOsal_Event_t *eventHandle, pphOsal_EventObj_t eventObj)
{
  ...
  PH_CHECK_SUCCESS(phOsal_NullOs_GetFreeIndex(&bEventIndex, gdwEventBitMap,
          PH_OSAL_CONFIG_WINOS_MAX_NUM_EVENTS));
  ...
  phOsal_MutexCreate((phOsal_Mutex_t *)(
          &gphOsalWinOsEventHandles[bEventIndex].MutexObj.MutexHandle),
...
phOsal_TimerStart(phOsal_Timer_t *timerHandle)
{
  ...
  llDueTime = -1 * (timerObj.timePeriod.period *
          dwUnitCov[timerObj.timePeriod.unitPeriod]); /* DueTime is given in 100ns. negative value means wait relative time */
