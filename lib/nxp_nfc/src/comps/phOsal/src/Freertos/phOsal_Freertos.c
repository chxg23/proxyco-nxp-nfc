    ...
    pphOsal_StartFunc_t startFunc, void *arg)
{
  ...
  if (threadObj->stackSizeInNum) {
    PH_OSAL_CHECK_SUCCESS(phOsal_FreeRtos_GetFreeIndex(&dwFreeIndex, dwTaskBitMap,
...
phOsal_EventCreate(phOsal_Event_t *eventHandle, pphOsal_EventObj_t eventObj)
{
  ...
  PH_OSAL_CHECK_SUCCESS(phOsal_FreeRtos_GetFreeIndex(&dwFreeIndex, dwEventBitMap,
    ...
    phOsal_SemOpt_t opt)
{
  ...
  PH_OSAL_CHECK_SUCCESS(phOsal_FreeRtos_GetFreeIndex(&dwFreeIndex, dwSemBitMap,
  ...
  if (opt == E_OS_SEM_OPT_COUNTING_SEM) {
    *semHandle = xSemaphoreCreateCountingStatic(semObj->semMaxCount, semObj->semInitialCount,
            &sSemaphoreBuff[dwFreeIndex]);
  } else {
...
phOsal_MutexCreate(phOsal_Mutex_t *mutexHandle, pphOsal_MutexObj_t mutexObj)
{
  ...
  PH_OSAL_CHECK_SUCCESS(phOsal_FreeRtos_GetFreeIndex(&dwFreeIndex, dwMutexBitMap,
...
phOsal_TimerCreate(phOsal_Timer_t *timerHandle, pphOsal_TimerObj_t timerObj)
{
  ...
  PH_OSAL_CHECK_SUCCESS(phOsal_FreeRtos_GetFreeIndex(&dwFreeIndex, dwTimerBitMap,
          PH_OSAL_CONFIG_MAX_NUM_TIMERS));

  memset(&sTimerBuffer[dwFreeIndex], 0, sizeof(StaticTimer_t));

  *timerHandle = xTimerCreateStatic(((const char *)timerObj->pTimerName), TimerPeriod,
          timerObj->opt, timerObj->arg,
