...
#include <comps/phalMfdfEVx/src/phalMfdfEVx_Int.h>
    ...
    uint16_t *pDataLen)
{
  ...
  switch (bIns) {
    case PHAL_MFDFEVX_CMD_ISO7816_SELECT_FILE:
      ...
      statusTmp = phalMfdfEVx_Int_ISOSelectFile(pDataParams, pPalMifareDataParams, bOption, bCmdBuff,
              wCmdLen,
              (uint8_t)bLc, bExtendedLenApdu, pDataIn, (uint8_t)bLe, ppDataOut, pDataLen);
      break;

    case PHAL_MFDFEVX_CMD_ISO7816_UPDATE_BINARY:
      statusTmp = phalMfdfEVx_Int_ISOUpdateBinary(pDataParams, pPalMifareDataParams, bOption, bCmdBuff,
              wCmdLen,
    ...
    case PHAL_MFDFEVX_CMD_ISO7816_APPEND_RECORD:
      ...
      statusTmp = phalMfdfEVx_Int_ISOAppendRecord(pDataParams, pPalMifareDataParams, bOption, bCmdBuff,
              wCmdLen,
              bLc, bExtendedLenApdu, pDataIn, ppDataOut, pDataLen);
      break;

    case PHAL_MFDFEVX_CMD_ISO7816_GET_CHALLENGE:
      statusTmp = phalMfdfEVx_Int_ISOGetChallenge(pDataParams, pPalMifareDataParams, bOption, bCmdBuff,
              wCmdLen,
    ...
    case PHAL_MFDFEVX_CMD_ISO7816_EXT_AUTHENTICATE:
       ...
       */
      statusTmp = phalMfdfEVx_Int_ISOExternalAuthenticate(pDataParams, pPalMifareDataParams, bOption,
              bCmdBuff, wCmdLen,
              (uint8_t)bLc, bExtendedLenApdu, pDataIn, ppDataOut, pDataLen);
      break;

    case PHAL_MFDFEVX_CMD_ISO7816_INT_AUTHENTICATE:
      statusTmp = phalMfdfEVx_Int_ISOInternalAuthenticate(pDataParams, pPalMifareDataParams, bOption,
              bCmdBuff, wCmdLen,
    ...
    uint16_t *pDataLen)
{
  ...
  */
  ...
  if ((statusTmp & PH_ERR_MASK) == PHAL_MFDFEVX_ERR_DF_7816_GEN_ERROR) {
    PH_CHECK_SUCCESS_FCT(statusTmp1, phalMfdfEVx_GetConfig(pDataParams, PHAL_MFDFEVX_ADDITIONAL_INFO,
            &wVal));
  }
    ...
    uint8_t bLe, uint8_t bExtendedLenApdu, uint8_t **ppDataOut, uint16_t *pDataLen)
{
  ...
  if (0U != (bOption & 0x02U)) {
    ...
    if (bExtendedLenApdu == 0x01U) {
      ...
      bCmdBuff[wCmdLen++] = 0x00;
      bCmdBuff[wCmdLen++] =
          0x00; /* This byte is set to 0x00 since maximum bytes received will be either 0x08 or 0x10 as per ref arch ver 0.14*/
      bCmdBuff[wCmdLen++] = bLe;
  ...
  if (0U != (bOption & 0x01U)) {
    ...
    if (bExtendedLenApdu == 0x01U) {
      ...
      bCmdBuff[wCmdLen++] =
          0x00; /* first byte will be 0x00 if Ext apdu present. Next 2 byte contains actual data */
      bCmdBuff[wCmdLen++] =
          0x00; /* As of now this field will be set to 0x00 since maximum data that can be sent is 16 bytes or 32 bytes. In case if data to be sent exceeds 255 bytes, this byte shall be used  */
      bCmdBuff[wCmdLen++] = bLc;  /* Actual data to be sent */
    } else {
      /* Short APDU */
      bCmdBuff[wCmdLen++] = bLc;
  ...
  if (0U != (bOption & 0x01U)) {
    ...
    if (bExtendedLenApdu == 0x01U) {
      bCmdBuff[wCmdLen++] =
          0x00; /* first byte will be 0x00 if Ext apdu present. Next 2 byte contains actual data */
      bCmdBuff[wCmdLen++] =
          0x00; /* As of now this field will be set to 0x00 since maximum data that can be sent is 8 bytes or 16 bytes. In case if data to be sent exceeds 255 bytes, this byte shall be used  */
      bCmdBuff[wCmdLen++] = bLc;  /* Actual data to be sent */
    } else {
      /* Short APDU */
      bCmdBuff[wCmdLen++] = bLc;
