...
)
{
  ...
  if (pT1T->sSegment.bAddress == 0U) {
    ...
    /* Update Lock Bytes */
    for (wIndex = 0; wIndex < pT1T->bLockTlvCount; wIndex++) {
      if ((pT1T->asLockCtrlTlv[wIndex].wByteAddr / PHAL_TOP_T1T_SEGMENT_SIZE) ==
    ...
    for (wIndex = 0; wIndex < pT1T->bMemoryTlvCount; wIndex++) {
      if ((pT1T->asMemCtrlTlv[wIndex].wByteAddr / PHAL_TOP_T1T_SEGMENT_SIZE) ==
          (pT1T->sSegment.bAddress)) {
...
{
  if ((pT1T->sSegment.bLockReservedOtp[(wIndex % PHAL_TOP_T1T_SEGMENT_SIZE) / 8U] != 0U) &&
      (((1U << (wIndex % 8U)) != 0U))) {
...
)
{
  ...
  /* Write partially filled last block */
    ...
    if (((pDataParams->wNdefLength - pDataIndex) == 8U) && (pT1T->bTerminatorTlvPresence == PH_ON)) {
      ...
      /* Loop is needed, if current byte address is Lock, Reserved Memory
       * or OTP. Loop till we find data byte. */
      for (bIndex = ((uint8_t)((wAddress / 8U) + 1U)); (bIndex * 8U) < ((pT1T->bTms + 1U) * 8U);
          bIndex++) {
...
)
{
  ...
  for (wIndex = 0;
    ...
    aTlv[4] = (pT1T->asLockCtrlTlv[wIndex].bBytesLockedPerBit << 4U) |
        pT1T->asLockCtrlTlv[wIndex].bBytesPerPage;
