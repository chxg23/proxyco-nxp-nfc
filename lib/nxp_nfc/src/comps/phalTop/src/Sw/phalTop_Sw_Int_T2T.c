...
)
{
    ...
    for (wIndex = 0; wIndex < pT2T->bLockTlvCount; wIndex++) {
      ...
      */
      ...
      while (wLockBytesCount < wLockedBytes) {
        ...
        wTempIndex = wOffset;
        wLockedBytes = (((uint16_t)(pT2T->asLockCtrlTlv[wIndex].bSizeInBits)) / 8U);
        for (; ((wLockBytesCount < wLockedBytes) &&
                (wTempIndex <= 16U)); wLockBytesCount++, wTempIndex++) {
          if (wTempIndex != 16U) {
  ...
  /* If lock TLVs are not present, set default dynamic lock bits */
    ...
    if ((((wDefaultLockLength / 8U) % PHAL_TOP_T2T_BYTES_PER_BLOCK) != 0U) ||
        ((wDefaultLockLength % 8U) != 0U)) {
...
)
{
  ...
  for (wIndex = 0; ((wIndex < pT2T->bLockTlvCount) &&
          (wIndex < PHAL_TOP_T2T_MAX_LOCK_CTRL_TLV)); wIndex++) {
  ...
  for (wIndex = 0; ((wIndex < pT2T->bMemoryTlvCount) &&
          (wIndex < PHAL_TOP_T2T_MAX_MEM_CTRL_TLV)); wIndex++) {
    /* TLV offset from block start */
...
)
{
  ...
  if (pT2T->bTagMemoryType == PHAL_TOP_T2T_TAG_MEMORY_TYPE_DYNAMIC) {
    /* Update Lock Bytes */
    for (wIndex = 0; wIndex < pT2T->bLockTlvCount; wIndex++) {
      if ((pT2T->asLockCtrlTlv[wIndex].wByteAddr / PHAL_TOP_T2T_SECTOR_SIZE) ==
    ...
    for (wIndex = 0; wIndex < pT2T->bMemoryTlvCount; wIndex++) {
      if ((pT2T->asMemCtrlTlv[wIndex].wByteAddr / PHAL_TOP_T2T_SECTOR_SIZE) == pT2T->sSector.bAddress) {
        ...
        wBlockCount = (((uint16_t)(pT2T->asMemCtrlTlv[wIndex].bSizeInBytes)) /
                PHAL_TOP_T2T_BYTES_PER_BLOCK);
        for (wOffset = wSectorOffset;
            wOffset < (wSectorOffset +
                wBlockCount +
