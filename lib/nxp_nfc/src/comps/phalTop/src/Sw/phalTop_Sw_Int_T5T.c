...
#include <nxp_nfc/phpalSli15693.h>
      ...
      while (wIndex < PHAL_TOP_T5T_MAXIMUM_BLOCK_SIZE) {
        switch (aData[wIndex]) {
          ...
          case PHAL_TOP_T5T_NDEF_TLV:
            ...
            else if ((((pT5T->wMlen * 8U) - pT5T->wNdefHeaderAddr) == 0x101U) ||
                (((pT5T->wMlen * 8U) - pT5T->wNdefHeaderAddr) == 0x102U)) {
...
)
{
  ...
  aData[3] = (uint8_t)((pT5T->bMbRead & 0x01U) | ((pT5T->bLockBlock << 3U) & 0x08U) | ((
                  pT5T->bSplFrm << 4U) & 0x10U));
  ...
  if (pT5T->bBlockSize == PHAL_TOP_T5T_BYTES_PER_BLOCK_DEFAULT) {
    ...
    for (wIndex = (uint8_t)(pT5T->wNdefHeaderAddr / pT5T->bBlockSize);
      ...
      PH_CHECK_SUCCESS_FCT(status, phalTop_Sw_Int_T5T_Write(
              pT5T,
              wIndex,
              (&aNdefData[((wIndex - ((uint8_t)(pT5T->wNdefHeaderAddr / pT5T->bBlockSize))) *
                                                                      (pT5T->bBlockSize))]),
              pT5T->bBlockSize));
    }
  }
...
)
{
  ...
  wLoopCount = (pDataParams->wNdefLength + ((pT5T->wNdefMsgAddr) % (pT5T->bBlockSize))) /
      pT5T->bBlockSize;

  /* If entire data is not covered read one more block */
  if (0U != ((pDataParams->wNdefLength + ((pT5T->wNdefMsgAddr) % (pT5T->bBlockSize))) %
          (pT5T->bBlockSize))) {
    wLoopCount++;
  }
  ...
  if (pT5T->bMbRead == 0U) {
    ...
    for (wIndex = 0;
      ...
      if (0U != (wIndex)) {
        ...
        phMemcpy(pData, (pRxBuffer + ((pT5T->wNdefMsgAddr) % (pT5T->bBlockSize))),
            (wRxLength - ((pT5T->wNdefMsgAddr) % (pT5T->bBlockSize))));
    ...
    phMemcpy(pData, (pData + ((pT5T->wNdefMsgAddr) % (pT5T->bBlockSize))),
        (wRxLength - ((pT5T->wNdefMsgAddr) % (pT5T->bBlockSize))));
...
)
{
  ...
  if (wLength > 0xFEU) {
    ...
    if ((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) == (pT5T->bBlockSize - 1U)) {
      phMemcpy(&aLengthTemp[3], &pData[0],
          (((pT5T->bBlockSize - 3U) < wLength) ? (pT5T->bBlockSize - 3U) : wLength));
    ...
    /* Ndef Header lies at the second last byte and thus length info will split */
      ...
      phMemcpy(&aLengthTemp[2], &pData[0],
          (((pT5T->bBlockSize - 2U) < wLength) ? (pT5T->bBlockSize - 2U) : wLength));
    ...
    /* Ndef Header lies at the third last byte and thus length info will split */
      ...
      phMemcpy(&aLengthTemp[1], &pData[0],
          (((pT5T->bBlockSize - 1U) < wLength) ? (pT5T->bBlockSize - 1U) : wLength));
    ...
    /* Ndef Header lies at the fourth last byte and thus length info will split for all block length except block length of 4 */
      ...
      phMemcpy(&aLengthTemp[0], &pData[0],
          (((pT5T->bBlockSize) < wLength) ? (pT5T->bBlockSize) : wLength));
      wBytesUsed = pT5T->bBlockSize;
      wBlockNum++;
      bLengthSplit = 1;
    } else {
      phMemcpy(&aLengthTemp[(pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) + 4U], &pData[0],
          (((pT5T->bBlockSize - (((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) + 4U) %  pT5T->bBlockSize)) <
                  wLength) ? (pT5T->bBlockSize - (((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) + 4U) %
                      pT5T->bBlockSize)) : wLength));   /* PRQA S 3200 */
      wBytesUsed = pT5T->bBlockSize - (((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) + 4U) %
              (pT5T->bBlockSize));
    ...
    /* Ndef Header lies at the second last byte and thus length info will not split */
      ...
      (void)memcpy(&aLengthTemp[0], &pData[0],
          (((pT5T->bBlockSize) < wLength) ? (pT5T->bBlockSize) : wLength));
      wBytesUsed = pT5T->bBlockSize;
      wBlockNum++;
      bLengthSplit = 1;
      ...
      phMemcpy(&aLengthTemp[(pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) + 2U], &pData[0],
          (((pT5T->bBlockSize - (((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) + 2U) %
                          (pT5T->bBlockSize))) < wLength) ? (pT5T->bBlockSize - (((pT5T->wNdefHeaderAddr %
                              (pT5T->bBlockSize)) + 2U) % (pT5T->bBlockSize))) : wLength));    /* PRQA S 3200 */
      wBytesUsed = pT5T->bBlockSize - (((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) + 2U) %
              (pT5T->bBlockSize));
      wByteNum = ((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) + 1U) % (pT5T->bBlockSize);
    }
  }
  ...
  if (wLength > wBytesUsed) {
    ...
    if (0u != ((wLength - wBytesUsed) % (pT5T->bBlockSize))) {
      phMemcpy(&aTemp[0], &pData[((wLoopCount * pT5T->bBlockSize) + wBytesUsed)],
          ((wLength - wBytesUsed) % (pT5T->bBlockSize)));
      PH_CHECK_SUCCESS_FCT(status, phalTop_Sw_Int_T5T_Write(
              pT5T,
              (wBlockNum + 1U + wLoopCount),
              &aTemp[0],
  ...
  if (0U != (bLengthSplit)) {
    ...
    if (wLength > 0xFEU) {
      if ((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) == (uint16_t)(pT5T->bBlockSize - 2U)) {
        ...
        aLengthTemp[(pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) + 1U] = 0xFF;
        aLengthTemp[(pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) + 2U] = (uint8_t)((
                    wLength & 0xFF00U) >> 8U);
  ...
  if (pDataParams->wMaxNdefLength > wLength) {
    wBlockNum = ((pT5T->wNdefHeaderAddr + ((wLength > 0xFEU) ? 3U : 1U) +  wLength) /
            pT5T->bBlockSize);
    if (((pT5T->wNdefHeaderAddr + ((wLength > 0xFEU) ? 3U : 1U) +  wLength) % (pT5T->bBlockSize)) ==
        (uint16_t)(pT5T->bBlockSize - 1U)) {
      wBlockNum++;
    }
    ...
    if (((pT5T->wNdefHeaderAddr + ((wLength > 0xFEU) ? 3U : 1U) +  wLength) % (pT5T->bBlockSize)) !=
        (uint16_t)(pT5T->bBlockSize - 1U)) {
      aTemp[(((pT5T->wNdefHeaderAddr + ((wLength > 0xFEU) ? 3U : 1U) +  wLength) %
                                                                (pT5T->bBlockSize))) + 1U] = PHAL_TOP_T5T_TERMINATOR_TLV;
    } else {
